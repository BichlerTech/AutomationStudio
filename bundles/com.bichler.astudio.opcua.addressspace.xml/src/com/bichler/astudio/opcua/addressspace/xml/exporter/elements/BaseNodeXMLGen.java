...
public abstract class BaseNodeXMLGen extends BaseXMLGen {
	...
	public void writeXMLReference(BufferedWriter out, OPCInternalServer serverInstance, ReferenceNode refNode,
			NamespaceTable serverTable, NamespaceTable exportTable) throws IOException {

		NodeId nodeid = mapNodeId(refNode.getTargetId(), serverTable, exportTable);
		Node refType = serverInstance.getAddressSpaceManager().getNodeById(refNode.getReferenceTypeId());
	 ...
	 * @throws IOException
	 */
	protected void writeExtensions(BufferedWriter out) throws IOException {
		ArrayList<String> extension = Studio_ResourceManager.NODE_EXTENSIONS.get(getNode().getNodeId());
	...
	String helpParentNodeId(OPCInternalServer serverInstance, Node node, NamespaceTable exportTable) {
		NamespaceTable nsTable = serverInstance.getNamespaceUris();
		NodeId parentId = null;
		try {
			ExpandedNodeId found = serverInstance.getAddressSpaceManager().findParentInTypeHierachie(node.getNodeId());
			if (!ExpandedNodeId.isNull(found)) {
				parentId = nsTable.toNodeId(found);
			}
	...
	String helpDataType(OPCInternalServer serverInstance, Node node) {
		...
		Node datatypeNode = serverInstance.getAddressSpaceManager().getNodeById(datatypeId);
	...
	String helpBrowseName(QualifiedName browseName, NodeId mapId, boolean isZeroIndex) {
		if(!isZeroIndex) {
			return helpBrowseName(browseName, mapId);
		}
		
		return " " + BROWSENAME + "=\"" + helpString(browseName.getName()) + "\"";
	}
	
	
	String helpBrowseName(QualifiedName browseName, NodeId mappedId) {
		int nsIndex = mappedId.getNamespaceIndex();
		String name = browseName.getName();

		QualifiedName helper = new QualifiedName(nsIndex, name);

		return " " + BROWSENAME + "=\"" + helpString(helper.toString()) + "\"";
	}
	...
	void helpValue(BufferedWriter out, OPCInternalServer opcServer, NamespaceTable nsTable, Node node)
			throws IOException {
		...
		out.newLine();
		// simple typesddd
		helpWriteValue(out, opcServer, nsTable, node, value);
	...
	String helpSymbolicName(NodeId nodeId) {
		HashMap<NodeId, String> symbolicNames = Studio_ResourceManager.NODE_CUSTOMATTRIBUTES
				.get(NodesetImporter.SYMBOLICNAME);
		if (symbolicNames == null) {
			return "";
		}

		String symbolicName = symbolicNames.get(nodeId);
	...
	private void helpWriteValue(BufferedWriter out, OPCInternalServer opcServer, NamespaceTable nsTable, Node node,
			Variant value) throws IOException {
		...
		if (!isArray) {
			if (value.isEmpty()) {
				...
				writeValueSimple(out, opcServer, nsTable, "String", new Variant(""));
			} else {
				writeValueSimple(out, opcServer, nsTable, type, value);
			}
		}
		// matrix
		else if (dimensions > 1) {
			writeValueMatrix(out, opcServer, nsTable, type, value);
		}
		// array single dimension
		else {
			writeValueArray(out, opcServer, nsTable, type, value);
		}
	}

	private void writeComplexStructure(BufferedWriter out, OPCInternalServer opcServer, NamespaceTable nsTable,
			String preTabs, AbstractStructure obj) throws IOException {

//		DataDictionaryHelper ddHelper = new DataDictionaryHelper(opcServer);
//		NodeId datatypeId = NodeId.NULL;
//		ExpandedNodeId expDataTypeId = obj.getTypeId();
//
//		if (getNode() instanceof VariableNode) {
//			datatypeId = ((VariableNode) getNode()).getDataType();
//		} else if (getNode() instanceof VariableTypeNode) {
//			datatypeId = ((VariableTypeNode) getNode()).getDataType();
//		}
//
//		ByteString byteString = ddHelper.findByteStringFromNamespaceDataTypeId(datatypeId);
//		try {
//			org.w3c.dom.Node dataDict = ddHelper.findNodeFromByteString(byteString,
//					OpcUtils.getNode(opcServer, datatypeId).getBrowseName().getName());
//			String toString = ddHelper.nodeToString(dataDict);
//			System.out.println("");
...
//		}
		...
		for (Field f : fields) {
			...
			// Everythign else
				...
				out.write(preTabs + "\t\t<" + f.getName() + ">");
				writeValue(out, opcServer, nsTable, preTabs + "\t\t\t", var);
			}
			if (!useTabs) {
				out.write("</" + f.getName() + ">");
			} else {
	...
	private void writeValue(BufferedWriter out, OPCInternalServer opcServer, NamespaceTable nsTable, String preTabs,
		...
		if (obj instanceof AbstractStructure) {
			...
			writeComplexStructure(out, opcServer, nsTable, preTabs + "\t", (AbstractStructure) obj);
...
//			}
	...
	private void writeValueArray(BufferedWriter out, OPCInternalServer opcServer, NamespaceTable nsTable, String type,
			Variant value) throws IOException {
		...
		out.newLine();
		// iterate array
		boolean isNewLine = "ExtensionObject".equals(type) || "LocalizedText".equals(type);

		while (iterator.hasNext()) {
			Object obj = iterator.next();

			out.write("\t\t\t<" + type + ">");
			writeValue(out, opcServer, nsTable, "\t\t\t\t", obj);
	...
	private void writeValueMatrix(BufferedWriter out, OPCInternalServer opcServer, NamespaceTable nsTable, String type,
			Variant value) throws IOException {
	...
	private void writeValueSimple(BufferedWriter out, OPCInternalServer opcServer, NamespaceTable nsTable, String type,
			Variant value) throws IOException {
		out.write("\t\t\t<" + type + " " + XMLNS_TYPES + ">");
		writeValue(out, opcServer, nsTable, "\t\t\t\t", value.getValue());
		// \t\t\t
		out.write("</" + type + ">");
		out.newLine();
	}

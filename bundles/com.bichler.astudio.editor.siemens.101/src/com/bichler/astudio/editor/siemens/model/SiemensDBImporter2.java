...
public class SiemensDBImporter2 {
	private static FileOutputStream output = null;
	private static String pn_vdtype;
	private static String pn_clname;
	private static String struct_index;
	private static FileOutputStream dps;
	private static String pn_initvalue;

	public static void main(String[] args) {
		File out = new File("custom.xml");
		try {
			output = new FileOutputStream(out);
			if (!out.exists())
				out.createNewFile();
			File datapoints = new File("datapoints.com");
			if (!datapoints.exists())
				datapoints.createNewFile();
			dps = new FileOutputStream(datapoints);
			plc_nodes(null);
			output.flush();
			dps.flush();
			output.close();
	...
	public static void xml_header(String dbname) throws IOException {
	...
	public static void xml_objectnode(String dbname, int ns_cnt, int i_cnt) throws IOException {
		// dbname="${1}"
		// ns_cnt="${2}"
		// i_cnt="${3}"
		// #### node mit counter 1
	...
	public static void xml_objecttypenode(String dbname, int ns_cnt, int i_cnt) throws IOException {
		// dbname="${1}"
		// ns_cnt="${2}"
		// i_cnt="${3}"
		// #### node mit counter 1
	...
	public static void xml_node_header(String name, int ns_cnt, int i_cnt) throws IOException {
		// name="${1}"
		// ns_cnt="${2}"
		// i_cnt="${3}"
	...
	public static void xml_node_value_array_header(int arrdim, String clname) throws IOException {
		// arrdim=2;
		// String clname="${2}";
		// #### Arrays
	...
	// ####################################################################################################
	public static void xml_node_value_array_element(String type, String value) throws IOException {
		// type="${1}"
		// value="${2}"
	...
	// ####################################################################################################
	public static void xml_node_value_array_footer() throws IOException {
		int valuerank = 1;
	...
	// ####################################################################################################
	public static void xml_node_value_scalar(String type, String value) throws IOException {
		// type="${1}"
		// value="${2}"
		int valuerank = -1;
		// #### SCALAR
	...
	// ####################################################################################################
	public static void xml_node_footer() throws IOException {
		output.write("</Node> \n".getBytes());
	}

	// ####################################################################################################
	public static void xml_footer() throws IOException {
		output.write("</Nodes> \n".getBytes());
	...
	// ####################################################################################################
	// ####################################################################################################
	public static void com_header() throws IOException {
		// echo -n > ${pn_fcom}
	...
	// ####################################################################################################
	public static void com_datapoint(String dbname, int i_cnt, String name, String active, String dbnumb,
	...
	// ####################################################################################################
	public static void com_footer() throws IOException {
		dps.write("</DPs> \n".getBytes());
		dps.write("</DPConfiguration> \n".getBytes());
	}
	...
	// ####################################################################################################
	// ####################################################################################################
	public static void set_XML_vdtype_clname(String type) {
		// # type: Siemens-Type
		// # vdtype: OPC-Type
		// # clname: Java-Type
		// type="${1}"
	...
	public static void importFile(BufferedReader reader) {
		try {
			plc_nodes(reader);
	...
	// ####################################################################################################
	// ####################################################################################################
	public static void plc_nodes(BufferedReader reader) throws NumberFormatException, IOException {
		// pn_t1=$(date +%s)
		...
		xml_header(pn_DB); // "${@}"
		com_header();
		// BufferedReader reader = new BufferedReader(new FileReader(pn_DB +
		// ".csv"));
		String line = "";
		// ####################################################################################################
		while ((line = reader.readLine()) != null) {
			...
			if (pn_RC == 1) {
				...
				if (pn_type.toUpperCase().compareTo("DB") == 0) {
					...
					// we create an object
					xml_objectnode(pn_DB, pn_NS, pn_NC); // "${pn_DB}"
					// "${pn_NS}"
					// "${pn_NC}"
				} else {
					// we create an udt
					xml_objecttypenode(pn_DB, pn_NS, pn_NC);
				}
				// debug 1__
			}
			// ####################################################################################################
			// ### 2: TYPE (SCALAR , element of ARRAY , NO struct)
				...
				// cut -d ';' -f 4)
				pn_initvalue = line.split(";")[4].trim(); // V=$(echo "${line}"
				// |
				// cut -d ';' -f 5)
				...
				if (T.matches(pn_pat3)) { // && pn_aio == 0) {
					...
					pn_NC++;
					xml_node_header(N, pn_NS, pn_NC);
					xml_node_value_array_header(pn_elecnt, pn_clname);
					// write all initial values
					for (int i = 0; i < pn_elecnt; i++) {
						xml_node_value_array_element(pn_vdtype, pn_initvalue);
					}
					xml_node_value_array_footer();
					xml_node_footer();
					com_datapoint(pn_DB, pn_NC, N, A, pn_DN, "ARRAY_ARRAY", I, T, "[" + pn_elecnt + "]");
					...
					// "${pn_NS}"
					// "${pn_NC}"
					xml_node_value_scalar(pn_vdtype, pn_initvalue); // "${pn_vdtype}"
					// "${V:-novalue}"
					xml_node_footer();
					com_datapoint(pn_DB, pn_NC, N, A, pn_DN, "SCALAR", I, T, ""); // "${pn_DB:-noDB}"
					// "${pn_NC:-noNC}"
					// "${N:-noname}"
					// "${A}"
					// "${pn_DN:-noDN}"
					...
					if (pn_sio == 1) {
						// test "${pn_sio}" -eq 1; then struct_index="${I}"; fi
						struct_index = I;
					}
				...
				// else if (!T.matches(pn_pat3)) {// [[ "${T}" =~ ^${pn_pat3}$
				// ]];
				// // then
				// // ### valuedatatype + classname
				//
				...
				// // # starte neuen ARRAY, wenn pn_aio=1: ARRAY is open: FALSE
				// // 1
				// if (pn_aio == 1) {
				...
				// "${pn_arrdim}"
				// // "${pn_clname}"
				// com_datapoint(pn_DB, pn_NC, N, A, pn_DN, "ARRAY_ARRAY", I, T,

 ...
 * drag is initiated from a <code>Grid</code>.
 * 
 * <p>
 * Classes that wish to provide their own source image for a <code>Grid</code>
 * can extend <code>DragSourceAdapter</code> class and override the
...
public class GridDragSourceEffect extends DragSourceEffect {
	...
	/**
	 * This implementation of <code>dragFinished</code> disposes the image that was
	 * created in <code>GridDragSourceEffect.dragStart</code>.
	 * 
	 * Subclasses that override this method should call
	 * <code>super.dragFinished(event)</code> to dispose the image in the default
	 * implementation.
	 * 
	...
	/**
	 * This implementation of <code>dragStart</code> will create a default image
	 * that will be used during the drag. The image should be disposed when the drag
	 * is completed in the <code>GridDragSourceEffect.dragFinished</code> method.
	 * 
	 * Subclasses that override this method should call
	 * <code>super.dragStart(event)</code> to use the image from the default
	 * implementation.
	 * 
	...
	Image getDragSourceImage(DragSourceEvent event) {
		...
		for (int i = 0; i < selection.length; i++) {
			...
			gc.setClipping(currBounds.x - bounds.x - 1, currBounds.y - bounds.y - 1, currBounds.width + 2,
					currBounds.height + 2);

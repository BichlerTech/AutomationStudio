 ...
 * Chuck.Mastrandrea@sas.com - wordwrapping in bug 222280 smcduff@hotmail.com -
 * wordwrapping in bug 222280 Claes Rosell<claes.rosell@solme.se> - rowspan in
 * bug 272384 Marco Maccaferri<macca@maccasoft.com> - fixed arrow scrolling in
 * bug 294767 higerinbeijing@gmail.com . fixed selectionEvent.item in bug 286617
 * balarkrishnan@yahoo.com - fix in bug 298684 Enrico
 * Schnepel<enrico.schnepel@randomice.net> - new API in 238729, bugfix in
 * 294952, 322114 Benjamin Bortfeldt<bbortfeldt@gmail.com> - new tooltip support
 * in 300797 Thomas Halm <thha@fernbach.com> - bugfix in 315397 Justin Dolezy
 * <justin@neckdiagrams.com> - bugfix in 316598 Cosmin Ghita <cghita@ansis.eu> -
 * bugfix in 323687 Pinard-Legry Guilhaume <guilhaume_pl@yahoo.fr> - bugfix in
 * 267057
 *******************************************************************************/
...
public class Grid extends Canvas {
	...
	public int[] getSelectionIndices() {
		...
		if (!cellSelectionEnabled) {
			...
			int i = 0;
			for (Iterator<Object> itemIterator = selectedItems.iterator(); itemIterator.hasNext();) {
	...
	public int getTopIndex() {
		...
		if (!vScroll.getVisible()) {
			...
			if (isTree) {
				Iterator<Object> itemsIter = items.iterator();
	 ...
	 * @see RowRange
	 */
	private RowRange getRowRange(int startIndex, int availableHeight, boolean forceEndCompletelyInside,
			boolean inverse) {
		// parameter preparation
		if (startIndex == -1) {
			if (!inverse) {
		...
		if (isTree || hasDifferingHeights) {
			...
			// note: we use "+2" in next line, because we only try to add another row
			// if there
	...
	public void showItem(GridItem item) {
		...
		if (newTopIndex >= getBottomIndex()) {
			RowRange range = getRowRange(newTopIndex, getVisibleGridHeight(), true, true); // note:
																							// inverse==true
				...
				if (columnGroups.length != 0) {
					...
					if (dragDropBeforeColumn.getColumnGroup() == columnBeingPushed.getColumnGroup()) {
						...
						if (dragDropBeforeColumn.getColumnGroup() == null) {
							...
							insertAtIndex = displayOrderedColumns.indexOf(dragDropBeforeColumn);
							while (insertAtIndex > 0 && ((GridColumn) displayOrderedColumns.get(insertAtIndex - 1))
									.getColumnGroup() == beforeGroup) {
								insertAtIndex--;
							}
						}
					}
	...
	private void onPaint(PaintEvent e) {
		int insertMarkPosX1 = -1; // we will populate these values while drawing the
									// cells
		...
		if (hasSpanning) {
			// We need to find the first Item to draw. An earlier item can row-span
	...
	private Event updateCellSelection(Vector<Point> newCells, int stateMask, boolean dragging,
		...
		if (!shift && !ctrl) {
			...
			Point newCell = (Point) newCells.get(0); // shift selection should only
														// occur with one
	...
	private void onDispose(Event event) {
		// We only want to dispose of our items and such *after* anybody else who
		// may have been
	...
	private void onKeyDown(Event e) {
		...
		// These two variables are used because the key navigation when the shift
		// key is down is
		// based, not off the focus item/column, but rather off the implied focus
		// (i.e. where the
	...
	private boolean handleCellHover(int x, int y) {
		...
		if (hoverChange || hoveringOverText != overText) {
			...
			if (overText) {
				...
				if (hoveringItem != null && hoveringItem.getToolTipText(indexOf(col)) == null && // no
																									// inplace
																									// tooltips
																									// when
																									// regular
																									// tooltip
						!col.getWordWrap()) // dont show inplace tooltips for cells with
											// wordwrap
				{
				...
				if (textBounds != null && textBounds.width < preferredTextBounds.width) {
					...
					// the following 2 lines are done here rather than in showToolTip to
					// allow
		...
		if (hoverChange) {
			...
			// Avoid unnecessarily resetting tooltip - this will cause the tooltip to
			// jump around
	...
	int newItem(GridItem item, int index, boolean root) {
		...
		// Have to convert indexes, this method needs a flat index, the method is
		// called with indexes
	...
	private Point getSelectionRange(GridItem fromItem, GridColumn fromColumn, GridItem toItem, GridColumn toColumn) {
		...
		do {
			...
			if (cols.x != fromIndex || cols.y != toIndex) {
				...
				// Unfortunately we have to start all over again from the top with the
				// new range
	...
	void recalculateRowHeaderWidth(GridItem item, int oldWidth, int newWidth) {
		...
		if (newWidth > rowHeaderWidth) {
			...
			// if the changed width is smaller, and the previous width of that rows
			// header was equal
			// to the current row header width then its possible that we may need to
			// make the new
			// row header width smaller, but to do that we need to ask all the rows
			// all over again
		...
		accessible.addAccessibleControlListener(new AccessibleControlAdapter() {
			...
			public void getLocation(AccessibleControlEvent e) {
				...
				if (childID >= 0 && childID < items.size()) {
					...
					GridColumnGroup columnGroup = getColumnGroup(
							childID - items.size() - columns.size() - columnGroups.length);
	...
	private boolean handleHoverOnSelectionDragArea(int x, int y) {
		...
		if ((!rowHeaderVisible || x > rowHeaderWidth - SELECTION_DRAG_BORDER_THRESHOLD)
			...
			// !swSel || !seSel);
			//// inSelection = neSel ? neP : nwSel ? nwP : swSel ? swP : seSel ? seP :
			// null;
			// }
			// else
			// {

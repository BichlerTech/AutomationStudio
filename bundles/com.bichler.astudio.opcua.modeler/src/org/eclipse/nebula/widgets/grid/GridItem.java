 ...
 *
 * Contributors: chris.gross@us.ibm.com - initial API and implementation Claes
 * Rosell<claes.rosell@solme.se> - rowspan in bug 272384 Stefan
 * Widmaier<stefan.widmaier@faktorzehn.de> - rowspan in 304797
 *******************************************************************************/
...
public class GridItem extends Item {
	...
	/**
	 * (SWT.VIRTUAL only) Flag that specifies whether the client has already been
	 * sent a SWT.SetData event.
	 */
	private boolean hasSetData = false;

	/**
	 * Creates a new instance of this class and places the item at the end of the
	 * grid.
	 *
	...
	/**
	 * Creates a new instance of this class and places the item in the grid at the
	 * given index.
	 *
	 * @param parent parent grid
	 * @param style  item style
	...
	/**
	 * Creates a new instance of this class as a child node of the given GridItem
	 * and places the item at the end of the parents items.
	 *
	...
	/**
	 * Creates a new instance of this class as a child node of the given Grid and
	 * places the item at the given index in the parent items list.
	 *
	 * @param parent parent item
	 * @param style  item style
	 ...
	 * the row is resized, by sending it one of the messages defined in the
	 * <code>ControlListener</code> interface.
	 * <p>
	 * Clients who wish to override the standard row resize logic should use the
	 * untyped listener mechanisms. The untyped <code>Event</code> object passed to
	 * an untyped listener will have its <code>detail</code> field populated with
	 * the new row height. Clients may alter this value to, for example, enforce
	 * minimum or maximum row sizes. Clients may also set the <code>doit</code>
	...
	/**
	 * Fires the given event type on the parent Grid instance. This method should
	 * only be called from within a cell renderer. Any other use is not intended.
	 *
	 ...
	 * listener (all columns). This method manages that behavior. This method should
	 * only be called from within a cell renderer. Any other use is not intended.
	 *
	...
	/**
	 * Returns <code>true</code> if the first column in the receiver is grayed, and
	 * false otherwise. When the GridColumn does not have the <code>CHECK</code>
	 * style, return false.
	 *
	...
	/**
	 * Returns <code>true</code> if the column at the given index in the receiver is
	...
	/**
	 * Returns the receiver's parent item, which must be a <code>GridItem</code> or
	 ...
	 * @throws IllegalArgumentException
	 *                                  <ul>
	 *                                  <li>ERROR_INVALID_ARGUMENT - if the argument
	 *                                  has been disposed</li>
	 *                                  </ul>
	 * @throws SWTException
	 *                                  <ul>
	...
	/**
	 * Sets the column spanning for the column at the given index to span the given
	 * number of subsequent columns.
	 *
	...
	/**
	 * Sets the row spanning for the row at the given index to span the given number
	 * of subsequent rows.
	 *
	...
	/**
	 * Sets the grayed state of the checkbox for the first column. This state change
	 * only applies if the GridColumn was created with the SWT.CHECK style.
	 *
	...
	/**
	 * Sets the grayed state of the checkbox for the given column index. This state
	 * change only applies if the GridColumn was created with the SWT.CHECK style.
	 *
	 * @param index  the column index
	 * @param grayed the new grayed state of the checkbox;
	 * @throws org.eclipse.swt.SWTException
	...
	public void setHeight(int newHeight) {
		...
		if (isVisible()) {
			...
			if (parent.getTopIndex() <= myIndex && myIndex <= parent.getBottomIndex()) // note:
																						// cannot
																						// use
				// Grid#isShown()
				// here, because
	...
	/**
	 * Sets the visible state of this item. The visible state is determined by the
	 * expansion state of all of its parent items. If all parent items are expanded
	 * it is visible.
	 *
	...
	/**
	 * Returns the receiver's row header text. If the text is <code>null</code> the
	 * row header will display the row number.
	 *
	 * @return the text stored for the row header or code <code>null</code> if the
	 *         default has to be displayed
	 * @throws org.eclipse.swt.SWTException
	 *                                      <ul>
	 *                                      <li>ERROR_WIDGET_DISPOSED - if the
	 *                                      receiver has been disposed</li>
	 ...
	 *
	 * @return the image stored for the header or <code>null</code> if none has to
	 *         be displayed
	 * @throws org.eclipse.swt.SWTException
	 *                                      <ul>
	 *                                      <li>ERROR_WIDGET_DISPOSED - if the
	...
	/**
	 * Sets the receiver's row header text. If the text is <code>null</code> the row
	 * header will display the row number.
	 *
	 * @param text the new text
	 * @throws IllegalArgumentException
	 *                                      <ul>
	...
	/**
	 * Sets the receiver's row header image. If the image is <code>null</code> none
	...
	/**
	 * Returns the checkable state at the given column index in the receiver. If the
	 * column at the given index is not checkable then this will return false
	 * regardless of the individual cell's checkable state.
	 *
	 * @param index the column index
	 * @return the checked state
	 * @throws org.eclipse.swt.SWTException
	 ...
	 * checkbox which is uncheckable will not be modifiable by the user but still
	 * make be modified programmatically. If the column at the given index is not
	 * checkable then individual cell will not be checkable regardless.
	 *
	 * @param index   the column index
	 * @param checked the new checked state
	 * @throws org.eclipse.swt.SWTException
	 ...
	 *
	 * @param allChildren <code>true</code> if all child items should be cleared

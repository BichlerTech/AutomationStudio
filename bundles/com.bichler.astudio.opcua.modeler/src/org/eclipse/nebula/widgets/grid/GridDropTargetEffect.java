 ...
 * and expand) when a drag occurs over a <code>Grid</code>.
 * 
 * <p>
 * Classes that wish to provide their own drag under effect for a
 * <code>Grid</code> can extend the <code>DropTargetAdapter</code> class and
...
public class GridDropTargetEffect extends DropTargetEffect {
	...
	/**
	 * Creates a new <code>GridDropTargetEffect</code> to handle the drag under
	 * effect on the specified <code>Grid</code>.
	 * 
	 * @param grid the <code>Grid</code> over which the user positions the cursor to
	 *             drop the data
	 */
	...
	/**
	 * Set this value to true to make drop feedback in <code>Grid</code> always
	 * behave like the Grid was <em>not</em> in cell selection mode. The default is
	 * false.
	 * 
	 * <p>
	 * A value of true, means that for {@link DND#FEEDBACK_SELECT} full rows will be
	 * selected instead of cells and for {@link DND#FEEDBACK_INSERT_AFTER} and
	 ...
	 * effect for the feedback specified in <code>event.feedback</code>.
	 * 
		...
		if ((effect & DND.FEEDBACK_SCROLL) == 0) {
			...
			if (hoverItem != null && scrollItem == hoverItem && scrollBeginTime != 0) {
				if (System.currentTimeMillis() >= scrollBeginTime) {
					...
					GridItem nextItem = hoverItem == topItem ? grid.getPreviousVisibleItem(hoverItem)
							: grid.getNextVisibleItem(hoverItem);

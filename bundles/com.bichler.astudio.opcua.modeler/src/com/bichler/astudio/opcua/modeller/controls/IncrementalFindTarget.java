 ...
 * @since 2.0
 */
class IncrementalFindTarget implements IFindReplaceTarget, IFindReplaceTargetExtension, VerifyKeyListener,
	...
	private final static String TAB = JSEditorMessages.Editor_FindIncremental_render_tab;
	/**
	 * The string representing "Reverse Incremental Find"
	 * 
	 * @since 3.0
	 */
	private final static String FIELD_NAME = JSEditorMessages.Editor_FindIncremental_name;
	/**
	 * The string representing "Incremental Find"
	 * 
	 * @since 3.0
	 */
	private final static String REVERSE_FIELD_NAME = JSEditorMessages.Editor_FindIncremental_reverse_name;
	/**
	 * The string representing reverse
	 * 
	 * @since 2.1
	 */
	private final static String REVERSE = JSEditorMessages.Editor_FindIncremental_reverse;
	/**
	 * The string representing wrapped
	 * 
	 * @since 2.1
	 */
	private final static String WRAPPED = JSEditorMessages.Editor_FindIncremental_wrapped;
	...
	/**
	 * A flag indicating that a search is currently active. Used to ignore selection
	 * callbacks generated by the incremental search itself.
	 * 
	 * @since 2.1
	 */
	...
	/**
	 * Tells whether the status field implements <code>IStatusFieldExtension</code>.
	 * 
	...
	private class SearchResult {
		...
		/**
		 * Creates a new search result data object and fills it with the current values
		 * of this target.
		 */
		public SearchResult() {
	...
	/**
	 * Sets the direction for the next search. This can be called before
	 * <code>beginSession</code> to set the initial search direction.
	 * 
	...
	public void beginSession() {
		...
		if (fTextViewer instanceof ITextViewerExtension) {
			...
			if (fTextViewer instanceof ITextViewerExtension5)
				modelOffset = fCurrentIndex == -1 ? -1
						: ((ITextViewerExtension5) fTextViewer).widgetOffset2ModelOffset(fCurrentIndex);
			else
	...
	private void updateStatus() {
		...
		if (!fFound) {
			String pattern = JSEditorMessages.Editor_FindIncremental_not_found_pattern;
			...
			String pattern = JSEditorMessages.Editor_FindIncremental_found_pattern;
	...
	public void verifyKey(VerifyEvent event) {
		...
		if (event.character == 0) {
			...
			default:
				...
				if (stateMask == 0 || stateMask == SWT.SHIFT || !Util.isMac() && stateMask == (SWT.ALT | SWT.CTRL) // AltGr
																													// (see
																													// bug
																													// 43049)
						|| Util.isMac() && (stateMask == (SWT.ALT | SWT.SHIFT) || stateMask == SWT.ALT)) { // special
																											// chars on
																											// Mac (bug
																											// 272994)
					saveState();
	...
	private boolean repeatSearch(boolean forward) {
		...
		StyledText text = fTextViewer.getTextWidget();
		// Cannot use fTarget.getSelection since that does not return which side of
		// the
	...
	/**
	 * Adds the given character to the search string and repeats the search with the
	 * last parameters.
	 *
	 ...
	 * @return the given string with all tab characters replace with a proper status
	 *         line presentation
	 */
	...
	/*
	 * @see ISelectionChangedListener#selectionChanged(org.eclipse.jface.viewers.

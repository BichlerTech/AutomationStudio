...
public class JSSourceViewer extends SourceViewer {
	public JSSourceViewer(Composite parent, IVerticalRuler ruler, int styles) {
	...
	//
	// /**
	// * parent can be an editorpart, so we need to bring any informations to
	// * them.
	// */
	// private EditorPart fParentEditorPart = null;
	//
	// /**
	// * parent can be an swt control.
	// */
	...
	//
	// if (fDisplay == null) {
	// if (fParentEditorPart != null) {
	// fDisplay = fParentEditorPart.getSite().getShell().getDisplay();
	// } else if (fParentControl != null) {
	// fDisplay = fParentControl.getDisplay();
	// }
	// }
	// Assert.isLegal(fDisplay != null);
	// return;
	// }
	//
	...
	// if (fParentEditorPart != null) {
	// fParentShell = fParentEditorPart.getSite().getShell();
	// } else if (fParentControl != null) {
	// fParentShell = fParentControl.getShell();
	// }
	// }
	// Assert.isLegal(fParentShell != null);
	// return;
	// }
	//
	// /**
	...
	// "selectionHorizontalPixel"; //$NON-NLS-1$
	//
	// /**
	...
	// */
	// final static InsertMode INSERT = null;
	// /**
	// * Represents the smart insert mode.
	// */
	// final static InsertMode SMART_INSERT = null;
	//
	// /**
	...
	//
	// /*
	// * Detectors for editor input changes during the process of state
	// * validation.
	// */
	...
	//
	// /**
	// * Internal <code>VerifyListener</code> for performing the state
	// * validation of the editor input in case of the first attempted
	// * manipulation via typing on the keyboard.
	// *
	// * @since 2.0
	// */
	...
	// /*
	// * @see
	// * IElementStateListenerExtension#elementStateValidationChanged(Object,
	// * boolean)
	// *
	// * @since 2.0
	// */
	...
	// if (element != null && element.equals(getViewerInput())) {
	...
	// && deletedElement.equals(getViewerInput())) {
	// Runnable r = new Runnable() {
	// public void run() {
	// enableSanityChecking(true);
	// close(false);
	...
	// && originalElement.equals(getViewerInput())) {
	...
	// return;
	//
	// if (!canHandleMove((IEditorInput) originalElement,
	...
	// changed = d.getDocument(getViewerInput());
	// if (changed != null) {
	// if (wasDirty)
	...
	// public void run() {
	//
	// validateState(getViewerInput());
	// d.getDocument(getViewerInput()).set(
	// previousContent);
	...
	// * about why the parameter <code>postAsync</code> has been introduced in
	// * the course of 3.1.
	// *
	// * @param runnable
	// * runnable to be executed
	// * @param postAsync
	// * <code>true</code> if the runnable must be posted
	...
	// IEditorInput input = getViewerInput();
	// IDocument document = getDocumentProvider().getDocument(
	// input);
	//
	...
	// fLocalLastEditPosition = new Position(
	...
	//
	// // IMPORTANT: Do not call isBlockSelectionModeEnabled() before
	// // checking the property!
	//
	// if (BLOCK_SELECTION_MODE_FONT.equals(property)
	// && isBlockSelectionModeEnabled()) {
	// Font blockFont = JFaceResources
	// .getFont(BLOCK_SELECTION_MODE_FONT);
	...
	// }
	//
	// fParentEditorPart.getEditorSite().getKeyBindingService();
	//
	...
	// && (fKeyCode == -1 || event.keyCode == fKeyCode) && (fStateMask ==
	// SWT.DEFAULT || event.stateMask == fStateMask));
	// }
	// }
	//
	// /**
	...
	// if (part == JSSourceViewer.this && fMementoToRestore != null
	// && containsSavedState(fMementoToRestore)) {
	...
	// return;
	//
	// if (fActivePart == JSSourceViewer.this
	// || fActivePart != null
	// && fActivePart.getAdapter(JSSourceViewer.class) == JSSourceViewer.this) {
	// fIsHandlingActivation = true;
	// try {
	// safelySanityCheckState(getViewerInput());
	// } finally {
	...
	//
	// /**
	// * Internal action to scroll the editor's viewer by a specified number of
	// * lines.
	// *
	// * @since 2.0
	// */
	...
	//
	// /**
	// * Creates a new scroll action that scroll the given number of lines. If
	// * the increment is &lt; 0, it's scrolling up, if &gt; 0 it's scrolling
	// * down.
	// *
	...
	// fSourceViewer.setTopIndex(extension
	// .widgetLine2ModelLine(newTopIndex));
	// } else {
	...
	//
	// /**
	// * This action implements smart end. Instead of going to the end of a line
	// * it does the following: - if smart home/end is enabled and the caret is
	// * before the line's last non-whitespace and then the caret is moved
	// * directly after it - if the caret is after last non-whitespace the caret
	// * is moved at the end of the line - if the caret is at the end of the line
	// * the caret is moved directly after the line's last non-whitespace
	// * character
	// *
	...
	// try {
	// lineOffset = document
	// .getLineInformationOfOffset(offset).getOffset();
	...
	//
	// /**
	// * This action implements smart home. Instead of going to the start of a
	// * line it does the following: - if smart home/end is enabled and the caret
	// * is after the line's first non-whitespace then the caret is moved directly
	// * before it - if the caret is before the line's first non-whitespace the
	// * caret is moved to the beginning of the line - if the caret is at the
	// * beginning of the line the caret is moved directly before the line's first
	...
	// */
	// public LineStartAction(final StyledText textWidget,
	...
	// ((IVerticalRulerExtension) fVerticalRuler)
	// .setLocationOfLastMouseButtonActivity(location.x,
	// location.y);
	//
	...
	// {
	// IPostSelectionProvider provider = (IPostSelectionProvider) fSourceViewer
	...
	// return fSelectionListener != null
	// && fSelectionListener.isValid(postSelection);
	// }
	// }
	//
	// /**
	...
	// IPostSelectionProvider provider = (IPostSelectionProvider)
	// selectionProvider;
	...
	// * @since 3.0
	// */
	// private class SelectionListener extends AbstractSelectionChangedListener
	...
	// return fPostSelection != INVALID_SELECTION
	// && fPostSelection == selection;
	// }
	//
	...
	// private final List<Object> fColumns;
	//
	// /**
	// * Creates a new column support for the given editor. Only the editor
	// * itself should normally create such an instance.
	// *
	// * @param editor
	// * the editor
	// * @param registry
	...
	// */
	// public ColumnSupport(AbstractTextEditor editor,
	...
	// /*
	// * @see
	// * org.eclipse.ui.texteditor.IColumnSupport#setColumnVisible(java.lang
	// * .String, boolean)
	// */
	...
	// }
	//
	// private void addColumn(final CompositeRuler ruler,
	// final RulerColumnDescriptor descriptor) {
	//
	// final int idx = computeIndex(ruler, descriptor);
	//
	// SafeRunnable runnable = new SafeRunnable() {
	// public void run() throws Exception {
	// IContributedRulerColumn column = descriptor
	// .createColumn(fEditor);
	// fColumns.add(column);
	...
	// }
	//
	// private void removeColumn(final CompositeRuler ruler,
	// final RulerColumnDescriptor descriptor) {
	// removeColumn(ruler, getVisibleColumn(ruler, descriptor));
	// }
	//
	// private void removeColumn(final CompositeRuler ruler,
	// final IContributedRulerColumn rulerColumn) {
	// if (rulerColumn != null) {
	// SafeRunnable runnable = new SafeRunnable() {
	// public void run() throws Exception {
	// if (ruler != null)
	...
	// *
	// * @param ruler
	// * the composite ruler to scan
	// * @param descriptor
	// * the descriptor of the column of interest
	// * @return the matching column or <code>null</code>
	// */
	// private IContributedRulerColumn getVisibleColumn(CompositeRuler ruler,
	...
	//
	// /**
	// * Computes the insertion index for a column contribution into the
	// * currently visible columns.
	// *
	// * @param ruler
	// * the composite ruler into which to insert the column
	// * @param descriptor
	// * the descriptor to compute the index for
	// * @return the insertion index for a new column
	// */
	// private int computeIndex(CompositeRuler ruler,
	// RulerColumnDescriptor descriptor) {
	// int index = 0;
	...
	// IVerticalRulerColumn column = (IVerticalRulerColumn) it.next();
	// if (column instanceof IContributedRulerColumn) {
	// RulerColumnDescriptor rcd = ((IContributedRulerColumn) column)
	// .getDescriptor();
	// if (rcd != null && all.indexOf(rcd) > newPos)
	// break;
	// } else if
	// ("org.eclipse.jface.text.source.projection.ProjectionRulerColumn".equals(column.getClass().getName()))
	// { //$NON-NLS-1$
	...
	// /*
	// * @see
	// * org.eclipse.ui.texteditor.IColumnSupport#isColumnVisible(java.lang
	// * .String)
	// */
	...
	// /*
	// * @see
	// * org.eclipse.ui.texteditor.IColumnSupport#isColumnSupported(java.lang
	// * .String)
	// */
	...
	//
	// /**
	// * Returns the editor's vertical ruler, if it is a
	// * {@link CompositeRuler}, <code>null</code> otherwise.
	// *
	...
	// */
	// public void dispose() {
	// for (Iterator iter = new ArrayList(fColumns).iterator(); iter
	// .hasNext();)
	...
	// super(resourceBundle, prefix, JSSourceViewer.this);
	...
	// && makeAnnotationHoverFocusable(annotationHover))
	// return;
	// }
	//
	...
	// if (textHover instanceof IInformationProviderExtension2) // this
	// // is
	// // conceptually
	// // wrong,
	// // but
	...
	//
	// IInformationProvider informationProvider = new
	// FocusedInformationPresenter.InformationProvider(
	// hoverRegion, hoverInfo, controlCreator);
	//
	// FocusedInformationPresenter informationPresenter =
	...
	// informationPresenter.setMargins(6, 6); // default values from
	// // AbstractInformationControlManager
	// String contentType = TextUtilities.getContentType(sourceViewer
	// .getDocument(), getSourceViewerConfiguration()
	...
	// return false;
	//
	// return getInformationPresenter().openFocusedAnnotationHover(
	// annotationHover, line);
	// }
	//
	// /**
	...
	// fInformationPresenter = new FocusedInformationPresenter(
	...
	// "AbstractTextEditor.Color.Foreground"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.Background"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.Foreground.SystemDefault"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.Background.SystemDefault"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.SelectionForeground"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.SelectionBackground"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.SelectionForeground.SystemDefault"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.SelectionBackground.SystemDefault"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Color.FindScope"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Navigation.SmartHomeEnd"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Accessibility.UseCustomCarets"; //$NON-NLS-1$
	// /**
	...
	// "AbstractTextEditor.Accessibility.WideCaret"; //$NON-NLS-1$
	// /**
	...
	// "hyperlinkKeyModifier"; //$NON-NLS-1$
	// /**
	// * A named preference that controls the key modifier mask for hyperlinks.
	// * The value is only used if the value of
	// * <code>PREFERENCE_HYPERLINK_KEY_MODIFIER</code> cannot be resolved to
	// * valid SWT modifier bits.
	// * <p>
	// * Value is of type <code>String</code>.
	// * </p>
	// *
	...
	// "hyperlinkKeyModifierMask"; //$NON-NLS-1$
	// /**
	...
	// "showWhitespaceCharacters"; //$NON-NLS-1$
	//
	// /**
	...
	// "showLeadingSpaces"; //$NON-NLS-1$
	//
	// /**
	// * A named preference that controls the display of enclosed Space
	...
	// "showEnclosedSpaces"; //$NON-NLS-1$
	//
	// /**
	// * A named preference that controls the display of trailing Space
	// * characters. The value is used only if the value of
	// * {@link #PREFERENCE_SHOW_WHITESPACE_CHARACTERS} is <code>true</code>.
	// * <p>
	// * Value is of type <code>Boolean</code>.
	...
	// "showLeadingIdeographicSpaces"; //$NON-NLS-1$
	//
	// /**
	// * A named preference that controls the display of enclosed Ideographic
	// * Space characters. The value is used only if the value of
	// * {@link #PREFERENCE_SHOW_WHITESPACE_CHARACTERS} is <code>true</code>.
	// * <p>
	// * Value is of type <code>Boolean</code>.
	// * </p>
	...
	// "showEnclosedIdeographicSpaces"; //$NON-NLS-1$
	//
	// /**
	// * A named preference that controls the display of trailing Ideographic
	// * Space characters. The value is used only if the value of
	// * {@link #PREFERENCE_SHOW_WHITESPACE_CHARACTERS} is <code>true</code>.
	// * <p>
	// * Value is of type <code>Boolean</code>.
	...
	// "showTrailingIdeographicSpaces"; //$NON-NLS-1$
	//
	// /**
	...
	// "showTrailingTabs"; //$NON-NLS-1$
	//
	// /**
	// * A named preference that controls the display of Carriage Return
	// * characters. The value is used only if the value of
	// * {@link #PREFERENCE_SHOW_WHITESPACE_CHARACTERS} is <code>true</code>.
	// * <p>
	// * Value is of type <code>Boolean</code>.
	...
	//
	// /**
	// * A named preference that controls the alpha value of whitespace
	// * characters. The value is used only if the value of
	// * {@link #PREFERENCE_SHOW_WHITESPACE_CHARACTERS} is <code>true</code>.
	// * <p>
	...
	// "whitespaceCharacterAlphaValue"; //$NON-NLS-1$
	//
	// /**
	...
	// "textDragAndDropEnabled"; //$NON-NLS-1$
	//
	// /**
	...
	// * </p>
	// * <ul>
	// * <li>-1: <code>null</code> (don't allow moving the mouse into a
	// hover),</li>
	// * <li>0:
	// * {@link
	// org.eclipse.jface.text.ITextViewerExtension8.EnrichMode#AFTER_DELAY}
	// * ,</li>
	// * <li>1:
	// * {@link
	// org.eclipse.jface.text.ITextViewerExtension8.EnrichMode#IMMEDIATELY}
	// * ,</li>
	// * <li>2:
	// * {@link org.eclipse.jface.text.ITextViewerExtension8.EnrichMode#ON_CLICK}.
	// * </li>
	// * </ul>
	// *
	// * @since 3.4
	...
	//
	// /**
	// * Menu id used to contribute to the editor context menu of all textual
	// * editors.
	// *
	// * @since 3.5
	// */
	// public static final String COMMON_EDITOR_CONTEXT_MENU_ID =
	// "#AbstractTextEditorContext"; //$NON-NLS-1$
	//
	// /**
	// * Menu id used to contribute to the ruler context menu of all textual
	// * editors.
	// *
	// * @since 3.5
	// */
	// public static final String COMMON_RULER_CONTEXT_MENU_ID =
	// "#AbstractTextEditorRulerContext"; //$NON-NLS-1$
	//
	...
	// new IdMapEntry(ITextEditorActionDefinitionIds.LINE_DOWN,
	// ST.LINE_DOWN),
	// new IdMapEntry(ITextEditorActionDefinitionIds.LINE_START,
	// ST.LINE_START),
	// new IdMapEntry(ITextEditorActionDefinitionIds.LINE_END, ST.LINE_END),
	// new IdMapEntry(ITextEditorActionDefinitionIds.COLUMN_PREVIOUS,
	// ST.COLUMN_PREVIOUS),
	// new IdMapEntry(ITextEditorActionDefinitionIds.COLUMN_NEXT,
	// ST.COLUMN_NEXT),
	// new IdMapEntry(ITextEditorActionDefinitionIds.PAGE_UP, ST.PAGE_UP),
	// new IdMapEntry(ITextEditorActionDefinitionIds.PAGE_DOWN,
	// ST.PAGE_DOWN),
	// new IdMapEntry(ITextEditorActionDefinitionIds.WORD_PREVIOUS,
	// ST.WORD_PREVIOUS),
	// new IdMapEntry(ITextEditorActionDefinitionIds.WORD_NEXT,
	// ST.WORD_NEXT),
	// new IdMapEntry(ITextEditorActionDefinitionIds.TEXT_START,
	// ST.TEXT_START),
	// new IdMapEntry(ITextEditorActionDefinitionIds.TEXT_END, ST.TEXT_END),
	// new IdMapEntry(ITextEditorActionDefinitionIds.WINDOW_START,
	// ST.WINDOW_START),
	// new IdMapEntry(ITextEditorActionDefinitionIds.WINDOW_END,
	// ST.WINDOW_END),
	// // selection
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_LINE_UP,
	// ST.SELECT_LINE_UP),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_LINE_DOWN,
	// ST.SELECT_LINE_DOWN),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_LINE_START,
	// ST.SELECT_LINE_START),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_LINE_END,
	// ST.SELECT_LINE_END),
	// new IdMapEntry(
	// ITextEditorActionDefinitionIds.SELECT_COLUMN_PREVIOUS,
	// ST.SELECT_COLUMN_PREVIOUS),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_COLUMN_NEXT,
	// ST.SELECT_COLUMN_NEXT),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_PAGE_UP,
	// ST.SELECT_PAGE_UP),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_PAGE_DOWN,
	// ST.SELECT_PAGE_DOWN),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_WORD_PREVIOUS,
	// ST.SELECT_WORD_PREVIOUS),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_WORD_NEXT,
	// ST.SELECT_WORD_NEXT),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_TEXT_START,
	// ST.SELECT_TEXT_START),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_TEXT_END,
	// ST.SELECT_TEXT_END),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_WINDOW_START,
	// ST.SELECT_WINDOW_START),
	// new IdMapEntry(ITextEditorActionDefinitionIds.SELECT_WINDOW_END,
	// ST.SELECT_WINDOW_END),
	// // modification
	...
	// new IdMapEntry(ITextEditorActionDefinitionIds.DELETE_PREVIOUS,
	// ST.DELETE_PREVIOUS),
	// new IdMapEntry(ITextEditorActionDefinitionIds.DELETE_NEXT,
	// ST.DELETE_NEXT),
	// new IdMapEntry(ITextEditorActionDefinitionIds.DELETE_PREVIOUS_WORD,
	// ST.DELETE_WORD_PREVIOUS),
	// new IdMapEntry(ITextEditorActionDefinitionIds.DELETE_NEXT_WORD,
	// ST.DELETE_WORD_NEXT),
	// // miscellaneous
	// new IdMapEntry(ITextEditorActionDefinitionIds.TOGGLE_OVERWRITE,
	// ST.TOGGLE_OVERWRITE) };
	//
	// private final String fReadOnlyLabel =
	// EditorMessages.Editor_statusline_state_readonly_label;
	// private final String fWritableLabel =
	// EditorMessages.Editor_statusline_state_writable_label;
	// private final String fInsertModeLabel =
	// EditorMessages.Editor_statusline_mode_insert_label;
	// private final String fOverwriteModeLabel =
	// EditorMessages.Editor_statusline_mode_overwrite_label;
	// private final String fSmartInsertModeLabel =
	// EditorMessages.Editor_statusline_mode_smartinsert_label;
	//
	// /**
	...
	// EditorMessages.Editor_statusline_error_label;
	//
	// /**
	...
	// EditorMessages.Editor_statusline_position_pattern;
	...
	//
	// /**
	// * The editor's status line if parent is an EditorPart.
	// *
	// * @since 2.1
	// */
	...
	// private ActivationCodeTrigger fActivationCodeTrigger = new
	...
	// private IElementStateListener fElementStateListener = new
	...
	// * @since 2.1
	// */
	// private IPropertyChangeListener fFontPropertyChangeListener = new
	...
	// * @since 3.5
	// */
	// private KeyBindingSupportForAssistant
	// fKeyBindingSupportForContentAssistant;
	//
	// /**
	// * The save action.
	// *
	...
	// * <code>SourceViewerConfiguration</code> to configure its source viewer.
	// * This viewer does not have a range indicator installed, nor any menu id
	// * set. By default, the created editor runs in 1.0 context menu registration
	// * compatibility mode.
	// *
	// * protected JSSourceViewer() { super(); fEditorContextMenuId = null;
	// * fRulerContextMenuId = null; fHelpContextId = null; }
	// */
	//
	// /*
	...
	//
	// /**
	// * Sets this editor's document provider. This method must be called before
	...
	// *
	// * @param compatible
	// * <code>true</code> if compatibility mode is enabled
	// * @since 2.0
	// */
	...
	// fPreferenceStore
	// .removePropertyChangeListener(fFontPropertyChangeListener);
	// }
	//
	...
	// fPreferenceStore
	// .addPropertyChangeListener(fFontPropertyChangeListener);
	// }
	// }
	//
	// /*
	...
	// IDocumentProviderExtension extension = (IDocumentProviderExtension)
	// provider;
	// return extension.isModifiable(getViewerInput());
	// }
	// return false;
	// }
	//
	...
	// * when, e.g., the content of the editor is about to be reverted to the
	// * saved state. This method remembers the selection in a semantic format,
	// * i.e., in a format which allows to restore the selection even if the
	// * originally selected text is no longer part of the editor's content.
	// * <p>
	// * Subclasses should implement this method including all necessary state.
	// * This default implementation remembers the textual range only and is thus
	// * purely syntactic.
	// * </p>
	// *
	// * @return
	...
	// * <code>rememberSelection</code>. Subclasses may reimplement this method
	// * and thereby semantically adapt the remembered selection. This default
	...
	// if (isValidSelection(textSelection.getOffset(),
	...
	// if (provider != null) {
	//
	// IDocument document = provider.getDocument(getViewerInput());
	// if (document != null) {
	...
	// selectAndReveal(textSelection.getOffset(),
	// textSelection.getLength());
	// }
	// }
	//
	...
	// private final int fDoubleClickTime = fParentShell.getDisplay()
	// .getDoubleClickTime();
	...
	// : fDoubleClickTime
	// - (int) (System.currentTimeMillis() - fMouseUpDelta);
	// if (1 != e.button)
	// return;
	//
	...
	// *
	// * @param ruler
	// * the composite ruler to add contributions to
	// * @since 3.3
	// */
	...
	// if (fPreferenceStore != null)
	// adapter = new RulerColumnPreferenceAdapter(getPreferenceStore(),
	...
	// final RulerColumnDescriptor descriptor = (RulerColumnDescriptor) it
	// .next();
	// support.setColumnVisible(descriptor,
	// adapter == null || adapter.isEnabled(descriptor));
	// }
	// }
	//
	// /**
	// * Creates the column support to be used by this editor to manage the
	// * contributed ruler columns. Subclasses may re-implement this method using
	// * the {@link ColumnSupport}, e.g. by returning
	// * <code>new ColumnSupport(this, RulerColumnRegistry.getDefault());</code>.
	// * <p>
	// * <strong>Note:</strong> If you override this method to provide column
	// * support you will also need to override {@link #createVerticalRuler()} to
	// * return a {@link CompositeRuler}.
	// * </p>
	// * <p>
	...
	// return;
	//
	// ITextEditorDropTargetListener listener = (ITextEditorDropTargetListener)
	// getAdapter(ITextEditorDropTargetListener.class);
	//
	// if (listener == null) {
	...
	// * <ul>
	// * <li>
	// * {@linkplain #initializeViewerColors(ISourceViewer)
	...
	// | SWT.FULL_SELECTION;
	// fSourceViewer = createSourceViewer(parent, getVerticalRuler(), styles);
	//
	...
	// IQuickAssistAssistant assistant = ((ISourceViewerExtension3) fSourceViewer)
	// .getQuickAssistAssistant();
	// if (assistant != null)
	// fKeyBindingSupportForQuickAssistant = new KeyBindingSupportForAssistant(
	// assistant);
	// }
	//
	...
	//
	// /*
	// * gestures commented out until proper solution (i.e. preference page)
	// * can be found for bug # 28417:
	// *
	// * final Map gestureMap= new HashMap();
	// *
	// * gestureMap.put("E",
	// * IWorkbenchCommandConstants.NAVIGATE_FORWARDHISTORY); //$NON-NLS-1$
	// * gestureMap.put("N", IWorkbenchCommandConstants.FILE_SAVE);
	// * //$NON-NLS-1$ gestureMap.put("NW",
	// * IWorkbenchCommandConstants.FILE_SAVEALL); //$NON-NLS-1$
	// * gestureMap.put("S", IWorkbenchCommandConstants.FILE_CLOSE);
	// * //$NON-NLS-1$ gestureMap.put("SW",
	// * IWorkbenchCommandConstants.FILE_CLOSEALL); //$NON-NLS-1$
	// * gestureMap.put("W",
	// * IWorkbenchCommandConstants.NAVIGATE_BACKWARDHISTORY); //$NON-NLS-1$
	// * gestureMap.put("EN", IWorkbenchCommandConstants.EDIT_COPY);
	// * //$NON-NLS-1$ gestureMap.put("ES",
	// * IWorkbenchCommandConstants.EDIT_PASTE); //$NON-NLS-1$
	// * gestureMap.put("EW", IWorkbenchCommandConstants.EDIT_CUT);
	// * //$NON-NLS-1$
	// *
	// * Capture capture= Capture.create(); capture.setControl(styledText);
	// *
	// * capture.addCaptureListener(new CaptureListener() { public void
	// * gesture(Gesture gesture) { if (gesture.getPen() == 3) { String
	// * actionId= (String) gestureMap.get(Util.recognize(gesture.getPoints(),
	// * 20));
	// *
	// * if (actionId != null) { IKeyBindingService keyBindingService=
	// * getEditorSite().getKeyBindingService();
	// *
	// * if (keyBindingService instanceof KeyBindingService) { IAction action=
	// * ((KeyBindingService) keyBindingService).getAction(actionId);
	// *
	// * if (action != null) { if (action instanceof IUpdate) ((IUpdate)
	// * action).update();
	// *
	...
	// .setHelp(styledText, getHelpContextId());
	//
	...
	// getEditorSite().registerContextMenu(fEditorContextMenuId, manager,
	// getSelectionProvider(),
	// isEditorInputIncludedInContextMenu());
	// else if (fCompatibilityMode)
	// getEditorSite().registerContextMenu(DEFAULT_EDITOR_CONTEXT_MENU_ID,
	// manager, getSelectionProvider(),
	// isEditorInputIncludedInContextMenu());
	//
	// if ((fEditorContextMenuId != null && fCompatibilityMode)
	// || fEditorContextMenuId == null) {
	// String partId = getEditorSite().getId();
	// if (partId != null)
	// getEditorSite()
	// .registerContextMenu(
	// partId + ".EditorContext", manager, getSelectionProvider(),
	// isEditorInputIncludedInContextMenu()); //$NON-NLS-1$
	// }
	//
	// getEditorSite().registerContextMenu(COMMON_EDITOR_CONTEXT_MENU_ID,
	...
	// getEditorSite().registerContextMenu(fRulerContextMenuId, manager,
	// getSelectionProvider(), false);
	// else if (fCompatibilityMode)
	// getEditorSite().registerContextMenu(DEFAULT_RULER_CONTEXT_MENU_ID,
	// manager, getSelectionProvider(), false);
	//
	// if ((fRulerContextMenuId != null && fCompatibilityMode)
	// || fRulerContextMenuId == null) {
	// String partId = getSite().getId();
	// if (partId != null)
	// getEditorSite()
	// .registerContextMenu(
	// partId + ".RulerContext", manager, getSelectionProvider(), false);
	// //$NON-NLS-1$
	// }
	//
	// getEditorSite().registerContextMenu(COMMON_RULER_CONTEXT_MENU_ID,
	// manager, getSelectionProvider(), false);
	//
	// if (fRulerContextMenuId == null)
	...
	// fSelectionListener.setDocument(getDocumentProvider().getDocument(
	// getViewerInput()));
	//
	...
	// createActions();
	//
	// initializeSourceViewer(getViewerInput());
	//
	// /*
	// * since 3.2 - undo redo actions should be created after the source
	// * viewer is initialized, so that the undo manager can obtain its undo
	// * context from its document.
	// */
	...
	// // st.insert(text);
	// // IDocument document=
	// // getDocumentProvider().getDocument(getViewerInput());
	...
	// FontData data = PreferenceConverter.getFontData(
	// fPreferenceStore, JFaceResources.TEXT_FONT);
	//
	// if (data != null) {
	// isSharedFont = false;
	...
	// // ----------- foreground color --------------------
	// Color color = store
	// .getBoolean(PREFERENCE_COLOR_FOREGROUND_SYSTEM_DEFAULT) ? null
	...
	// // ---------- background color ----------------------
	// color = store
	// .getBoolean(PREFERENCE_COLOR_BACKGROUND_SYSTEM_DEFAULT) ? null
	...
	// color = store
	// .getBoolean(PREFERENCE_COLOR_SELECTION_FOREGROUND_SYSTEM_DEFAULT) ? null
	...
	// color = store
	// .getBoolean(PREFERENCE_COLOR_SELECTION_BACKGROUND_SYSTEM_DEFAULT) ? null
	...
	//
	// IElementStateListenerExtension extension = (IElementStateListenerExtension)
	...
	// IPreferenceStore store = getPreferenceStore();
	// EnrichMode mode = store != null ? convertEnrichModePreference(store
	// .getInt(PREFERENCE_HOVER_ENRICH_MODE))
	...
	// IDocumentProviderExtension2 extension = (IDocumentProviderExtension2)
	// provider;
	...
	// * runnable from <code>init</code>, this method does the actual setting of
	// * the editor input. Closes the editor if <code>input</code> is
	...
	// SaveablesLifecycleEvent.POST_CLOSE, getSaveables(),
	// false));
	...
	// }
	//
	// IEditorInput oldInput = getViewerInput();
	// if (oldInput != null)
	...
	// IStatus.OK, EditorMessages.Editor_error_no_provider,
	// null);
	...
	// boolean areOperationActionHandlersInstalled = undoAction instanceof
	...
	// fSelectionListener.setDocument(getDocumentProvider()
	// .getDocument(input));
	//
	// IVerticalRuler ruler = getVerticalRuler();
	// if (ruler instanceof CompositeRuler)
	...
	// listener.handleLifecycleEvent(new SaveablesLifecycleEvent(this,
	// SaveablesLifecycleEvent.POST_OPEN, getSaveables(),
	// false));
	//
	// }
	//
	...
	// if (fSourceViewer != null)
	// if (fParentEditorPart != null) {
	// fParentEditorPart.getSite().getPage()
	// .closeEditor(fParentEditorPart, save);
	// }
	// }
	// });
	// }
	...
	// * <p>
	// * If the editor is defined with a <code>symbolicFontName </code> then this
	// * name is returned and the font is looked up in the JFace resource
	// * registry. Otherwise, {@link JFaceResources#TEXT_FONT} is returned and the
	// * font is looked up in this editor's preference store.
	// * </p>
	// *
	...
	// int stateMask = getSourceViewerConfiguration()
	...
	// String[] difference = StringSetSerializer.getDifference(
	// (String) event.getOldValue(), (String) event.getNewValue());
	// IColumnSupport support = (IColumnSupport) getAdapter(IColumnSupport.class);
	// for (int i = 0; i < difference.length; i++) {
	// RulerColumnDescriptor desc = RulerColumnRegistry.getDefault()
	// .getColumnDescriptor(difference[i]);
	...
	// IAction action =
	// getAction(ITextEditorActionConstants.SHOW_WHITESPACE_CHARACTERS);
	// if (action instanceof IUpdate)
	// ((IUpdate) action).update();
	// return;
	// }
	...
	// IPreferenceStore store = getPreferenceStore();
	// if (store != null) {
	// ((ITextViewerExtension8) fSourceViewer)
	// .setHoverEnrichMode(convertEnrichModePreference(store
	// .getInt(PREFERENCE_HOVER_ENRICH_MODE)));
	// }
	// }
	// return;
	...
	// String title;
	// String msg;
	// Shell shell = null; // getSite().getShell();
	//
	// if (fParentEditorPart != null) {
	// shell = fParentEditorPart.getSite().getShell();
	// } else if (fParentControl != null) {
	// shell = fParentControl.getShell();
	// }
	//
	...
	// }
	//
	// final IEditorInput input = getViewerInput();
	...
	// msg = NLSUtility
	// .format(EditorMessages.Editor_error_activated_deleted_save_message,
	// inputName);
	//
	// String[] buttons = {
	...
	// msg, MessageDialog.QUESTION, buttons, 0);
	//
	// if (dialog.open() == 0) {
	...
	// msg = NLSUtility
	// .format(EditorMessages.Editor_error_activated_deleted_close_message,
	// inputName);
	...
	// msg = NLSUtility.format(
	// EditorMessages.Editor_error_activated_outofsync_message,
	// inputName);
	//
	// if (fHasBeenActivated
	...
	// msg = NLSUtility
	// .format(EditorMessages.Editor_error_refresh_outofsync_message,
	// inputName);
	...
	//
	// /**
	// * Performs a save as and reports the result state back to the given
	// * progress monitor. This default implementation does nothing. Subclasses
	// * may reimplement.
	// *
	...
	//
	// /**
	// * Validates the state of the given editor input. The predominate intent of
	// * this method is to take any action probably necessary to ensure that the
	// * input can persistently be changed.
	// *
	// * @param input
	...
	// /*
	// * @see
	// * org.eclipse.ui.texteditor.ITextEditorExtension2#validateEditorInputState
	// * ()
	// *
	// * @since 2.1
	// */
	...
	// && !fTextInputListener.inputChanged;
	//
	// } finally {
	...
	// try {
	//
	// provider.aboutToChange(getViewerInput());
	// IEditorInput input = getViewerInput();
	...
	// } finally {
	// provider.changed(getViewerInput());
	// }
	// }
	//
	// /**
	...
	// if (p instanceof IDocumentProviderExtension3) {
	// IDocumentProviderExtension3 p3 = (IDocumentProviderExtension3) p;
	// isSynchronized = p3.isSynchronized(getViewerInput());
	// } else {
	// long modifiedStamp = p.getModificationStamp(getViewerInput());
	// long synchStamp = p.getSynchronizationStamp(getViewerInput());
	// isSynchronized = (modifiedStamp == synchStamp);
	// }
	//
	// if (isNotSynchronizedException(exception)
	// && fErrorCorrectionOnSave == 1 && !isSynchronized) {
	// String title = EditorMessages.Editor_error_save_outofsync_title;
	// String msg = NLSUtility.format(
	// EditorMessages.Editor_error_save_outofsync_message,
	// getViewerInput().getToolTipText());
	//
	// if (MessageDialog
	// .openQuestion(getSite().getShell(), title, msg))
	// performSave(true, progressMonitor);
	// else {
	// /*
	// * 1GEUPKR: ITPJUI:ALL - Loosing work with simultaneous
	// * edits Set progress monitor to canceled in order to report
	// * back to enclosing operations.
	// */
	...
	// * Set progress monitor to canceled in order to report back to
	...
	// return ((IDocumentProviderExtension5) provider)
	// .isNotSynchronizedException(getViewerInput(), ex);
	// return false;
	// }
	//
	// /**
	...
	// IDocumentProvider p = getDocumentProvider();
	// return p == null ? false : p.canSaveDocument(getViewerInput());
	// }
	//
	// /**
	// * The <code>AbstractTextEditor</code> implementation of this
	...
	//
	// provider.aboutToChange(getViewerInput());
	// provider.resetDocument(getViewerInput());
	// editorSaved();
	//
	// } catch (CoreException x) {
	// IStatus status = x.getStatus();
	...
	// int size = fActivationCodes.size();
	// for (int i = 0; i < size; i++) {
	// ActionActivationCode code = (ActionActivationCode) fActivationCodes
	// .get(i);
	...
	// * <code>org.eclipse.swt.custom.ST</code>. Overwrites and extends the list
	// * of these actions afterwards.
	// * <p>
	// * Subclasses may extend.
	// * </p>
	// *
	...
	// }
	//
	// action = new ToggleOverwriteModeAction(
	// EditorMessages.getBundleForConstructedKeys(),
	...
	// setAction(ITextEditorActionDefinitionIds.SHOW_RULER_CONTEXT_MENU,
	// action);
	// }
	//
	// /**
	...
	// OperationHistoryActionHandler undoAction = new UndoActionHandler(
	// getEditorSite(), undoContext);
	// PlatformUI
	// .getWorkbench()
	// .getHelpSystem()
	// .setHelp(undoAction,
	...
	// OperationHistoryActionHandler redoAction = new RedoActionHandler(
	// getEditorSite(), undoContext);
	// PlatformUI
	// .getWorkbench()
	// .getHelpSystem()
	...
	// if (fLinearUndoViolationApprover != null)
	// history.removeOperationApprover(fLinearUndoViolationApprover);
	// fLinearUndoViolationApprover = new LinearUndoViolationUserApprover(
	// undoContext, this);
	...
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.Undo.", this, ITextOperationTarget.UNDO); //$NON-NLS-1$
	...
	// action = new TextOperationAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.Redo.", this, ITextOperationTarget.REDO); //$NON-NLS-1$
	...
	// * @param action
	// * the action to register or <code>null</code> to dispose them
	// * @since 3.1
	// */
	...
	// return new NonLocalUndoUserApprover(undoContext, this,
	// new Object[] { getViewerInput() }, Object.class);
	// }
	//
	// /**
	...
	// action = new TextOperationAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.Delete.", this, ITextOperationTarget.DELETE); //$NON-NLS-1$
	...
	//
	// action = new DeleteLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.DeleteLine.", this, DeleteLineAction.WHOLE, false); //$NON-NLS-1$
	...
	// action = new DeleteLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.CutLine.", this, DeleteLineAction.WHOLE, true); //$NON-NLS-1$
	...
	// action = new DeleteLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.DeleteLineToBeginning.", this, DeleteLineAction.TO_BEGINNING,
	// false); //$NON-NLS-1$
	...
	// action = new DeleteLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.CutLineToBeginning.", this, DeleteLineAction.TO_BEGINNING, true);
	// //$NON-NLS-1$
	...
	// action = new DeleteLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.DeleteLineToEnd.", this, DeleteLineAction.TO_END, false);
	// //$NON-NLS-1$
	...
	// action = new DeleteLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.CutLineToEnd.", this, DeleteLineAction.TO_END, true); //$NON-NLS-1$
	...
	//
	// action = new JoinLinesAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.JoinLines.", this, " "); //$NON-NLS-1$ //$NON-NLS-2$
	...
	//
	// action = new MarkAction(EditorMessages.getBundleForConstructedKeys(),
	// "Editor.SetMark.", this, MarkAction.SET_MARK); //$NON-NLS-1$
	...
	//
	// action = new MarkAction(EditorMessages.getBundleForConstructedKeys(),
	// "Editor.ClearMark.", this, MarkAction.CLEAR_MARK); //$NON-NLS-1$
	...
	//
	// action = new MarkAction(EditorMessages.getBundleForConstructedKeys(),
	// "Editor.SwapMark.", this, MarkAction.SWAP_MARK); //$NON-NLS-1$
	...
	// action = new TextOperationAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.SelectAll.", this, ITextOperationTarget.SELECT_ALL, true);
	// //$NON-NLS-1$
	...
	//
	// action = new ShiftAction(EditorMessages.getBundleForConstructedKeys(),
	// "Editor.ShiftRight.", this, ITextOperationTarget.SHIFT_RIGHT);
	// //$NON-NLS-1$
	...
	//
	// action = new ShiftAction(EditorMessages.getBundleForConstructedKeys(),
	// "Editor.ShiftLeft.", this, ITextOperationTarget.SHIFT_LEFT); //$NON-NLS-1$
	...
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.FindReplace.", this); //$NON-NLS-1$
	...
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.FindNext.", this, true); //$NON-NLS-1$
	...
	// action = new FindNextAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.FindPrevious.", this, false); //$NON-NLS-1$
	...
	//
	// action = new IncrementalFindAction(
	// EditorMessages.getBundleForConstructedKeys(),
	...
	//
	// action = new RevertToSavedAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.Revert.", this); //$NON-NLS-1$
	...
	//
	// action = new GotoLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.GotoLine.", this); //$NON-NLS-1$
	...
	// action = new MoveLinesAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.MoveLinesDown.", this, getSourceViewer(), false, false);
	// //$NON-NLS-1$
	// action.setHelpContextId(IAbstractTextEditorHelpContextIds.MOVE_LINES_ACTION);
	...
	// action = new MoveLinesAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.CopyLineDown.", this, getSourceViewer(), false, true);
	// //$NON-NLS-1$
	// action.setHelpContextId(IAbstractTextEditorHelpContextIds.COPY_LINES_ACTION);
	...
	//
	// action = new CaseAction(EditorMessages.getBundleForConstructedKeys(),
	// "Editor.UpperCase.", this, true); //$NON-NLS-1$
	...
	//
	// action = new CaseAction(EditorMessages.getBundleForConstructedKeys(),
	// "Editor.LowerCase.", this, false); //$NON-NLS-1$
	...
	//
	// action = new InsertLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.SmartEnter.", this, false); //$NON-NLS-1$
	...
	// action = new InsertLineAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.SmartEnterInverse.", this, true); //$NON-NLS-1$
	// action.setHelpContextId(IAbstractTextEditorHelpContextIds.SMART_ENTER_ACTION);
	...
	//
	// action = new ToggleInsertModeAction(
	// EditorMessages.getBundleForConstructedKeys(),
	...
	//
	// action = new HippieCompleteAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.HippieCompletion.", this); //$NON-NLS-1$
	...
	// action = new TextOperationAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.ContentAssistContextInformation.", this,
	...
	// action = new TextOperationAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.QuickAssist.", this, ISourceViewer.QUICK_ASSIST); //$NON-NLS-1$
	...
	// markAsStateDependentAction(ITextEditorActionConstants.QUICK_ASSIST,
	// true);
	//
	...
	//
	// action = new RecenterAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.Recenter.", this); //$NON-NLS-1$
	...
	//
	// action = new ShowWhitespaceCharactersAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.ShowWhitespaceCharacters.", this, getPreferenceStore());
	// //$NON-NLS-1$
	...
	// action = new TextOperationAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.ShowInformation.", this, ISourceViewer.INFORMATION, true);
	// //$NON-NLS-1$
	// action = new InformationDispatchAction(
	// EditorMessages.getBundleForConstructedKeys(),
	...
	//
	// action = new BlockSelectionModeToggleAction(
	// EditorMessages.getBundleForConstructedKeys(),
	...
	// action = new TextOperationAction(
	// EditorMessages.getBundleForConstructedKeys(),
	// "Editor.OpenHyperlink.", this, HyperlinkManager.OPEN_HYPERLINK, true);
	// //$NON-NLS-1$;
	...
	//
	// public ISelection getSelection() {
	// return new StructuredSelection(getViewerInput());
	// }
	//
	// public void removeSelectionChangedListener(
	// ISelectionChangedListener listener) {
	...
	// markAsContentDependentAction(ITextEditorActionConstants.FIND_PREVIOUS,
	// true);
	// markAsContentDependentAction(
	// ITextEditorActionConstants.FIND_INCREMENTAL, true);
	// markAsContentDependentAction(
	// ITextEditorActionConstants.FIND_INCREMENTAL_REVERSE, true);
	//
	...
	// markAsSelectionDependentAction(ITextEditorActionConstants.SHIFT_RIGHT,
	// true);
	// markAsSelectionDependentAction(
	// ITextEditorActionConstants.SHIFT_RIGHT_TAB, true);
	// markAsSelectionDependentAction(ITextEditorActionConstants.UPPER_CASE,
	// true);
	// markAsSelectionDependentAction(ITextEditorActionConstants.LOWER_CASE,
	// true);
	//
	...
	// ITextEditorActionConstants.REVERT_TO_SAVED, true);
	...
	// markAsStateDependentAction(ITextEditorActionConstants.SHIFT_RIGHT_TAB,
	// true);
	...
	// markAsStateDependentAction(
	// ITextEditorActionConstants.DELETE_LINE_TO_BEGINNING, true);
	// markAsStateDependentAction(
	// ITextEditorActionConstants.DELETE_LINE_TO_END, true);
	// markAsStateDependentAction(ITextEditorActionConstants.MOVE_LINE_UP,
	// true);
	// markAsStateDependentAction(ITextEditorActionConstants.MOVE_LINE_DOWN,
	// true);
	// markAsStateDependentAction(ITextEditorActionConstants.CUT_LINE, true);
	// markAsStateDependentAction(
	// ITextEditorActionConstants.CUT_LINE_TO_BEGINNING, true);
	// markAsStateDependentAction(ITextEditorActionConstants.CUT_LINE_TO_END,
	// true);
	//
	// setActionActivationCode(ITextEditorActionConstants.SHIFT_RIGHT_TAB,
	// '\t', -1, SWT.NONE);
	// setActionActivationCode(ITextEditorActionConstants.SHIFT_LEFT, '\t',
	// -1, SWT.SHIFT);
	// }
	//
	// /**
	...
	// *
	// * @param menu
	// * the menu to add the new group to
	// * @param existingGroup
	// * the group after which to insert the new group
	// * @param newGroup
	// * the new group
	// */
	// protected final void addGroup(IMenuManager menu, String existingGroup,
	// String newGroup) {
	...
	// fEditorStatusLine = new EditorStatusLine(statusLineManager,
	// selectionProvider);
	// }
	...
	// fDeleteLineTarget = new TextViewerDeleteLineTarget(
	// fSourceViewer);
	// }
	...
	// IFindReplaceTarget target = (fSourceViewer == null ? null
	...
	// return (fSourceViewer == null ? null : fSourceViewer
	...
	// || offset != rangeIndication.getOffset()
	...
	// /*
	// * @see
	// * org.eclipse.ui.INavigationLocationProvider#createNavigationLocation()
	// *
	// * @since 2.1
	// */
	...
	//
	// /**
	// * Hook which gets called when the editor has been saved. Subclasses may
	// * extend.
	// *
	// * @since 2.1
	// */
	// protected void editorSaved() {
	// INavigationLocation[] locations = getSite().getPage()
	// .getNavigationHistory().getLocations();
	// IEditorInput input = getViewerInput();
	// for (int i = 0; i < locations.length; i++) {
	// if (locations[i] instanceof TextSelectionNavigationLocation) {
	// if (input.equals(locations[i].getInput())) {
	// TextSelectionNavigationLocation location =
	// (TextSelectionNavigationLocation) locations[i];
	...
	// *
	// * protected void firePropertyChange(int property) {
	// * super.firePropertyChange(property); updatePropertyDependentActions(); }
	// *
	// * /*
	// *
	...
	//
	// if (fIncrementalFindTarget != null
	// && ITextEditorActionConstants.STATUS_CATEGORY_FIND_FIELD
	// .equals(category))
	...
	// *
	// * @return <code>true</code> if in insert mode, <code>false</code> for
	// * overwrite mode
	// * @since 2.0
	// */
	...
	// /*
	// * @see
	// * org.eclipse.ui.texteditor.ITextEditorExtension3#setInsertMode(org.eclipse
	// * .ui.texteditor.ITextEditorExtension3.InsertMode)
	// *
	// * @since 3.0
	// */
	...
	//
	// /**
	// * Configures the given insert mode as legal or illegal. This call is
	// * ignored if the set of legal input modes would be empty after the call.
	// *
	// * @param mode
	// * the insert mode to be configured
	// * @param legal
	// * <code>true</code> if the given mode is legal,
	// * <code>false</code> otherwise
	// * @since 3.0
	// */
	...
	// *
	// * @param enable
	// * <code>true</code> to enable new overwrite mode,
	...
	// styledText.getLineHeight(), 1, caretPalette);
	//
	// Display display = styledText.getDisplay();
	// Image bracketImage = new Image(display, imageData);
	// GC gc = new GC(bracketImage);
	// gc.setForeground(display.getSystemColor(SWT.COLOR_WHITE));
	// gc.setLineWidth(0); // NOTE: 0 means width is 1 but with optimized
	// // performance
	...
	// && getPreferenceStore().getBoolean(PREFERENCE_WIDE_CARET))
	...
	// String text = null;
	//
	// if (ITextEditorActionConstants.STATUS_CATEGORY_INPUT_POSITION
	// .equals(category))
	// text = getCursorPosition();
	// else if (ITextEditorActionConstants.STATUS_CATEGORY_ELEMENT_STATE
	// .equals(category))
	// text = isEditorInputReadOnly() ? fReadOnlyLabel
	// : fWritableLabel;
	// else if (ITextEditorActionConstants.STATUS_CATEGORY_INPUT_MODE
	// .equals(category)) {
	// InsertMode mode = getInsertMode();
	// if (fIsOverwriting)
	...
	// return NLSUtility.format(fPositionLabelPattern,
	...
	// IDocumentProviderExtension extension = (IDocumentProviderExtension)
	// provider;
	// return extension.isReadOnly(getViewerInput());
	// }
	// return true;
	// }
	//
	...
	// /*
	// * @see
	// * org.eclipse.ui.texteditor.ITextEditorExtension3#showChangeInformation
	// * (boolean)
	// *
	// * @since 3.0
	// */
	...
	// * @see
	// *
	// org.eclipse.ui.texteditor.ITextEditorExtension3#isChangeInformationShowing
	// * ()
	// *
	// * @since 3.0
	// */
	...
	// Annotation annotation = findAnnotation(selection.getOffset(),
	...
	// /*
	// * @see
	// * org.eclipse.ui.texteditor.ITextEditorExtension4#showRevisionInformation
	// * (org.eclipse.jface.text.revisions.RevisionInformation, java.lang.String)
	// *
	// * @since 3.2
	// */
	...
	// /*
	// * @see
	// * org.eclipse.ui.IEditorPersistable#restoreState(org.eclipse.ui.IMemento)
	// *
	// * @since 3.3
	// */
	...
	// memento.putInteger(TAG_SELECTION_LENGTH,
	...
	// memento.putInteger(TAG_SELECTION_HORIZONTAL_PIXEL,
	...
	// *
	// * @param memento
	// * the saved state of this editor
	// * @return <code>true</code> if the given memento contains saved state
	// * @since 3.3
	// */
	// protected boolean containsSavedState(IMemento memento) {
	// return memento.getInteger(TAG_SELECTION_OFFSET) != null
	// && memento.getInteger(TAG_SELECTION_LENGTH) != null;
	// }
	//
	// /**
	...
	//
	// Integer horizontalPixel = memento
	// .getInteger(TAG_SELECTION_HORIZONTAL_PIXEL);
	...
	//
	// /** The cached editor. */
	// private JSSourceViewer fsourceEditor;
	...
	// * the text editor
	// */
	// public TextEditorSavable(JSSourceViewer sourceEditor) {
	// Assert.isLegal(sourceEditor != null);
	// fsourceEditor = sourceEditor;
	// // fEditorInput = fTextEditor.getViewerInput();
	...
	// if (fDocument == null) {
	// IDocumentProvider documentProvider = fsourceEditor
	// .getDocumentProvider();
	// if (documentProvider != null)
	// fDocument = documentProvider.getDocument(fsourceEditor);
	// }
	// return fDocument;
	// }
	...
	// ((ITextViewerExtension7) fSourceViewer)
	...
	//
	// /**
	// * Tells whether tabs should be converted to spaces while editing inside
	// * this editor.
	// *
	// * <p>
	...
	//
	// /**
	// * Updates the source viewer's indent prefixes with the values provided by
	...
	// String[] prefixes = configuration.getIndentPrefixes(fSourceViewer,
	// types[i]);
	...
	// * <p>
	// * By default block selection mode is supported. Subclasses may override
	// * this method to disable it.
	// * </p>
	// *
	...
	// /**
	// * @see
	// org.eclipse.ui.texteditor.ITextEditorExtension5#isBlockSelectionModeEnabled()
	// * @since 3.5
	// */
	...
	// /**
	// * @see
	// org.eclipse.ui.texteditor.ITextEditorExtension5#setBlockSelectionMode(boolean)
	// * @since 3.5
	// */
	...
	// * - set the font _before enabling_ block selection mode in
	// * order to maintain the selection - revert the font _after
	// * disabling_ block selection mode in order to maintain the
	// * selection
	// */
	// if (enable) {
	// Font blockFont = JFaceResources
	...
	// && !normalFont.getFontData()[0].equals(blockFont
	// .getFontData()[0])) {
	...
	// }
	//
	// // TODO fill following stubs with real data
	// public IEditorInput getViewerInput() {
	// return null;
	// }
	//
	// /*

...
public class DesignerAboutDialog extends TrayDialog {
	 ...
	 */
	public DesignerAboutDialog(Shell parentShell) {
		super(parentShell);
		...
		String desc = product.getProperty("aboutText");
		desc += "\n discoveryurl: " + "blabla";
		...
		case DETAILS_ID:
			BusyIndicator.showWhile(getShell().getDisplay(), new Runnable() {
				public void run() {
					...
					dialog.setModalParent(DesignerAboutDialog.this);
					dialog.open();
				}
			});
			break;
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (product != null) {
			...
			if (aboutImage == null || aboutImage.getBounds().width <= MAX_IMAGE_WIDTH_FOR_TEXT) {
				...
				if (aboutText != null) {
					Endpoint[] discoveries = ServerInstance.getInstance().getServerInstance().getEndpoints();
					String more = "";
					if (discoveries.length > 1) {
						more = "s";
					}
					aboutText += "\n\ndiscoveryurl" + more + ":    ";
					for (Endpoint disco : discoveries) {
						aboutText += "\n           " + disco;
					}
	...
	private Button createFeatureButton(Composite parent, final AboutBundleGroupData info) {
		...
		button.getAccessible().addAccessibleListener(new AccessibleAdapter() {
			 ...
			 * (non-Javadoc)
			 * 
			 * @see org.eclipse.swt.accessibility.AccessibleAdapter#getName(org.eclipse

...
public class ChangeModellingRuleHandler extends AbstractHandler {
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		ExpandedNodeId enid = selectedNode.findTarget(Identifiers.HasTypeDefinition, false);
		// new NodeId(enid.getNamespaceIndex(), enid.toString()getValue());
		NodeId result = NodeId.parseNodeId(enid.toString());
		modellingDialog.setParentNodeId(result);
		/**
		 * Changed by Hannes Bichler
		 * 
		 * Open Mandatory/Optional Dialog only when required (references exist)
		 * 
		 */
		boolean hasRules = modellingDialog.hasTreeViewerChildren(result,
						...
						try {
							...
							Display.getDefault().syncExec(new Runnable() {
								...
								public void run() {
									...
									// refresh View
									DesignerUtils.askModelChange((BrowserModelNode) selection.getFirstElement(), info);
								}
							});
						} finally {
							monitor.done();
	...
	private void create(BrowserModelNode treeSelection, ModelChangeInfo result, Node selection) {
		...
		if (selection instanceof UAObjectTypeNode || selection instanceof UAVariableTypeNode)
			...
			CreateFactory.create(NodeClass.Object, parentNodeId, parentNodeClass, referenceTypeId, result, false,
					false);
		// CreateFactory.create(NodeClass.Object, parentNodeId, parentNodeClass,
		// referenceTypeId, result, false, );

...
import com.bichler.astudio.opcua.opcmodeler.views.modeldesignbrowser.CreateObjectModellingContentProvider;
import com.bichler.astudio.utils.internationalization.CustomString;

import opc.sdk.core.node.NamingRuleType;
import opc.sdk.core.node.Node;

public class CreateObjectModellingDialog extends AbstractCreateModellingDialog implements ITreeViewerChildren {
	...
	private void addNodes(List<Node> nodes, TreeItem[] items) {
		...
		for (TreeItem item : items) {
			...
			if (item.getImage().equals(imgChecked) || item.getImage().equals(img1Checked)) {
				...
				nodes.add(node);
				this.mapping.put(node, true);
				addNodes(nodes, item.getItems());
			} else {
				this.mapping.put(node, false);
			}
		}
	}
		...
		for (ReferenceNode ref : node.getReferences()) {
			if (ref.getReferenceTypeId().equals(Identifiers.HasModellingRule)) {
				if (ref.getTargetId().equals(Identifiers.ModellingRule_Mandatory)
						...
						|| ref.getTargetId().equals(Identifiers.ModellingRule_ExposesItsArray)
						|| ref.getTargetId().equals(Identifiers.ModellingRule_OptionalPlaceholder)) {
					continue;
				}
			}
			refs.add(ref);
	...
	public boolean hasTreeViewerChildren(NodeId result, UnsignedInteger filter) {
		List<NodeId> inheritated = DesignerUtils.getInheritance(Identifiers.BaseVariableType, result);
	 ...
	 * @param parentShell
	 */
	public CreateObjectModellingDialog(Shell parentShell) {
		super(parentShell);
		...
		filtersModellingRule = new NodeClass[] { NodeClass.ObjectType, NodeClass.DataType, NodeClass.VariableType };
	...
	private void createHasRulesDialogSection(Composite container) {
		...
		inherit = DesignerUtils.getInheritance(Identifiers.BaseObjectType, this.parentNodeId);
		treeViewer.setContentProvider(new CreateObjectModellingContentProvider(this.inherit));
		treeViewer.setInput(inherit);
		// close if no modeling rules

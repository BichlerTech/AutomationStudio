 ...
 * </p>
 */
public class CometProgressStartupMonitorDialog extends IconAndMessageDialog implements IRunnableContext {
	/**
	 ...
	 * 
	 * @param parent the parent shell, or <code>null</code> to create a top-level
	 *               shell
	 */
	public CometProgressStartupMonitorDialog(Shell parent) {
		super(parent);
		// no close button on the shell style
		if (isResizable()) {
			setShellStyle(
					getDefaultOrientation() | SWT.BORDER | SWT.TITLE | SWT.APPLICATION_MODAL | SWT.RESIZE | SWT.MAX);
		} else {
	 ...
	 * dialog is opened before the runnable is run, and closed after it completes.
	 * It is recommended that <code>fork</code> is set to true in most cases. If
	 * <code>fork</code> is set to <code>false</code>, the runnable will run in the
	 * UI thread and it is the runnable's responsibility to call
	 ...
	 * 
	 * @return <code>true</code> to open the dialog before run, <code>false</code>
	...
	/**
	 * Sets whether the dialog should be opened before the operation is run. NOTE:
	 * Setting this to false and not forking a process may starve any asyncExec that
	 * tries to open the dialog later.
	 * 
	...
	/**
	 * Called just after the operation is run. Default behaviour is to decrement the
	 * nesting depth, and close the dialog.
	 * 
	 * @since 3.0
	 */
	 ...
	 * 
	 * @param cancelable <code>true</code> if the end user can cancel this progress
	 ...
	 * 
	 * @param b <code>true</code> to enable the cancel button, and
	 ...
	 * @param force         If force is true then always set the message text.
	 */
	private void setMessage(String messageString, boolean force) {
		...
		if (messageString == null) {
			...
			String[] discoveryUrls = ServerInstance.getInstance().getServerInstance().getApplication()
					.getApplicationDescription().getDiscoveryUrls();
			String more = "";
			if (discoveryUrls.length > 1) {
				more = "s";
			}
			String discovery = "\n\n\n\nRunning comet ua server in demo mode for 5 hours.\n\ncreate discovery url"
					+ more + ":\n";
			for (String disco : discoveryUrls) {
				discovery += "\n                         " + disco;
			}
			message = messageString + discovery;
		}

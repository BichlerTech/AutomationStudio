 ...
 * 
 * <p>
 * Handles several embedded {@link MessageRouterProtocol} read or write
 * requests.
 * 
 * @author Kay Kasemir
 */
	...
	public void decode(final ByteBuffer buf, final int available) throws Exception {
		...
		for (int i = 0; i < count; ++i) { // Track buffer offset from start
			...
			if (i < count - 1)
				...
				section_length = available - offset[i]; // .. from distance to
														// end for last section

			services[i].decode(buf, section_length);
		}
	}
}

...
/**
 * Unsigned Long represents an integer number value between 0 .. and
 * 0xFFFFFFFFFFFFFFFF.
 * <p>
 * There is a static instance for values between 0..1023 which can be accessed
 * using static methods {@link UnsignedLong#valueOf(long)} or
 * {@link UnsignedLong#getFromBits(long)}
 * <p>
 * This class is immutable - once it has been constructed its value cannot be
 * changed.
 * <p>
 * To use int as backend use {@link UnsignedLong#toLongBits()} and
 * {@link UnsignedLong#getFromBits(long)}.
 */
public final class UnsignedLong extends Number implements Comparable<Number> {
	...
	/**
	 * Parses {@link UnsignedLong} from String. Behaves same as
	 * {@link #parseUnsignedLong(String)}.
	 */
	...
	/**
	 * Parses {@link UnsignedLong} from String. Behaves same as
	 * {@link #parseUnsignedLong(String, int)}.
	 */
	...
	public UnsignedLong(BigInteger value) throws IllegalArgumentException {
		if (value.compareTo(BI_MIN_VALUE) < 0)
			throw new IllegalArgumentException("Value underflow");
		if (value.compareTo(BI_MAX_VALUE) > 0)
	...
	public UnsignedLong(String value) {
		...
		if (bi.compareTo(BI_MIN_VALUE) < 0)
			throw new IllegalArgumentException("Value underflow");
		if (bi.compareTo(BI_MAX_VALUE) > 0)
			throw new IllegalArgumentException("Value overflow");
	...
	public boolean equals(Object obj) {
		...
		if (obj == null)
			return false;
		if (!(obj.getClass().equals(UnsignedLong.class)))
			return false;
	 ...
	 * @throws java.lang.IllegalArgumentException if the parsed value does not fit
	 *                                            in the range of UnsignedInteger
	 */
	public static UnsignedLong parseUnsignedLong(String s, int radix)
			throws NumberFormatException, IllegalArgumentException {
		return new UnsignedLong(new BigInteger(s, radix));
	}

	/**
	 * Increase the value by one. Note that this object is not changed, but a new
	 * one is created.
	 *
	...
	/**
	 * Decrease the value by one. Note that this object is not changed, but a new
	 * one is created.
	 *
	...
	/**
	 * Subtract a value from this value. Note that this object is not changed, but a
	 * new one is created.
	 *
	 ...
	 * @throws java.lang.IllegalArgumentException if the decrement is bigger than
	 *                                            the current value
	 */

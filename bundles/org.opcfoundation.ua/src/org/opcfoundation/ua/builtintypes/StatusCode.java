 ...
 * or an operation. StatusCode is immutable.
 *
...
public final class StatusCode {
	...
	/**
	 * The code is a numeric value assigned to represent different conditions. Each
	 * code has a symbolic name and a numeric value. All descriptions in the OPC UA
	 * specification refer to the symbolic name. Part 6 maps the symbolic names onto
	 * a numeric value.
	 */
	 ...
	 * the last Notification. Clients should not process the data value unless they
	 * re-read the metadata. Servers shall set this bit if the DataTypeEncoding used
	 * for a Variable changes. Clause 7.23 describes how the DataTypeEncoding is
	 * specified for a Variable.
	 * 
	 * The bit is also set if the data type Attribute of the Variable changes. A
	 * Variable with data type BaseDataType does not require the bit to be set when
	 ...
	 * value has changed.
	 * 
	 * This bit has meaning only for StatusCodes returned as part of a data change
	 * Notification or the HistoryRead. StatusCodes used in other contexts shall
	 ...
	 * Clients should not process the data value until they re-read the metadata
	 * associated with the Variable.
	 *
	 * Servers should set this bit if the metadata has changed in way that could
	 * cause application errors if the Client does not re-read the metadata. For
	 * example, a change to the engineering units could create problems if the
	 * Client uses the value to perform calculations.
	 * 
	 * Part 8 defines the conditions where a Server shall set this bit for a DA
	...
	/**
	 * Additional information bits that qualify the StatusCode. The structure of
	 * these bits depends on the Info Type field.
	 */
	...
	/**
	 * If these bits are set, not every detected change has been returned since the
	 * Server's queue buffer for the MonitoredItem reached its limit and had to
	 * purge out data.
	 * <p>
	...
	/**
	 * If this bit is set, not every detected change has been returned since the
	 * Server's queue buffer for the MonitoredItem reached its limit and had to
	 * purge out data.
	 * <p>
	...
	/**
	 * These bits are set only when reading historical data. They indicate where the
	 * data value came from and provide information that affects how the Client uses
	 * the data value.
	 */
	 ...
	 * </p>
	 *
	 * @param value a {@link org.opcfoundation.ua.builtintypes.UnsignedInteger}
	 *              object.
	 */
	...
	public String toString() {
		return String.format("%s (0x%08X) \"%s\"", getName(), value, getDescription());
	}
	...
	/**
	 * Matches argument against subcode and severity. This method can be used to
	 * compare statuscode to values in {@link StatusCodes}.
	 *
	 * @param statusCode a {@link org.opcfoundation.ua.builtintypes.UnsignedInteger}
	 *                   object.
	...
	/**
	 * Check if the status codes equal to severity and subcode, ignoring the lowest
	 * bits of the code.
	 *
	 * @param statusCode the StatusCode to compare this one to
	 * @return true if the codes are equal when masked with SEVERITY_MASK and
	 *         SUBCODE_MASK
	 */

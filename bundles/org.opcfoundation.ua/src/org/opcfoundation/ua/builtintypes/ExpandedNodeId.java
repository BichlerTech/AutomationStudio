...
/**
 * A NodeId that allows the NamespaceUri to be specified explicitly instead of
 * NamespaceIndex. ExpandedNodeId may still use NamespaceIndex.
 * <p>
 * Instances of ExpandedNodeId are equals comparable only within server context.
 * <p>
 * ExpandedNodeIds are equals comparable with NodeIds if they are constructed
 * with NamespaceIndex and no ServerIndex.
 *
...
public final class ExpandedNodeId implements Comparable<ExpandedNodeId> {
	 ...
	 *
	 * @param nodeId a {@link org.opcfoundation.ua.builtintypes.ExpandedNodeId}
	 *               object.
	 ...
	 */
	public static ExpandedNodeId parseExpandedNodeId(String s, NamespaceTable nsTable) {
		...
		for (int i = 0; i < parts.length - 1; i++) {
			...
			if (subParts[0].equalsIgnoreCase("svr"))
				...
				returnable = new ExpandedNodeId(UnsignedInteger.valueOf(svrIndex), ns, nodeIdValue.getValue(), nsTable);
			} else
	...
	/**
	 * Convenience constructor that creates ExpandedNodeId from NamespaceIndex and
	 * Identifier of an nodeId. Server Index is null.
	 *
	 * @param nodeId nodeId
	 */
	public ExpandedNodeId(NodeId nodeId) {
		this(UnsignedInteger.ZERO, nodeId.getNamespaceIndex(), nodeId.getValue());
	}

	/**
	 * Convenience constructor that creates ExpandedNodeId from namespaceUri and
	 * value. Server index is 0
	 *
	 * @param namespaceUri a {@link java.lang.String} object.
	 * @param value        a {@link java.lang.Object} object.
	 */
	public ExpandedNodeId(String namespaceUri, Object value, NamespaceTable nsTable) {
		this(UnsignedInteger.ZERO, namespaceUri, value, nsTable);
	}
	 ...
	 * @param value          value (must be UnsignedInteger, String, UUID,
	 *                       ByteString or null)
	 */
	public ExpandedNodeId(UnsignedInteger serverIndex, int namespaceIndex, Object value) {
		...
		if (value == null)
			type = IdType.String; // changed from opaque
			...
			throw new IllegalArgumentException("value cannot be " + value.getClass().getName());
	}

	/**
	 * Convenience constructor that creates ExpandedNodeId from NamespaceIndex and
	 * Identifier of an nodeId.
	 *
	 * @param serverIndex Server Index (optional)
	 * @param nodeId      nodeId
	 */
	 ...
	 * @param serverIndex  Server Index (optional)
	 * @param namespaceUri a {@link java.lang.String} object.
	 * @param value        value (must be UnsignedInteger, String, UUID or
	 *                     ByteString)
	 */
	public ExpandedNodeId(UnsignedInteger serverIndex, String namespaceUri, Object value, NamespaceTable nsTable) {
...
//		this.namespaceIndex = 0;
		// hbhb 20201102 we also set namespace index, we need it for serialization
		if (nsTable == null || nsTable.size() == 0)
			this.namespaceIndex = 0;
		else
			this.namespaceIndex = nsTable.getIndex(namespaceUri);
	...
	public int compareTo(ExpandedNodeId other) {
		...
		if (value == 0)
			switch (type) {
			case Numeric:
				value = ((UnsignedInteger) this.value).compareTo((UnsignedInteger) other.value);
				break;
			case String:
	...
	public boolean equals(Object obj) {
		if (obj == null)
			return this.equals(ExpandedNodeId.NULL);
		if (obj instanceof NodeId) {
			if ((namespaceUri != null && namespaceUri != NamespaceTable.OPCUA_NAMESPACE) || !isLocal())
				return false;
			NodeId other = (NodeId) obj;
			if (other.namespaceIndex != namespaceIndex || other.type != type)
				return false;
			if (this.value == other.value)
				return true;
			return other.value.equals(value);
		} else if (obj instanceof ExpandedNodeId) {
			ExpandedNodeId other = (ExpandedNodeId) obj;
			if (namespaceUri != null) {
				if (other.namespaceUri == null || !other.namespaceUri.equals(namespaceUri))
					return false;
			} else {
			...
			if (!isLocal()) {
				if (other.isLocal() || !other.serverIndex.equals(serverIndex))
					return false;
			} else {
				if (!other.isLocal())
					return false;
	...
	public int hashCode() {
		...
		// Does not calc using nsIdx/nsUri (one or both can be defined and cannot do
		// idx->uri mapping here)
		if (serverIndex != null)
			hashCode += serverIndex.hashCode() * 17;
		return hashCode;
	}

	/*
	 * returns true if the nodeId is absolute, i.e. it refers to an external server
	 * (with namespaceUri or serverIndex).
	 *
	 */
	/**
	 * <p>
	...
	/**
	 * Check if the nodeId refers to a local node, i.e. a node that is in the
	...
	public boolean isNullNodeId() {
		/*
		 * This is not defined in the specification (TODO change this if added at some
		 * point), so making some assumptions here.
		 *
		 * Assuming ExpandedNodeId is null if it were null when converted to a NodeId.
		 * For this comparison, if the namespaceURI is defined and is the standard
		 * namespace, this ExpandedNodeId is considered to have namespaceIndex of 0
		 * instead, otherwise it is not null. Also assuming server index does not matter
		 * when making null check, because it would not make sense to create a reference
		 * to another server in which it would point to a null node on the other server.
		 *
		 */

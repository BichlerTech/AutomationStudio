...
public final class XmlElement {
	...
	static boolean areNodesEqual(Node n1, Node n2) {
		if (n2 == n1)
			return true;
		if (!ObjectUtils.objectEquals(n1.getNodeType(), n2.getNodeType()))
			return false;
		if (!ObjectUtils.objectEquals(n1.getNodeName(), n2.getNodeName()))
			return false;
		if (!ObjectUtils.objectEquals(n1.getLocalName(), n2.getLocalName()))
			return false;
		if (!ObjectUtils.objectEquals(n1.getNamespaceURI(), n2.getNamespaceURI()))
			return false;
		...
		if ((v1 == null && v2 != null) || (v1 != null && v2 == null))
			return false;
		if (v1 != null && !v1.trim().equals(v2.trim()))
			return false;

		NodeList nl1 = n1.getChildNodes();
		NodeList nl2 = n2.getChildNodes();
		if ((nl1 == null && nl2 != null) || (nl1 != null && nl2 == null))
			return false;
		if (nl1 != null) {

			int len = nl1.getLength();
			if (nl2.getLength() != nl1.getLength())
				return false;
		...
		if (node == null) {
			if (encoded != null) {
				try {
					...
					reader.read(cbuf, 0, reader.getEncoding().equals("UTF8") ? 1 : 2);

...
/**
 * OPC UA DateTime. The OPC UA dateTime is defined as follows (OPCUA Part 3)
 *
 * 7.29 UtcTime This primitive DataType is used to define Coordinated Universal
 * Time (UTC) values. All time values conveyed between servers and clients in
 * OPC UA are UTC values. Clients must provide any conversions between UTC and
 * local time. This DataType is represented as a 64-bit signed integer which
 * represents the number of 100 nanosecond intervals since January 1, 1601. [UA
 * Part 6] defines details about this DataType.
 *
...
//    private static final TimeZone UTC = TimeZone.getTimeZone("UTC");
	...
	/**
	 * Constructs a new DateTime value, initializing it with the given time value
	 * (in the defined timezone).
	 * <p>
	 * Creates a {@link GregorianCalendar} using the parameters year..second; sets
	 * the TimeZone and adds nanoSeconds.
	 * <p>
	 * <b>Note</b>: that month is 0-based, and the values correspond to the
	 * constants, Calendar.JANUARY, etc.
	 *
	 * @param year       a int.
	 * @param month      a int.
	 * @param day        a int.
	 ...
	 * (in UTC timezone).
	 * <p>
	 * Creates a {@link GregorianCalendar} using the parameters year..second and
	 * adds nanoSeconds.
	 * <p>
	 * <b>Note</b>: that month is 0-based, and the values correspond to the
	 * constants, Calendar.JANUARY, etc.
	...
	/**
	 * Get 1/10 microseconds (1 unit = 100 nanoseconds) between the time and
	 * midnight, January 1, 1601 UTC.
	 *
	 * @return time value
	 */
	...
	/**
	 * Get standard Java milliseconds between the time and midnight, January 1, 1970
	 * UTC. This equals to the value returned by the respective Calendar instance,
	 ...
	 * @return a new GregorianCalendar instance, initialized to the value of the
	 *         DateTime.
	 */
	...
	public static DateTime parseDateTime(String s) throws ParseException {
		// Cannot use SimpleDateFormat.parse, because it always gives a time in local
		// timezone
		// or at least the Calendar interprets it so
		final int l = s.length();
		if (l < 10)
			throw new ParseException("Cannot parse DateTime from " + s
					+ " expecting format 'yyyy-MM-dd'T'hh:mm:ssZ', for example '2011-04-13T11:47:12Z' for UTC timezone",
					0);
	...
	/**
	 * Create a new DateTime instance from standard Java milliseconds since the
	 * beginning of epoch, January 1, 1970 UTC.
	 *
	 * @param value a long.
	...
	/**
	 * If this is true, then DateTime.toString should print local timestamps and Utc
	 * if this is false. Default is false.
	 *
	 * @return a boolean.
	 */

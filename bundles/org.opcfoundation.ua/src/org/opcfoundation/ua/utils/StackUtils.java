...
public class StackUtils {
	...
	/**
	 * Naming pattern for the {@link #getNonBlockingWorkExecutor()} thread names.
	 * Can include a single %d for the number of the thread.
	 */
	public static String NON_BLOCKING_WORK_EXECUTOR_NAME_PATTERN = "OPC-UA-Stack-Non-Blocking-Work-Executor-%d";

	/**
	 * Naming pattern for the {@link #getBlockingWorkExecutor()} thread names. Can
	 * include a single %d for the number of the thread.
	 */
	...
	private static volatile UncaughtExceptionHandler uncaughtExceptionHandler = new UncaughtExceptionHandler() {
		...
		public void uncaughtException(Thread t, Throwable e) {
			...
			java.util.logging.Logger.getLogger(StackUtils.class.getName()).log(Level.SEVERE, e.getMessage(), e);
		}
	};
	 ...
	 */
	public static boolean barrierWait(AsyncResult<?>[] requests, long timeout) throws InterruptedException {
	...
	public static synchronized Executor createBlockingWorkExecutor(String name, int maxThreadPoolSize) {
		if (BLOCKING_EXECUTOR == null) {
			...
			ThreadFactory tf = new ThreadFactory() {
				...
				public Thread newThread(Runnable r) {
					Thread t = new Thread(r,
							String.format(BLOCKING_WORK_EXECUTOR_NAME_PATTERN, counter.incrementAndGet()));
					t.setDaemon(true);
					t.setUncaughtExceptionHandler(uncaughtExceptionHandler);
					return t;
				}
			...
			BLOCKING_EXECUTOR = new ThreadPoolExecutor(blockingWorkerThreadPoolCoreSize, maxThreadPoolSize,
					blockingWorkerThreadPoolTimeout, TimeUnit.SECONDS, queue, tf);

			((ThreadPoolExecutor) BLOCKING_EXECUTOR).setRejectedExecutionHandler(new RejectedExecutionHandler() {
				@Override
				public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
					/*
					 * Technically this should happen only in shutdown because we are using
					 * Integer.MAX_VALUE bound queue. It should be enough, so this is more of a
					 * fail-safe.
					 */
	...
	/**
	 * <p>
	 * Getter for the field <code>blockingWorkerThreadPoolCoreSize</code>.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Getter for the field <code>blockingWorkerThreadPoolTimeout</code>.
	 * </p>
	 *
	...
	public synchronized static IEncodeableSerializer getDefaultSerializer() {
		if (DEFAULT_SERIALIZER == null) {
			...
			reflectionTable
					.addStructureInfo(EncodeableDesc.readFromClass(ErrorMessage.class, ErrorMessage.getFields()));
			reflectionTable
					.addStructureInfo(EncodeableDesc.readFromClass(ReverseHello.class, ReverseHello.getFields()));
	...
	/**
	 * Get Executor for non-blocking operations. This executor has one thread for
	 * one core in the system.
	 *
	...
	public static synchronized Executor getNonBlockingWorkExecutor() {
		if (NON_BLOCKING_EXECUTOR == null) {
			...
			ThreadFactory tf = new ThreadFactory() {
				...
				public Thread newThread(Runnable r) {
					Thread t = new Thread(r,
							String.format(NON_BLOCKING_WORK_EXECUTOR_NAME_PATTERN, counter.incrementAndGet()));
					t.setDaemon(true);
					t.setUncaughtExceptionHandler(uncaughtExceptionHandler);
					return t;
				}
			...
			NON_BLOCKING_EXECUTOR = // new ScheduledThreadPoolExecutor( CORES );
					new ThreadPoolExecutor(CORES, CORES, 3L, TimeUnit.SECONDS, workQueue, tf, rejectHandler);
		}
	 ...
	 * </p>
	 *
	 * @return a {@link org.opcfoundation.ua.utils.asyncsocket.AsyncSelector}
	 *         object.
	 */
	...
	/**
	 * The handler that is called, if any of the worker threads encounter an
	...
	/**
	 * <p>
	 * Setter for the field <code>blockingWorkerThreadPoolCoreSize</code>.
	 * </p>
	 *
	...
	/**
	 * Define the timeout (in seconds) of the thread pool for
	 * BlockingWorkerExecutor.
	 * <p>
	 * Default: 3
	 *
	...
	/**
	 * Define the handler that is called, if any of the worker threads encounter an
	 * exception that is not handled.
	 * <p>
	 ...
	 * <p>
	 * The handler is set when new worker threads are started and setting the value
	 * will not affect already running threads.
	 *
	...
	/**
	 * Perform a "context shutdown" to clean up the Stack resources. Necessary for
	 * web service modules, etc.
	 * <p>
	 * Shuts down {@link #BLOCKING_EXECUTOR} and {@link #NON_BLOCKING_EXECUTOR},
	 * closes {@link #SELECTOR} and cancels {@link TimerUtil#getTimer()}.
	 */
	...
	private static void logExecutor(String name, final ThreadPoolExecutor e) {
		logger.debug("{}: ActiveCount={} CompletedTaskCount={} PoolSize={} LargestPoolSize={} TaskCount={}", name,
				e.getActiveCount(), e.getCompletedTaskCount(), e.getPoolSize(), e.getLargestPoolSize(),
				e.getTaskCount());
	}

...
/**
 * Input stream with a sequence of ByteBuffers as backend. ByteBuffers can be
 * submitted in random order. Input stream sleeps until data becomes available.
 * Sequence number determines the order of how the data becomes visible to the
 * input stream.
 *
...
public class OrderedByteBufferInputStream extends InputStream {
	...
	public synchronized void offer(int sequenceNumber, ByteBuffer buf) {
		...
		if (close > 0 && sequenceNumber >= close)
			throw new RuntimeException(
					"Cannot put data at " + sequenceNumber + " because the stream was closed at " + close);
	...
	public synchronized void close(int sequenceNumber) {
		if (close >= 0)
			return; // throw new RuntimeException("Already closed");
	 ...
	 */
	/*
	 * [Ei toimi koska puskurit poistetaan getByteBuffer] public synchronized
	 * ByteBuffer[] getChunks() { if (close<0) throw new
	 * RuntimeException("Not closed"); ByteBuffer[] result = new ByteBuffer[close];
	 * for (int i=0; i<close; i++) result[i] = bufs.get(i); return result; }
	 */
}

 ...
 * thread at a time, and it the event handled accordingly, new selection events
 * do not occur.
 */
public class AsyncSelector implements Runnable {
	 ...
	 */
	public void interestOps(SelectableChannel channel, int interestOps) throws CancelledKeyException {
		if (channel != null) {
	 ...
	 * <p>
	 * selectEventListener is invoked by one thread at a time. The rule of thumb is
	 * that the listener must not block.
	 *
	 * Note! If channel is registered and closed, select event is invoked until the
	 * channel is unregistered.
	 *
	 * @param channel             a {@link java.nio.channels.SelectableChannel}
	 *                            object.
	 * @param ops                 initial interest ops See {@link SelectionKey}
	 * @param selectEventListener a
	 *                            {@link org.opcfoundation.ua.utils.asyncsocket.AsyncSelector.SelectListener}
	 *                            object.
	 * @throws java.nio.channels.ClosedChannelException if any.
	 */
	public void register(SelectableChannel channel, int ops, SelectListener selectEventListener)
	...
	public interface SelectListener {
		 ...
		 * 
		 * Note! InterestSet of key is set to 0, the implementor must return as a result
		 * of onSelection the new set of interest ops.
		 * 
			...
			while (sel.isOpen()) {
				...
				for (SelectionKey key : selectedKeys)
					try {
						...
						if (l != null)
							l.onSelected(AsyncSelector.this, key.channel(), readyOps, iop);
					} catch (CancelledKeyException e) {
						/* ignore */}

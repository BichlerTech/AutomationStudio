...
/**
 * ListenableSocketChannel adds event listening convenience to the use of async
 * sockets.
 * <p>
 * Select events (read, write, connect) are handled in thread other than
 * selector. The thread is determined by an executor which is given as argument
 * to the constructor. ListenableSocketChannel guarantees that each event type
 * (read/write/connect) is handled at most by one thread.
 *
...
public class ListenableSocketChannel {
	 ...
	 */
	public ListenableSocketChannel(Executor eventExecutor) throws IOException {
		this((SocketChannel) SocketChannel.open().configureBlocking(false), eventExecutor);
	}

	/**
	 * Create async-socket wrapper over given socket channel and given listener
	 * handling thread.
	 *
	 ...
	 * @throws java.io.IOException if any.
	 */
	public ListenableSocketChannel(SocketChannel channel, Executor eventExecutor) throws IOException {
		this(channel, eventExecutor, new AsyncSelector());
		ownsSelector = true;
	}

	/**
	 * Create async-socket wrapper over given socket channel, given listener
	 * handling thread, and given selector thread.
	 *
	 * @param channel        a {@link java.nio.channels.SocketChannel} object.
	 * @param executor       a {@link java.util.concurrent.Executor} object.
	 * @param selectorThread a
	 *                       {@link org.opcfoundation.ua.utils.asyncsocket.AsyncSelector}
	 *                       object.
	 * @throws java.io.IOException if any.
	 */
	public ListenableSocketChannel(SocketChannel channel, Executor executor, AsyncSelector selectorThread)
		...
		if (channel.isBlocking())
			throw new IllegalArgumentException(
					"channel arg must be in non-blocking mode. (SocketChannel.configureBlocking(false))");
	...
	public /* synchronized */ void close() {
		// These are used is logging and if-clauses; call methods only once for
		// performance
	...
	/**
	 * Determines whether to enable or disable operation interest ops
	 * 
	...
	/**
	 * Connect to a remote socket. To monitor connect result, set ConnectListener
	 * first.
	 *
	 ...
	 * @throws java.io.IOException if any.
	 */
	public boolean syncConnect(SocketAddress addr, long timeout) throws IOException {
	...
	public interface ConnectionListener {
		/**
		 * This event is triggered when an incoming connection has been established, or
		 * when there was an error in connect attempt. The implementor must invoke
		 * getChannel().finishConnect() to determine which was the case.
		 * 
		 * @param sender sender
		 */
	...
	public interface ReadableListener {
		/**
		 * This event is triggered either (a) data can be read from the socket channel
	 ...
	 *
	 * @param connectListener a
	 *                        {@link org.opcfoundation.ua.utils.asyncsocket.ListenableSocketChannel.ConnectionListener}
	 *                        object.
	 */
	 ...
	 *
	 * @param readListener a
	 *                     {@link org.opcfoundation.ua.utils.asyncsocket.ListenableSocketChannel.ReadableListener}
	 *                     object.
	 */
	...
	Runnable readRun = new Runnable() {
		public void run() {
			...
			try {
				if (rl != null)
					rl.onDataReadable(ListenableSocketChannel.this);
			} finally {
...
//				}
	...
	/**
	 * Set write listener (makes its selector key interAsyncSocketImplested on write
	 * events)
	 *
	 * @param writeListener a
	 *                      {@link org.opcfoundation.ua.utils.asyncsocket.ListenableSocketChannel.WriteableListener}
	 *                      object.
	 */
	...
	Runnable writeRun = new Runnable() {
		public void run() {
			...
			try {
				if (wl != null)
					wl.onDataWriteable(ListenableSocketChannel.this);
			} finally {
//				synchronized(ListenableSocketChannel.this) {
...
//				}
	...
	/**
	 * <p>
	 * Getter for the field <code>connectListener</code>.
	 * </p>
	 *
	 * @return a
	 *         {@link org.opcfoundation.ua.utils.asyncsocket.ListenableSocketChannel.ConnectionListener}
	...
	/**
	 * <p>
	 * Getter for the field <code>readListener</code>.
	 * </p>
	 *
	 * @return a
	 *         {@link org.opcfoundation.ua.utils.asyncsocket.ListenableSocketChannel.ReadableListener}
	...
	/**
	 * <p>
	 * Getter for the field <code>writeListener</code>.
	 * </p>
	 *
	 * @return a
	 *         {@link org.opcfoundation.ua.utils.asyncsocket.ListenableSocketChannel.WriteableListener}

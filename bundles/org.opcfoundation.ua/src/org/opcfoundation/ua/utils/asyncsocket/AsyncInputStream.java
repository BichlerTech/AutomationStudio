...
/**
 * Asyncronous input stream of asynchronous socket. There are two positions
 * properties: Bytes received (to buffer) and bytes read.
 * <p>
 * Synchronous reads can be made with use of alarm, e.g. BufferMonitor a =
 * is.createBufferMonitor(is.getPosition() + 1000, null);
 * a.waitForState(AlarmState.FINAL_STATES); byte[] data = new byte[1000];
 * is.read(data);
 *
 * <p>
 * Note there is a limit (40kb) how much data is buffered. To up the limit set
 * an alarm. Creating an alarm to position Long.MAX_VALUE removes any limits of
 * the buffer.
 *
 * @see AsyncSocketImpl
 * @see BufferMonitor
...
public abstract class AsyncInputStream extends InputStream {
	...
	/**
	 * Create an object that monitors the position input stream. Alarm is triggered
	 * when buffer reaches the requested position.
	 * <p>
	 * The buffer size is enlarged when a monitor with a position outside the buffer
	 * range is created. For instance, there is an input stream. Its position is 5,
	 * buffer size is 5. It received data to 10 and stops. If monitor at position 20
	 * is created, the stream received 10 more additional bytes.
	 *
	...
	/**
	 * Read from the input stream. Returns readable byte buffers. This is the most
	 * efficient read method as it does not copy byte data.
	 *
	 * @param len a int.
	 * @return byte buffer
	 */
	...
	/**
	 * Peek bytes to byte chunks. (Efficient for big chunks as there is no byte
	 * copying)
	 *
	 * @param len a int.

...
/**
 * Asyncronous output stream of asynchronous socket. There are two positions
 * properties: Bytes written and bytes flushed.
 * <p>
 * Flushing of data can be monitored asyncronously with Alarm object. e.g.
 * byte[] data; long pos = os.getPosition(); os.write(data); Alarm a =
 * os.createAlarm(pos + data.length, flushListener);
 *
 * @see BufferMonitor
 * @see AsyncSocketImpl
...
public abstract class AsyncOutputStream extends OutputStream {
	...
	/**
	 * Offers byte buffer to the output stream for write. The ownership of the byte
	 * buffer and its back-end will be taken over by the stream.
	 *
	...
	/**
	 * Get the position of stream that has been flushed. This position lags behind
	 * getPosition() value
	 *

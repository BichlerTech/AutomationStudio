 ...
 */
public class AsyncSocketImpl extends AbstractState<SocketState, IOException>
	...
	public AsyncSocketImpl() throws IOException {
		this((SocketChannel) SocketChannel.open().configureBlocking(false), CurrentThreadExecutor.INSTANCE,
				new AsyncSelector());
	}
	 ...
	 * @throws java.io.IOException if any.
	 */
	public AsyncSocketImpl(SocketChannel chan) throws IOException {
	 ...
	 * @throws java.io.IOException if any.
	 */
	public AsyncSocketImpl(SocketChannel channel, Executor triggerExecutor, AsyncSelector selectorThread)
			throws IOException {
	 ...
	 * </p>
	 *
	 * @return a {@link org.opcfoundation.ua.utils.asyncsocket.AsyncInputStream}
	 *         object.
	 */
	 ...
	 * </p>
	 *
	 * @return a {@link org.opcfoundation.ua.utils.asyncsocket.AsyncOutputStream}
	 *         object.
	 */
	 ...
	 * </p>
	 *
	 * @return a {@link org.opcfoundation.ua.utils.asyncsocket.AsyncSocketImpl}
	 *         object.
	 * @throws java.io.IOException if any.
	 */
	 ...
	 * </p>
	 *
	 * @param state a {@link org.opcfoundation.ua.utils.asyncsocket.SocketState}
	 *              object.
	 * @return a boolean.
	 */
	 ...
	 * returns without exeception). Once the connection attempt is finished the
	 * object state will shift either to Error or to Connected (See getState()).
	 * State changes can be monitored by attaching listeners (See
	 * addStateListener()).
	 */
	 ...
	 * @throws java.io.IOException if any.
	 */
	public boolean syncConnect(SocketAddress addr) throws IOException {
		connect(addr);
		try {
	...
	public void closeOnFlush() {
		getOutputStream().createMonitor(getOutputStream().getPosition(), new MonitorListener() {
			@Override
			public void onStateTransition(IStatefulObject<BufferMonitorState, ?> monitor, BufferMonitorState oldState,
	...
	class AsyncSocketInputStream extends AsyncInputStream {
		...
		private synchronized void readChannel() {
			// Only one thread goes here at a time thanks to the impl in
...
//						a.trigger();
	...
	class AsyncSocketOutputStream extends AsyncOutputStream {
		...
		private synchronized void writeToChannel() {
			// Only one thread goes here at a time due to the impl in
			// ListenableSocketChannel.
			// Channel is writable
			while (!q.isEmpty()) {
				try {

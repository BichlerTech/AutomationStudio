...
public class CryptoUtil {
	...
	/**
	 * Log4J Error logger. Security failures are logged with INFO level. Security
	 * info are printed with DEBUG level. Unexpected errors are logged with ERROR
	 * level.
	 */
	 ...
	 * @param algorithm a
	 *                  {@link org.opcfoundation.ua.transport.security.SecurityAlgorithm}
	 *                  object.
	 ...
	 */
	@Deprecated
	public static byte[] asymmEncrypt(byte[] input, Key key, SecurityAlgorithm algorithm)
			throws InvalidKeyException, IllegalBlockSizeException, BadPaddingException, ServiceResultException,
	 ...
	 * @return MAC
	 * @throws org.opcfoundation.ua.common.ServiceResultException Bad_SecurityPolicyRejected
	 *                                                            algorithm not
	 *                                                            supported
	 */
	 ...
	 * @return an array of byte.
	 * @throws org.opcfoundation.ua.common.ServiceResultException if any.
	 * @deprecated use {@link #createNonce(int)} and get the size from
	 *             {@link SecurityPolicy#getSecureChannelNonceLength()} directly
	 */
	@Deprecated
	public static ByteString createNonce(SecurityAlgorithm algorithm) throws ServiceResultException {
	 ...
	 * @param profile       a
	 *                      {@link org.opcfoundation.ua.transport.security.SecurityConfiguration}
	 *                      object.
	 ...
	 * @throws org.opcfoundation.ua.common.ServiceResultException if any.
	 */
	public static void decryptAsymm(PrivateKey decryptingKey, SecurityConfiguration profile, byte[] dataToDecrypt,
			byte[] output, int outputOffset) throws ServiceResultException {

		CryptoUtil.getCryptoProvider().decryptAsymm(decryptingKey,
				profile.getSecurityPolicy().getAsymmetricEncryptionAlgorithm(), dataToDecrypt, output, outputOffset);
	}
	 ...
	 * @return an array of byte.
	 */
	public static byte[] encryptAsymm(byte[] input, PublicKey key, SecurityAlgorithm algorithm)
			throws InvalidKeyException, IllegalBlockSizeException, BadPaddingException, ServiceResultException,
			NoSuchAlgorithmException, NoSuchPaddingException {
		int outputBlockSize = getCipherBlockSize(algorithm, key);
		byte[] output = new byte[outputBlockSize];
		CryptoUtil.getCryptoProvider().encryptAsymm(key, algorithm, input, output, 0);
		return output;
	}
	 ...
	 *
	 * @param encryptingCertificate Certificate which public key will be used during
	 *                              encryption.
	 * @param profile               Asymmetric encryption algorithm will be taken
	 *                              from this SecurityConfiguration
	 * @param dataToEncrypt         Data to encrypt
	 * @param output                output
	 * @param outputOffset          output offset
	 * @throws org.opcfoundation.ua.common.ServiceResultException if any.
	 */
	public static void encryptAsymm(Certificate encryptingCertificate, SecurityConfiguration profile,
			byte[] dataToEncrypt, byte[] output, int outputOffset) throws ServiceResultException {
		LOGGER.info("encryptAsymm called.");
		CryptoUtil.getCryptoProvider().encryptAsymm(encryptingCertificate.getPublicKey(),
				profile.getSecurityPolicy().getAsymmetricEncryptionAlgorithm(), dataToEncrypt, output, outputOffset);
	}
	 ...
	 * @return Cipher
	 * @throws org.opcfoundation.ua.common.ServiceResultException if algorithm is
	 *                                                            not supported by
	 *                                                            the stack
	 */
	 ...
	 *                                                            supported
	 */
	public static int getCipherBlockSize(SecurityAlgorithm algorithm, Key key) throws ServiceResultException {
		// No security
		if (algorithm == null)
			return 1;
	 ...
	 * @param omitProtocol    if true the first 3 characters are ignored in compare
	 * @return an intersection of suites
	 */
	public static String[] getCipherSuiteIntersection(String[] cipherSuiteSet1, String[] cipherSuiteSet2,
	...
	/**
	 * Returns {@link CryptoProvider} previously set with
	 * {@link #setCryptoProvider(CryptoProvider)}. If it is not set, tries to load
	 * either Bouncy or SpongyCastle based on the return value of
	 * {@link #getSecurityProviderName()}. Throws {@link RuntimeException} if cannot
	 * be loaded.
	 */
	...
	/**
	 * Returns the length of the nonce to be used with an asymmetric or symmetric
	 * encryption algorithm.
	 * <p>
	 * For symmetric algorithms, returns the algorithm key size (in bytes). For
	 * asymmetric algorithms, returns 32.
	 *
	 * @param algorithm encryption algorithm or null (=no encryption)
	 * @return the length of the nonce in bytes
	 * @throws org.opcfoundation.ua.common.ServiceResultException Bad_SecurityPolicyRejected,
	 *                                                            if the algorithm
	 *                                                            is not supported
	 * @deprecated use {@link SecurityPolicy#getSecureChannelNonceLength()} directly
	 *             instead
	 */
	@Deprecated
	public static int getNonceLength(SecurityAlgorithm algorithm) throws ServiceResultException {
		if (algorithm == null)
			return 0;
		if (algorithm.equals(SecurityAlgorithm.Rsa15))
			return 32;
		if (algorithm.equals(SecurityAlgorithm.RsaOaep))
			return 32;
		if (SecurityAlgorithm.AlgorithmType.SymmetricEncryption == algorithm.getType())
			return algorithm.getKeySize() / 8;
	 ...
	 * @param key               Optional, required for asymmetric encryption
	 *                          algorithms
	 * @return cipher block size
	 * @throws org.opcfoundation.ua.common.ServiceResultException Bad_SecurityPolicyRejected
	 *                                                            algorithm not
	...
	/**
	 * <p>
	 * Getter for the field <code>random</code>.
	 * </p>
	 *
	 * @return the random
	 */
	...
	/**
	 * The Preferred Security Provider name. If not set via
	 * {@link #setSecurityProviderName(String)}, will return 'SC' on android and
	 * 'BC' otherwise. Otherwise returns the set String.
	 */
	 ...
	 *                                                            supported
	 */
	public static int getSignatureSize(SecurityAlgorithm signatureAlgorithm, Key key) throws ServiceResultException {
	 ...
	 * NOTE! This method will set the {@link #setSecurityProviderName(String)} from
	 * the given provider.
	 *
	 * @param cryptoProvider the cryptoProvider to set, if null will clear the
	 *                       previous one.
	 */
	 ...
	 * automatically if SpongyCastle (on Android) or BouncyCastle is found, but you
	 * may define the provider name that you wish to use yourself.
	 * 
	 * NOTE! Calling this with a different provider name than previously set will
	 * reset possible calls made to {@link #setCryptoProvider(CryptoProvider)}.
	 * 
	 *
	 * @param securityProviderName the securityProviderName to set, e.g. "BC" for
	 ...
	 * </p>
	 *
	 * @param signerKey  the private key used to sign the data
	 * @param algorithm  asymmetric signer algorithm, See {@link SecurityAlgorithm}
	 * @param dataToSign the data to sign
	 * @return SignatureData
	 * @throws org.opcfoundation.ua.common.ServiceResultException if the signing
	 *                                                            fails. Read the
	 *                                                            StatusCode and
	 *                                                            cause for more
	 *                                                            details
	 */
	public static SignatureData signAsymm(PrivateKey signerKey, SecurityAlgorithm algorithm, byte[] dataToSign)
			throws ServiceResultException {
		if (algorithm == null)
			return new SignatureData(null, null);
		return new SignatureData(algorithm.getUri(),
				ByteString.valueOf(getCryptoProvider().signAsymm(signerKey, algorithm, dataToSign)));
	}

	/**
	 * Convenience method for "displaying" a hex-string of a given byte array. Calls
	 * {@link #toHex(byte[], int)} with bytesPerRow=0 (no line breaks)
	 *
	 ...
	 *
	 * @param bytes       the byte array to "display"
	 * @param bytesPerRow number of bytes to include on a text row. If it is 0, no
	 ...
	 * Verify a signature.
	 *
	 * @param certificate the certificate used to verify the signature
	 * @param algorithm   the signature algorithm
	 * @param data        data to verify
	 * @param signature   the signature to verify
	 * @return true if the signature is valid
	 * @throws org.opcfoundation.ua.common.ServiceResultException if the
	 *                                                            verification fails
	 */
	public static boolean verifyAsymm(X509Certificate certificate, SecurityAlgorithm algorithm, byte[] data,
			byte[] signature) throws ServiceResultException {
		return getCryptoProvider().verifyAsymm(certificate.getPublicKey(), algorithm, data, signature);
	}

	/**
	 * Returns a loaded JCE {@link Provider} of the given class. Tries to add the
	 * provider if it is not already loaded. Throws {@link IllegalArgumentException}
	 * if cannot be done.
	 */
	public static synchronized Provider loadOrInstallProvider(String jceName, String providerName)

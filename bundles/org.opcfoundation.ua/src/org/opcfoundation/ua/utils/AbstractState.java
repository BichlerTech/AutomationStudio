...
/**
 * This is a default implementation to {@link IStatefulObject}. This class can
 * be subclassed or used as it. The state type is parametrized (typically an
 * enumeration).
 *
...
public abstract class AbstractState<StateType, ErrorType extends Throwable>
	...
	// TODO Replace with array that provides snapshots with-out instantiating new
	// object
	...
	/**
	 * Creates a state with a error state. The state object goes to errorState on
	 * setError().
	 *
	 * @param initialState a StateType object.
	...
	public synchronized void addStateListener(StateListener<StateType> listener) {
		...
		if (listenerList == null)
			listenerList = new SnapshotArray(StateListener.class);
	 ...
	 * {@inheritDoc}
	 *
	 * Add post-event notification listener. The prosessing thread is random. The
	 * prosessing order is not guaranteed if the handling is not synchronized.
	 */
	@Override
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public synchronized void addStateNotifiable(StateListener<StateType> listener) {
		...
		if (notifiableList == null)
			notifiableList = new SnapshotArray(StateListener.class);
	...
	public synchronized void removeStateNotifiable(StateListener<StateType> listener) {
		if (notifiableList != null)
			notifiableList.remove(listener);
	...
	/** {@inheritDoc} */
	@Override
	public StateType waitForState(Set<StateType> set, long timeout, TimeUnit unit)
	...
	/**
	 * Attempts to change the state. The state will be changed only if current state
	 * is one of the expected states.
	 *
	 ...
	 *
	 * @param state              a StateType object.
	 * @param listenerExecutor   executor for post listener handling or null for
	 *                           immediate
	...
	protected StateType setState(StateType state, Executor listenerExecutor, Set<StateType> prerequisiteStates) {
		if (listenerExecutor != null && listenerExecutor instanceof CurrentThreadExecutor)
		...
		synchronized (this) {
			...
			this.state = newState;
			if (listenerList != null)
				listeners = listenerList.getArray();
			if (notifiableList != null)
				notifiables = notifiableList.getArray();
		}
		synchronized (lock) {
			lock.notifyAll();
		}

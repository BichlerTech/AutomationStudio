...
/**
 * A ByteArrayOutputStream that has a maximum size limit, after which it will
 * throw {@link EncodingLimitsExceededIoException} if more is tried to be
 * written.
 */
public class LimitedByteArrayOutputStream extends OutputStream {

	/**
	 * Creates a new limited {@link ByteArrayOutputStream} with the given limit and
	 * default capasity of {@link ByteArrayOutputStream}
	 * 
	 * @param maxBytes maximum number of bytes that can be written to the stream
	 *                 before it throws
	...
	/**
	 * Behaves same as {@link ByteArrayOutputStream#write(int)}, but if the size of
	 * the underlying stream is at limit, will throw
	 * {@link EncodingLimitsExceededIoException} instead
	 */
	@Override
	...
	public void write(byte[] b, int off, int len) throws EncodingLimitsExceededIoException {
		// NOTE, if b is null will throw NPE, which the the same logic as in
		// OutputStream.
		if (delegate.size() >= limit - len) {
			throw new EncodingLimitsExceededIoException(
					"Stream size is " + delegate.size() + " limit is " + limit + ", cannot write " + len + " bytes");
		} else {

...
public class MultiDimensionArrayUtils {
	...
	/**
	 * Format object to String. Does not decode ExtensionObjects (use
	 * {@link #toString(Object, EncoderContext)} for that).
	 *
	...
	/**
	 * Format object to String, try decoding {@link ExtensionObject}s with the given
	 * context. Uses {@link ExtensionObject#toString()} if decoding fails.
	 * 
	 * @param o   the object
	 * @param ctx context to use for {@link ExtensionObject} decoding, can be null,
	 *            in which case no decoding is attempted
	 * @return String representation of the object
	 */
	...
	/**
	 * Clones the multi-dimension structure of an array but the not the individual
	 * elements. Element references remain the same is in the original.
	 *
	...
	public static Object[] createMultiDimArray(Class<?> componentType, int[] dims) {
		...
		/*
		 * // Older than 1.6.0 if (dims==null || componentType==null || dims.length==0)
		 * throw new IllegalArgumentException(); if (dims.length==1) return
		 * Array.newInstance(componentType, dims[0]);
		 * 
		 * int[][] dimArrays = new int[dims.length][]; dimArrays[0] = dims; for (int
		 * j=1; j<dims.length; j++) { dimArrays[j] = new int[dims.length-j];
		 * System.arraycopy(dims, 1, dimArrays[j], 0, dims.length-j); } return
		 * _createMultiDimArray(componentType, dimArrays, 0);
		 */
	}
	/*
	 * private static Object[] _createMultiDimArray(Class<?> componentType, int[][]
	 * dimArrays, int lvl) { int[] dims = dimArrays[lvl]; int len = dims[0];
	 * Object[] result = (Object[]) Array.newInstance(componentType, dims); if
	 * (lvl<dimArrays.length-1) for (int i=0; i<len; i++) result[i] =
	 * _createMultiDimArray(componentType, dimArrays, lvl+1); return result; }
	 */
	 ...
	 * @param <R>     a R object.
	 * @return a
	 *         {@link org.opcfoundation.ua.utils.MultiDimensionArrayUtils.ArrayIterator}
	 *         object.
	 */

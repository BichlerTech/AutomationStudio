...
/**
 * A cache for XXXFactory instances which are needed in XML processing. Idea is
 * that this is the common place to find a factory so it needs to be created
 * only once (which is a time-consuming operation).
 */
public class XMLFactoryCache {
	...
	private static final SAXTransformerFactory SAX_TRANSFORMER_FACTORY = (SAXTransformerFactory) SAXTransformerFactory
			.newInstance();
	...
	/**
	 * Set this to true to suppress error logs if DTD processing cannot be disabled
	 * (CVE-2018-12585). This should be only used if custom XML parser framework is
	 * in use and the equivalent of default initialization has been done to the
	 * static factories available.
	 * 
	 * @see <a href=
	 *      "https://www.owasp.org/index.php/XML_External_Entity_(XXE)_Prevention_Cheat_Sheet#Java">https://www.owasp.org/index.php/XML_External_Entity_(XXE)_Prevention_Cheat_Sheet#Java</a>
	 * 
	 */
	public static void setIgnoreErrorsOnDefaultInitialization(boolean ignoreErrorsOnDefaultInitialization) {
	...
	static {
		...
		try {
			...
			if (!ignoreErrorsOnDefaultInitialization) {
				logger.warn(
						"Cannot initialize XML factories to ignore DTD processing (CVE-2018-12585), please update java to newer version or configure manually if custom XML parser framework is in use");
			}
		}

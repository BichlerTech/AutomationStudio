...
/**
 * IncubationQueue is ordered queue where objects are added in two phases; (a)
 * as incubating, they are given queuing number, and as (b) hatched they become
 * available to the consumer. Objects become consumable when they are hatch()ed.
 * The order of incubation is maintained. Objects become consumable in the order
 * they were incubate()ed. This class is synchronized and is multi-thread-safe.
 * <p>
 * Example:
 *
 * IncubationQueue q = new IncubationQueue(); q.incubate("a"); q.incubate("b");
 * q.incubate("c"); q.hatch("b"); q.removeNextHatchedIfAvailable(); // returns
 * null q.hatch("a"); q.removeNextHatched(); // returns "a"
 * q.removeNextHatched(); // returns "b" q.removeNextHatchedIfAvailable(); //
 * returns null q.hatch("c"); q.removeNextHatched(); // returns "c"
 *
...
public class IncubationQueue<T> {
	 ...
	 *
	 * @param identityComparison if true objects are compared with ==, false compare
	 *                           with equals()/hashCode()
	 */
	public IncubationQueue(boolean identityComparison) {
		hatchMap = identityComparison ? new IdentityHashMap<T, T>() : new HashMap<T, T>();
	}
	...
	/**
	 * Hatch incubating object o. If all objects in the queue before o are hatched,
	 * all of them are returned in the order they were added to the queue.
	 *
	 ...
	 */
	public synchronized boolean hatch(T o) throws IllegalArgumentException {
		if (o == null)
			throw new IllegalArgumentException("null arg");
		if (!hatchMap.containsKey(o))
	 ...
	 * @throws java.lang.InterruptedException if any.
	 */
	public synchronized void waitUntilIncubated(T o) throws InterruptedException {

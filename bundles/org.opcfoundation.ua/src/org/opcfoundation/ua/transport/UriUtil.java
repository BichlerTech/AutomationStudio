...
public class UriUtil {
	...
	public static final int HTTP_DISCOVERY_PORT = 56201; // Alternate to 80 (not in 1.04, 1.03 defined this in Part 6
															// section 7.4)
	...
	public static final Pattern PATTERN_HTTPS = Pattern.compile("^opc.https://([^/]+)(/.*)?$",
	 ...
	 * @throws org.opcfoundation.ua.common.ServiceResultException Bad_ServerUriInvalid
	 *                                                            if the protocol is
	 *                                                            unknown
	 */
	public static MessageFormat getMessageFormat(String endpointUri) throws ServiceResultException {
		if (PATTERN_OPCTCP.matcher(endpointUri).matches())
			return MessageFormat.Binary;
		if (PATTERN_HTTPS.matcher(endpointUri).matches())
			return MessageFormat.Binary;
		if (PATTERN_HTTP.matcher(endpointUri).matches())
			return MessageFormat.Binary;
	 ...
	 */
	public static InetSocketAddress getSocketAddress(String endpointUri) throws ServiceResultException {
		try {
			URI uri = new URI(endpointUri);
			if (uri.getScheme() == null)
				throw new ServiceResultException(StatusCodes.Bad_ServerUriInvalid,
						"Invalid endpointUri (no scheme): " + endpointUri);

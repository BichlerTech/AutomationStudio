...
public class HttpsClientSecureChannel implements SecureChannel {
	 ...
	 * </p>
	 *
	 * @param client a {@link org.opcfoundation.ua.transport.https.HttpsClient}
	 *               object.
	 */
...
//			createSecureChannel(false);
	...
	public ServiceResponse serviceRequest(ServiceRequest request) throws ServiceResultException {
		AsyncResult<ServiceResponse> asyncResult = client.serviceRequestAsync(request, client.getTimeout(request),
				secureChannelId);
...
//	}
	...
	public void close() {
		...
		HashMap<Integer, HttpsClientPendingRequest> copy = new HashMap<Integer, HttpsClientPendingRequest>(
				client.requests);
	 ...
	 */
	// AtomicReference<TimerTask> timeoutPendingRequestsTask = new
	 ...
	 * logger.debug("createSecureChannel: renew="+renew); final long startTime =
	 * System.currentTimeMillis(); int requestId =
	 * client.requestIdCounter.incrementAndGet(); if (logger.isDebugEnabled())
	 * logger.debug("createSecureChannel: requestId="+requestId);
	 * 
	 * OpenSecureChannelRequest req = new OpenSecureChannelRequest();
	 * 
	 * SecurityPolicy policy = SecurityPolicy.getSecurityPolicy(
	 * client.transportChannelSettings.getDescription().getSecurityPolicyUri() );
	 * SecurityAlgorithm algo = policy.getAsymmetricEncryptionAlgorithm(); int
	 * nonceLength = CryptoUtil.getNonceLength( algo ); byte[] nonce =
	 * CryptoUtil.createNonce( nonceLength );
	 * 
	 * Integer tokenLifetime =
	 * client.transportChannelSettings.getConfiguration().getSecurityTokenLifetime()
	 * ; if (tokenLifetime==null) tokenLifetime = 3600000; if
	 * (logger.isDebugEnabled()) logger.debug("tokenLifetime: " + tokenLifetime);
	 * 
	 * req.setClientNonce( nonce ); req.setClientProtocolVersion(
	 * UnsignedInteger.valueOf(0) ); req.setRequestedLifetime(
	 * UnsignedInteger.valueOf( tokenLifetime ) ); req.setRequestType( renew ?
	 * SecurityTokenRequestType.Renew : SecurityTokenRequestType.Issue );
	 * req.setSecurityMode(
	 * client.transportChannelSettings.getDescription().getSecurityMode() );
	 * 
	 * int chanId = renew ? this.secureChannelId : 0;
	 * 
	 * IEncodeable msg = client.serviceRequest( req ); if ( msg instanceof
	 * OpenSecureChannelResponse == false ) { if ( msg instanceof ServiceFault ) {
	 * throw new ServiceFaultException( (ServiceFault) msg ); } else { throw new
	 * ServiceResultException( "Unexpected response "+msg ); } }
	 * OpenSecureChannelResponse res = (OpenSecureChannelResponse) msg;
	 * ChannelSecurityToken token = res.getSecurityToken(); this.secureChannelId =
	 * token.getChannelId().intValue();
	 * 
	 * if (logger.isDebugEnabled()) if (renew) {
	 * logger.debug(this.secureChannelId+" Secure channel renewed, SecureChannelId="
	 * +this.secureChannelId+", TokenId="+token.getTokenId().longValue()); } else {
	 * logger.debug(this.secureChannelId+" Secure channel opened, SecureChannelId="
	 * +this.secureChannelId+", TokenId="+token.getTokenId().longValue()); }
	 * 
	 * // HAX! In Reconnect to secure channel -situation, the C# Server
	 * implementation sends // two conflicting secure channel id's. // The old
	 * channel (correct) in message header and a new channel id in the message body.
	 * // if (renew) this.secureChannelId = chanId;
	 * 
	 * long currentTime = System.currentTimeMillis(); this.tokenIssueTime =
	 * startTime/2 + currentTime/2; this.tokenLifetime =
	 * token.getRevisedLifetime().longValue();
	 * 
	 * // Cancel token renewal { TimerTask t = renewSecurityTokenTask;
	 * renewSecurityTokenTask = null; if (t!=null) t.cancel(); } // Setup new token
	 * renewal { long renewTime = token.getRevisedLifetime().longValue(); if
	 * (logger.isDebugEnabled()) logger.debug("RevisedLifetime: " + renewTime);
	 * renewSecurityTokenTask = TimerUtil.schedule(client.timer,
	 * renewSecurityTokenRunnable, executor, currentTime + (long)(renewTime*
	 * TcpMessageLimits.TokenRenewalPeriod )); }
	 * 
	 * }
	 * 
	 * private Runnable renewSecurityTokenRunnable = new Runnable() { public void
	 * run() { try { if (logger.isDebugEnabled())
	 * logger.debug(secureChannelId+" Renewing security token");
	 * createSecureChannel(true); } catch (ServiceResultException e) {

 ...
 * All HTTPS communications via a URL shall be treated as a single SecureChannel
 * that is shared by multiple Clients. Stack shall provide a unique identifier
 * for the SecureChannel which allows Applications correlate a request with a
 * SecureChannel.This means that Sessions can only be considered secure if the
 * AuthenticationToken (see Part 4) is long (&gt;20 bytes) and HTTPS encryption
 * is enabled.
 */
public class HttpsServerConnection extends AbstractState<CloseableObjectState, ServiceResultException>
	 ...
	 * </p>
	 *
	 * @param server a {@link org.opcfoundation.ua.transport.https.HttpsServer}
	 *               object.

...
public class HttpsServerEndpointHandler implements HttpAsyncRequestHandler<HttpRequest> {
	 ...
	 * </p>
	 *
	 * @param endpointBinding a
	 *                        {@link org.opcfoundation.ua.transport.EndpointBinding}
	 *                        object.
	 */
	...
	public void handle(HttpRequest request, HttpAsyncExchange httpExchange, HttpContext context)
		...
		HttpsServerPendingRequest req = new HttpsServerPendingRequest(this, httpExchange, request, singleSecureChannel,
	...
	/**
	 * Handle incoming request. This method is called from HttpsServerPendingRequest
	 * from worker thread.
	 * 
	...
	void handleOpenSecureChannelRequest(OpenSecureChannelRequest req, HttpsServerPendingRequest msgExchange)
			throws ServiceResultException {
	...
	// Secure Channels
	// Map<Integer, HttpsServerSecureChannel> secureChannels = new
	// ConcurrentHashMap<Integer, HttpsServerSecureChannel>();
	// SecureChannel-TokenId <-> SecureChannel
	// Map<UnsignedInteger, HttpsServerSecureChannel> tokenIdMap = new
	// ConcurrentHashMap<UnsignedInteger, HttpsServerSecureChannel>();
	// Session-AuthenticationToken <-> SecureChannelId
	// ConcurrentBijectionMap<NodeId, Integer> authenticationTokenMap = new
	// ConcurrentBijectionMap<NodeId, Integer>();

	/*
	 * void handleOpenSecureChannelRequest(OpenSecureChannelRequest req,
	 * HttpsServerPendingRequest msgExchange) throws ServiceResultException { if
	 * (req.getRequestType() == SecurityTokenRequestType.Issue) { int
	 * secureChannelId = endpointServer.secureChannelCounter.incrementAndGet();
	 * HttpsServerSecureChannel channel = new HttpsServerSecureChannel( this,
	 * secureChannelId ); secureChannels.put(secureChannelId, channel);
	 * logger.debug(endpointServer + " SecureChannelId=" +
	 * channel.getSecureChannelId()); channel.handleOpenChannel(msgExchange, req);
	 * 
	 * } else if (req.getRequestType() == SecurityTokenRequestType.Renew) { int
	 * secureChannelId = msgExchange.secureChannelId; if ( secureChannelId == -1 ) {
	 * NodeId authenticationToken = req.getRequestHeader().getAuthenticationToken();
	 * Integer secureChannelId_ = authenticationTokenMap.getRight(
	 * authenticationToken ); if ( secureChannelId_ != null ) secureChannelId =
	 * secureChannelId_; } HttpsServerSecureChannel channel =
	 * (HttpsServerSecureChannel) secureChannels.get( secureChannelId ); if (
	 * channel == null ) throw new ServiceResultException(
	 * Bad_SecureChannelIdInvalid );
	 * 
	 * if ( !ObjectUtils.objectEquals( req.getRequestType(),
	 * SecurityTokenRequestType.Renew ) ) { throw new
	 * ServiceResultException(Bad_UnexpectedError); }
	 * 
	 * channel.handleRenewSecureChannelRequest(msgExchange, req); }
	 * 
	 * // Remove closed channels, should maybe be based on timestamps TODO Object[]
	 * channelArray = secureChannels.values().toArray(); for (Object channel :
	 * channelArray) { AbstractState<CloseableObjectState, ServiceResultException>
	 * state = (AbstractState<CloseableObjectState, ServiceResultException>)
	 * channel; if (state.equals(CloseableObjectState.Closed)) {
	 * secureChannels.remove(((AbstractServerSecureChannel)channel).
	 * getSecureChannelId()); } } } void
	 * handleCloseSecureChannelRequest(CloseSecureChannelRequest msg,
	 * HttpsServerPendingRequest msgExchange) throws ServiceResultException { int
	 * secureChannelId = msgExchange.secureChannelId; HttpsServerSecureChannel
	 * channel = (HttpsServerSecureChannel) secureChannels.get( secureChannelId );
	 * if ( channel == null ) throw new ServiceResultException(
	 * Bad_SecureChannelIdInvalid );
	 * 
	 * channel.handleCloseChannel( msgExchange, msg ); secureChannels.remove(
	 * secureChannelId ); authenticationTokenMap.removeWithRight( secureChannelId );
	 * }
	 */
	///// Secure channels are disabled /////

...
class HttpsServerPendingRequest extends EndpointServiceRequest<ServiceRequest, ServiceResponse> implements Runnable {
	...
	/**
	 * <p>
	 * Constructor for HttpsServerPendingRequest.
	 * </p>
	 *
	 * @param endpoint     a
	 *                     {@link org.opcfoundation.ua.transport.https.HttpsServerEndpointHandler}
	 *                     object.
	 * @param httpExchange a {@link org.apache.http.nio.protocol.HttpAsyncExchange}
	 *                     object.
	 * @param httpRequest  a {@link org.apache.http.HttpRequest} object.
	 * @param channel      a
	 *                     {@link org.opcfoundation.ua.transport.https.HttpsServerSecureChannel}
	 *                     object.
	 * @param requestId    a int.
	 */
	public HttpsServerPendingRequest(HttpsServerEndpointHandler endpoint, HttpAsyncExchange httpExchange,
			HttpRequest httpRequest, HttpsServerSecureChannel channel, int requestId) {
		...
		if (httpRequest instanceof HttpMessage) {
			...
			if (securityPolicyUri == null)
				securityPolicyUri = SecurityPolicyUri.URI_BINARY_NONE; // 1.04 Part 7.4.1
		}
	...
	/**
	 * Returns {@link SecurityPolicy} equivalent of the given String as part of the
	 * header 'OPCUA-SecurityPolicy'. {@link SecurityPolicy#NONE} is used if the
	 * header is not set (as defined in 1.04 Part 6 section 7.4.1).
	 */
	...
	void sendResponse(int statusCode, IEncodeable responseObject) {
		try {
			...
			if (responseObject != null) {
				try {
					logger.trace("sendResponse: requestId={} statusCode={} responseObject={}", requestId, statusCode,
							responseObject);
					logger.debug("sendResponse: requestId={} statusCode={} responseObject={}", requestId, statusCode,
					...
					if (logger.isDebugEnabled() && channel.getConnection() != null) {
						NHttpServerConnection nHttpServerConnection = ((HttpsServerConnection) channel.getConnection())
	...
	void sendError(int httpStatusCode, UnsignedInteger opcuaStatusCode, String errorMessage) {
		try {
			...
			if (errorMessage != null) {
				errorMsg = new ErrorMessage(opcuaStatusCode, errorMessage);
			}
	...
	public SecurityMode getSecurityMode() {
		/*
		 * Note that this cannot be determined from the getChannel, as that is per 1.04
		 * Part 6 section 7.4.1. all HTTPS is treated as a single SecureChannel and thus
		 * cannot represent the client selection.
		 */

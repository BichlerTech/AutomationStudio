...
class HttpsClientPendingRequest implements Runnable {
	...
	/**
	 * <p>
	 * Constructor for HttpsClientPendingRequest.
	 * </p>
	 *
	 * @param httpsClient    a
	 *                       {@link org.opcfoundation.ua.transport.https.HttpsClient}
	 *                       object.
	 * @param requestMessage a
	...
	public HttpsClientPendingRequest(HttpsClient httpsClient, ServiceRequest requestMessage) {
		...
		// Read time-out time
		UnsignedInteger timeoutHint = requestMessage.getRequestHeader() != null
				? requestMessage.getRequestHeader().getTimeoutHint()
				: null;
	...
	public void run() {
		try {
			...
			try {
				...
				// Currently, TLS_1_2 is not supported by JSSE implementations, for some odd
				// reason
			...
			result.setError(new ServiceResultException(StatusCodes.Bad_DecodingError,
					"Stack overflow: " + Arrays.toString(Arrays.copyOf(e.getStackTrace(), 30)) + "..."));
		} finally {
	 ...
	 */
	public void cancel() {
		if (httpsClient.requests.remove(this) == null)
			return;
	 ...
	 */
	public void timeout() {
		if (httpsClient.requests.remove(requestId) == null)
			return;

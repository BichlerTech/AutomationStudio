...
/**
 * Interface for encrypting and decrypting services. Designed for stack internal
 * use to allow pluggable system of providers.
 */
public interface CryptoProvider {
	 ...
	 * 
	 * @param clazz, Specific case that could require different provider name. Can
	 *               be null, if no specific one is requested.
	...
	public int decryptSymm(SecurityPolicy policy, byte[] encryptingKey, byte[] initializationVector,
			byte[] dataToDecrypt, int inputOffset, int inputLength, byte[] output, int outputOffset)
	...
	public int encryptSymm(SecurityPolicy policy, byte[] encryptingKey, byte[] initializationVector,
			byte[] dataToEncrypt, int inputOffset, int inputLength, byte[] output, int outputOffset)
			throws ServiceResultException;

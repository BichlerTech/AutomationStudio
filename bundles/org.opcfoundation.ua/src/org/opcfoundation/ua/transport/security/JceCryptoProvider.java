...
public class JceCryptoProvider implements CryptoProvider {
	...
	/**
	 * Constructs new {@link JceCryptoProvider} using the given JCE provider. JCE
	 * Providers can be obtained by calling {@link Security#getProvider(String)}.
	...
	public int decryptAsymm(PrivateKey decryptingKey, SecurityAlgorithm algorithm, byte[] dataToDecrypt, byte[] output,
		...
		try {
			...
			for (int index = outputOffset; index < maxIndex; index += inputBlockSize) {
				amountOfDecryptedBytes = cipher.doFinal(dataToDecrypt, inputOffset, inputBlockSize, output,
						outputOffset);
	...
	public int decryptSymm(SecurityPolicy policy, byte[] encryptingKey, byte[] iv, byte[] dataToDecrypt,
		...
		if (logger.isTraceEnabled()) {
			logger.trace("decrypt: token.getRemoteEncryptingKey()=" + CryptoUtil.toHex(encryptingKey));
			logger.trace("decrypt: token.getRemoteInitializationVector()=" + CryptoUtil.toHex(iv));
		...
		SecretKeySpec spec = new SecretKeySpec(encryptingKey, algorithm.getStandardName());

		try {
			cipher = Cipher.getInstance(algorithm.getTransformation());
			cipher.init(Cipher.DECRYPT_MODE, spec, new IvParameterSpec(iv));
			decryptedBytes = cipher.update(dataToDecrypt, inputOffset, inputLength, output, outputOffset);
	...
	/** {@inheritDoc} */
	@Override
	public void encryptAsymm(PublicKey remotePublicKey, SecurityAlgorithm algorithm, byte[] dataToEncrypt,
		...
		// key);
		inputBlockSize = CryptoUtil.getPlainTextBlockSize(algorithm, remotePublicKey);
		...
		try {
			...
			int inputOffset = 0;
			for (int index = outputOffset; index < maxIndex; index += inputBlockSize) {
				int amountOfEncryptedBytes = cipher.doFinal(dataToEncrypt, inputOffset,
						Math.min(maxIndex - index, inputBlockSize), output, outputOffset);
				inputOffset += inputBlockSize;
				outputOffset += amountOfEncryptedBytes;
				logger.debug("Asym encryption: amountOfEncryptedBytes={} inputOffset={} outputOffset={} index={}",
						amountOfEncryptedBytes, inputOffset, outputOffset, index);
			}
	...
	/** {@inheritDoc} */
	@Override
	public void signSymm(SecurityPolicy policy, byte[] key, byte[] input, int inputOffset, int verifyLen, byte[] output,
			int outputOffset) throws ServiceResultException {
	...
	private Cipher getAsymmetricCipher(SecurityAlgorithm algorithm, PublicKey publicKey) throws NoSuchPaddingException,
	...
	private Signature getSignature(SecurityAlgorithm algorithm) throws NoSuchAlgorithmException {

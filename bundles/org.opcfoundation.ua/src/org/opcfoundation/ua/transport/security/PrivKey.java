...
/**
 * Valid and encodeable private key. Wrapper to {@link java.security.PrivateKey}
 */
public class PrivKey {
	 ...
	 */
	public static PrivKey load(File file, final String password) throws IOException, NoSuchAlgorithmException,
	 ...
	 * @throws java.security.spec.InvalidParameterSpecException if any.
	 */
	public static PrivKey load(InputStream is, final String password) throws IOException, NoSuchAlgorithmException,
			InvalidKeySpecException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException,
			IllegalBlockSizeException, BadPaddingException, InvalidParameterSpecException {
	 ...
	 * @return a {@link org.opcfoundation.ua.transport.security.PrivKey} object.
	 */
	public static PrivKey load(byte[] keyBytes, final String password) throws IOException, NoSuchAlgorithmException,
			InvalidKeySpecException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException,
			IllegalBlockSizeException, BadPaddingException, InvalidParameterSpecException {
		...
		// this code is ...
		if (((keyBytes[0] == (byte) '-' && keyBytes[1] == (byte) '-' && keyBytes[2] == (byte) '-'))
		...
		if (isEncrypted) {
			...
			if (true) {
				...
				byte[] sKey = generateDerivedKey(keyBits / 8, pw, salt);
	...
	private static byte[] generateDerivedKey(int bytesNeeded, byte[] password, byte[] salt)
	 ...
	 * <p>
	 *
	 * @deprecated Use {@link #load(File, String)} or
	 *             {@link #loadFromKeyStore(File, String)} instead
	 * @param file a {@link java.io.File} object.
	 * @throws java.io.IOException                        if any.
	 * @throws java.security.spec.InvalidKeySpecException if any.
	 * @throws java.security.NoSuchAlgorithmException     if any.
	 ...
	 * @throws java.security.UnrecoverableKeyException if any.
	 */
	public static PrivKey loadFromKeyStore(File file, String password) throws IOException, UnrecoverableKeyException,
			NoSuchAlgorithmException, CertificateException, KeyStoreException {
		return loadFromKeyStore(file.toURI().toURL(), password);
	}

	/**
	 * Save the key in a binary file. Note that the file is not secured by a
	 * password.
	 *
	 * @param file a {@link java.io.File} object.
	 * @throws java.io.IOException if any.
	...
	public void save(File file, String privateKeyPassword) throws IOException {
		if (privateKeyPassword == null || privateKeyPassword.length() == 0) {
			PKCS8EncodedKeySpec spec = new PKCS8EncodedKeySpec(getPrivateKey().getEncoded());
	 ...
	 * @throws IOException
	 */
	private void savePemWithBC(File file, String privateKeyPassword) throws IOException {
	 ...
	 * @throws java.security.NoSuchAlgorithmException     if any.
	 */
	public PrivKey(byte[] encodedPrivateKey) throws IOException, InvalidKeySpecException, NoSuchAlgorithmException {
		if (encodedPrivateKey == null)
			throw new IllegalArgumentException("null arg");
		this.privateKey = decodeRSAPrivateKey(encodedPrivateKey);
	}

	private RSAPrivateKey decodeRSAPrivateKey(byte[] keyBytes)
		...
		try {
			...
			// For some reason the Sun provider cannot read all keys: try with Bouncy Castle
			// if it fails
			try {
				...
				throw new RuntimeException(
						"Could not read private key with default Provider and Bouncy Castle not available", e1);
			}
		}
	}
	...
	/**
	 * <p>
	 * Getter for the field <code>privateKey</code>.
	 * </p>
	 *
	 ...
	 *
	 * @param cert               the certificate used to chain the key
	 * @param file               the file used to store the key
	 * @param privateKeyPassword the password to secure the private key, must not be
	 *                           null for JKS
	 * @param keyStorePassword   the password to the key store, must not be null
	 * @param keyStoreType       key store type, either "PKCS12" or "JKS"
	 * @throws java.io.IOException                     if the file cannot be written
	 *                                                 to
	 * @throws java.security.NoSuchProviderException   Bouncy Castle Provider not
	 *                                                 found
	 ...
	 *                                                 found
	 */
	public void saveToKeyStore(Cert cert, File file, String privateKeyPassword, String keyStorePassword,
			String keyStoreType) throws IOException, KeyStoreException, NoSuchProviderException,
			NoSuchAlgorithmException, CertificateException {
		String alias = "key";
		CertificateUtils.saveToProtectedStore(getPrivateKey(), cert.getCertificate(), file, alias, privateKeyPassword,

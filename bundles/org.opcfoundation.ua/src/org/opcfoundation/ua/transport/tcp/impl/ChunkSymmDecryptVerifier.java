...
public class ChunkSymmDecryptVerifier implements Runnable {
	...
	/**
	 * <p>
	 * Constructor for ChunkSymmDecryptVerifier.
	 * </p>
	 *
	 * @param chunk a {@link java.nio.ByteBuffer} object.
	 * @param token a {@link org.opcfoundation.ua.transport.tcp.impl.SecurityToken}
	 *              object.
	 */
	...
	public void run() throws RuntimeServiceResultException {
		...
		try {
			...
			if (msm == MessageSecurityMode.SignAndEncrypt) {
				...
				decryptedBytes = decrypt(token, dataToDecrypt, 0, dataToDecrypt.length, chunk.array(),
	...
	private int decrypt(SecurityToken token, byte[] dataToDecrypt, int inputOffset, int inputLength, byte[] output,
			int outputOffset) throws ServiceResultException {
		logger.debug("decrypt: dataToDecrypt.length={} inputOffset={} inputLength={} output.length={} outputOffset={}",
				dataToDecrypt.length, inputOffset, inputLength, output.length, outputOffset);
		return CryptoUtil.getCryptoProvider().decryptSymm(token.getSecurityPolicy(), token.getRemoteEncryptingKey(),
				token.getRemoteInitializationVector(), dataToDecrypt, inputOffset, inputLength, output, outputOffset);
	}

	private void verify(SecurityToken token, byte[] dataToVerify, byte[] signature) throws ServiceResultException {
		CryptoUtil.getCryptoProvider().verifySymm(token.getSecurityPolicy(), token.getRemoteSigningKey(), dataToVerify,
				0, dataToVerify.length, signature);
	}

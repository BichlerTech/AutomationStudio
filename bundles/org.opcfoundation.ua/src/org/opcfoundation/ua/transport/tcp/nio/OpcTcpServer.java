 ...
 */
public class OpcTcpServer extends AbstractState<CloseableObjectState, ServiceResultException>
	...
	public EndpointHandle bind(SocketAddress socketAddress, EndpointBinding endpointBinding)
		...
		if (!"opc.tcp".equals(scheme))
			throw new ServiceResultException(StatusCodes.Bad_UnexpectedError,
					"Cannot bind " + scheme + " to opc.tcp server");
		...
		if (socketHandle.socket == null) {
			try {
				...
				socketHandle.socket = new AsyncServerSocket(socketHandle.getChannel(),
	...
	@Override
	public void bindReverse(final SocketAddress addressToConnect, final String endpointUrl) {
		...
		if (socketHandle.socket == null) {
			try {
				...
				socketHandle.socket = new AsyncSocketImpl(socketHandle.getChannel(),
						StackUtils.getNonBlockingWorkExecutor(), StackUtils.getSelector());
				...
				final OpcTcpServerConnection conn = new OpcTcpServerConnection(OpcTcpServer.this, socketHandle.socket,
						rh);
				connections.addConnection(conn);
	 ...
	 *
	 * @return an array of
	 *         {@link org.opcfoundation.ua.transport.tcp.nio.OpcTcpServer.SocketHandle}
	 *         objects.
	 */
	...
	public class SocketHandle {
		...
		void close() {
			for (OpcTcpEndpointHandle eph : endpoints.values())
				eph.close__();

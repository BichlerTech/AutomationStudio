...
public class ChunkAsymmDecryptVerifier implements Runnable {
	...
	public void run() throws RuntimeServiceResultException {
		try {
			...
			if (msm == MessageSecurityMode.SignAndEncrypt) {
				...
				// Set position to the starting point of the decrypted bytes (= starting point
				...
				decryptedBytes = decrypt(dataToDecrypt, securityProfile.getLocalPrivateKey(), chunk.array(),
						headersEnd + chunk.arrayOffset());
			...
			if (msm.hasSigning()) {
				...
				// The signature belongs to decrypted bytes, so we must reduce signatureSize of
				// bytes from decryptedBytes
			...
			chunk.limit(chunk.position() + decryptedBytes - 8 - paddingSize - signatureSize);
	...
	private boolean verify(byte[] dataToVerify, Certificate signingCertificate, byte[] signature)
		...
		return CryptoUtil.getCryptoProvider().verifyAsymm(signingCertificate.getPublicKey(),
				securityProfile.getSecurityPolicy().getAsymmetricSignatureAlgorithm(), dataToVerify, signature);
	}
	...
	private int decrypt(byte[] dataToDecrypt, PrivateKey privateKey, byte[] output, int outputOffset)
			throws ServiceResultException {
		int bytesDecrypted = CryptoUtil.getCryptoProvider().decryptAsymm(privateKey,
				securityProfile.getSecurityPolicy().getAsymmetricEncryptionAlgorithm(), dataToDecrypt, output,
				outputOffset);

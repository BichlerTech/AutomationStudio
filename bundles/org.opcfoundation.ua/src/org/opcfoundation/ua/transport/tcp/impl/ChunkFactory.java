...
/**
 * Chunk factory constructs byte buffers to be used for writing. The byte buffer
 * will be backed by an array that can fit the chunk. The writable portion of
 * the byte buffer (position -&gt; limit) reflects to writable plaintext region.
 * <p>
...
public class ChunkFactory extends ByteBufferFactory {
	 ...
	 * @param securityMode       a
	 *                           {@link org.opcfoundation.ua.core.MessageSecurityMode}
	 *                           object.
	 * @param keySize            a int.
	 */
	public ChunkFactory(int maxChunkSize, int messageHeaderSize, int securityHeaderSize, int sequenceHeaderSize,
			int signatureSize, int cipherBlockSize, MessageSecurityMode securityMode, int keySize) {
	...
	public ByteBuffer allocate(int bodySize) {
		...
		if (securityMode == MessageSecurityMode.SignAndEncrypt) {
			...
			if (modulo != 0)
				padding += cipherBlockSize - modulo;
	 ...
	 * @param padding         the size of the whole padding; padding, padding fill,
	 *                        extra padding byte
	 ...
	 *
	 * ByteBuffer allocated with allocate() returns a buffer that has only plaintext
	 * as writable portion. This method expands the ByteBuffer to include header and
	 * footer.
	 *
	 ...
	 * SecurityToken token; int requestId; public SecureChannelChunkFactory(int
	 * maxChunkSize, SecurityToken token) { super(maxChunkSize, 8, 8, 8,
	 * token.getSignatureSize(), token.getCipherBlockSize(),
	 * token.getMessageSecurityMode()); this.token = token; } public SecurityToken
	 * getToken() { return token; } public int getRequestId() { return requestId; }
...
//		private static final Charset UTF8 = Charset.forName("utf-8");
		...
		public AsymmMsgChunkFactory(int maxChunkSize, SecurityConfiguration profile) throws ServiceResultException {
			super(maxChunkSize, 12, 12 + profile.getSecurityPolicy().getEncodedPolicyUri().length
					...
					profile.getMessageSecurityMode().hasSigning()
							? CryptoUtil.getSignatureSize(profile.getSecurityPolicy().getAsymmetricSignatureAlgorithm(),
									profile.getLocalPrivateKey())
							: 0,

					// Cipher block size
					profile.getMessageSecurityMode() != MessageSecurityMode.None ? CryptoUtil.getCipherBlockSize(
							profile.getSecurityPolicy().getAsymmetricEncryptionAlgorithm(),
							profile.getRemoteCertificate().getPublicKey()) : 1,
		...
		public ByteBuffer allocate(int bodySize) {
			...
			if (msm == MessageSecurityMode.Sign)
				msm = MessageSecurityMode.SignAndEncrypt;
			bodySize = Math.min(bodySize, maxPlaintextSize);
			...
			// There is always padding: paddingSize, padding fill bytes [0..n], extra
			// padding byte
			int padding = 0;
			int minimumPadding = getMinimumPadding();
			...
			if (msm == MessageSecurityMode.SignAndEncrypt) {
				...
				try {
					...
					blockSize = CryptoUtil
							.getPlainTextBlockSize(profile.getSecurityPolicy().getAsymmetricEncryptionAlgorithm(), key);

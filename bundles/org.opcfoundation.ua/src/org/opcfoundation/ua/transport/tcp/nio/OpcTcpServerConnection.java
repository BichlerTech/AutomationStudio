...
public class OpcTcpServerConnection extends AbstractServerConnection {
	 ...
	 * establish a connection during the timeout, the socket is released. A smaller
	 * value might be usable to avoid DNS attacks.
	 *
	 * @param handshakeTimeout the handshakeTimeout to set. Default 600000 = 10
	 *                         minutes.
	 */
	...
	MonitorListener inputListener = new MonitorListener() {
		@Override
		public void onStateTransition(IStatefulObject<BufferMonitorState, ?> sender, BufferMonitorState oldState,
			...
			if (chunkSize > ctx.maxRecvChunkSize) {
				setError(new ServiceResultException(StatusCodes.Bad_TcpMessageTooLarge,
						"Chunk size (" + chunkSize + ") exceeded maximum (" + ctx.maxRecvChunkSize + ")"));
				if (secureMessageBuilder != null)
					secureMessageBuilder.close();
				return;
			}
	 ...
	 * @param endpointServer EndpointServer for opc.tcp
	 * @param s              socket
	 * @param rh,            if null, assumes normal connection, if non-null assumes
	 *                       ReverseHello connection.
	 */
	OpcTcpServerConnection(OpcTcpServer endpointServer, AsyncSocketImpl s, final ReverseHello rh) {
		...
		// Monitor the state of the socket, make changes reflect to the state of the
		// UATcpConnection
		socketListener = new StateListener<SocketState>() {
			@Override
			public void onStateTransition(IStatefulObject<SocketState, ?> monitor, SocketState oldState,
					SocketState newState) {
				if (newState == SocketState.Error) {
					setError(StackUtils
							.toServiceResultException(OpcTcpServerConnection.this.s.getStateMonitor().getError()));
				}
		...
		if (rh == null) {
			timeoutTimer = TimerUtil.schedule(timer, timeout, StackUtils.getBlockingWorkExecutor(),
					System.currentTimeMillis() + handshakeTimeout);
		}
		if (rh != null) {
			s.getStateMonitor().addStateListener(new StateListener<SocketState>() {
				@Override
				public void onStateTransition(IStatefulObject<SocketState, ?> sender, SocketState oldState,
						SocketState newState) {
	 ...
	 * @return a {@link org.opcfoundation.ua.utils.asyncsocket.BufferMonitorState}
	 *         object.
	...
	protected CertificateValidator getRemoteCertificateValidator() {
		return binding == null ? null
				: binding.serviceServer.getApplication().getOpctcpSettings().getCertificateValidator();
	}
	 ...
	 * </p>
	 *
	 * @param a a {@link org.opcfoundation.ua.transport.tcp.impl.Acknowledge}
	 *          object.
		...
		if (secureMessageBuilder == null) {
			...
			if (securityPolicy == null) {
				...
				throw new ServiceResultException(Bad_SecurityPolicyRejected,
						"Security policy \"" + securityPolicyUri + "\" is not supported by the stack");
			}

			// if (!binding.endpointAddress.supportsSecurityPolicy(securityPolicy))
			// {
			// logger.warn("Security policy \""+securityPolicyUri+"\" is not supported by
			// the endpoint");
			// throw new ServiceResultException(Bad_SecurityPolicyRejected, "Security policy
			// \""+securityPolicyUri+"\" is not supported by the endpoint");
			// }
			...
			KeyPair localCertificate = binding.serviceServer.getApplication()
					.getApplicationInstanceCertificate(encodedLocalCertificateThumbprint);
			...
			if (localCertificate == null && securityPolicy != SecurityPolicy.NONE) {
				...
				throw new ServiceResultException(Bad_SecurityChecksFailed,
						"Requested Application Instance Certificate is not found in the server");
			}

			// Decode remote certificate
			org.opcfoundation.ua.transport.security.Cert remoteCertificate;
			try {
				remoteCertificate = encodedRemoteCertificate == null ? null
						: new org.opcfoundation.ua.transport.security.Cert(
			...
			// MessageMode is unknown at this time. It is fixed in
			// UATcpServerSecureChannel.onOpenChannel
			MessageSecurityMode msm = securityPolicy == SecurityPolicy.NONE ? MessageSecurityMode.None
			...
			secureMessageBuilder = new SecureInputMessageBuilder(securityConfiguration, messageListener, ctx,
					encoderCtx, recvSequenceNumber);
		}
	...
	protected void handleChunk(ByteBuffer chunk) throws ServiceResultException {
		...
		} else if (messageType == TcpMessageType.HELLO || messageType == TcpMessageType.ACKNOWLEDGE
				|| messageType == TcpMessageType.ERROR) {
	 ...
	 * </p>
	 *
	 * @param mb a {@link org.opcfoundation.ua.transport.tcp.nio.InputMessage}
	 *           object.
	 * @throws org.opcfoundation.ua.common.ServiceResultException if any.
	 */
	 ...
	 * </p>
	 *
	 * @param e a {@link org.opcfoundation.ua.transport.tcp.impl.ErrorMessage}
	 *          object.
	 */
	...
	protected void handleHelloMessage(Hello h) throws ServiceResultException {
		...
		// Makes sense to check isDebugEnabled because Arrays.toString method probably
		// takes longer than this check.
		...
		if (binding != null) {
			...
			ctx.maxSendMessageSize = ctx.maxSendMessageSize == 0 ? maxMsgSize
					: Math.min(ctx.maxSendMessageSize, maxMsgSize);
			ctx.maxRecvMessageSize = ctx.maxRecvMessageSize == 0 ? maxMsgSize
					: Math.min(ctx.maxRecvMessageSize, maxMsgSize);
	...
	protected void handleOpenSecureChannelRequest(InputMessage mb) throws ServiceResultException {
		...
		if (req.getRequestType() == SecurityTokenRequestType.Issue) {
			OpcTcpServerSecureChannel channel = new OpcTcpServerSecureChannel(this,
					endpointServer.secureChannelCounter.incrementAndGet());
			logger.debug("handleOpenSecureChannelRequest: endpointServer={} SecureChannelId={}", endpointServer,
			...
			if (!ObjectUtils.objectEquals(req.getRequestType(), SecurityTokenRequestType.Renew))
				throw new ServiceResultException(Bad_UnexpectedError);
	...
	protected void handleSecureMessage(InputMessage mb) throws ServiceResultException {
		...
		int secureChannelId = mb.getSecureChannelId();
		if (logger.isDebugEnabled())
			logger.debug("handleSecureMessage: secureChannelId=" + secureChannelId + "msg="
	...
	protected void handleSymmChunk(ByteBuffer chunk) throws ServiceResultException {
		...
		if (secureMessageBuilder != null && !secureMessageBuilder.moreChunksRequired())
			secureMessageBuilder = null;
		if (secureMessageBuilder == null) {
			secureMessageBuilder = new SecureInputMessageBuilder(token/* channel */, messageListener, ctx, encoderCtx,
					channel.recvSequenceNumber);
	 ...
	 * @throws org.opcfoundation.ua.common.ServiceResultException if any.
	 */
	protected void sendAcknowledge(Acknowledge a) throws ServiceResultException {
	 ...
	 */
	@Override
	protected int sendAsymmSecureMessage(final AsyncWrite msg, final SecurityConfiguration securityConfiguration,
			int secureChannelId, int requestNumber, AtomicInteger sendSequenceNumber) throws ServiceResultException {
		synchronized (msg) {
			if (msg.isCanceled())
				return -1;
			msg.setQueued();
	 ...
	 * @throws org.opcfoundation.ua.common.ServiceResultException if any.
	 */
	protected void sendError(ErrorMessage e) throws ServiceResultException {
	 ...
	 */
	@Override
	protected void sendSecureMessage(final AsyncWrite msg, final SecurityToken token, final int requestId,
			final int messageType, final AtomicInteger sendSequenceNumber) {
		assert (token != null);
		...
		try {
			...
			// Probably more efficient to check isTraceEnabled before executing
			// ObjectUtils.printFieldsDeep
			...
			ChunkFactory cf = new ChunkFactory(optimalChunkSize, 8, 8, 8, signatureSize, cipherBlockSize, mode,
					keySize);
	...
	/**
	 * Put chunks into send queue. Chunks will be given a sequence number but will
	 * be flushed in endChunkSend().
	 *
	 * @param chunks a {@link java.nio.ByteBuffer} object.
	 */

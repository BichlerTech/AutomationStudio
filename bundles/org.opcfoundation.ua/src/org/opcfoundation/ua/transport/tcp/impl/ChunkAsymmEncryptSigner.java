...
public class ChunkAsymmEncryptSigner implements Runnable {
	 ...
	 * settings are logged with DEBUG level. Unexpected errors are logged with ERROR
	 * level.
	 */
	...
	public void run() throws RuntimeServiceResultException {
		try {
			...
			signatureSize = msm.hasSigning()
					? CryptoUtil.getSignatureSize(signatureAlgorithm, profile.getLocalPrivateKey())
					: 0;
			...
			// Padding size includes the total padding: PaddingSize and ExtraPaddingSize
			// bytes + padding
			int paddingSize = 0;

			// Encrypting
			if (msm == MessageSecurityMode.SignAndEncrypt) {
				// Because of the encypting, add the minimal +1 or +2 padding bytes to the total
				// size
			...
			if (msm == MessageSecurityMode.SignAndEncrypt) {
				...
				// There is sequenceHeader (size = 8) before the body and we want to encrypt
				// that too
				...
				encrypt(bytesToEncrypt, profile.getRemoteCertificate().getPublicKey(), chunk.array(),
	...
	/**
	 * Returns the number of bytes in the padding including paddingSize and possible
	 * extraPaddingByte.
	 * 
	 * @param keySize
	 * @return
	...
	private int getPaddingSize(int keySize) {
		...
		// isTraceEnabled checked because potentially time consuming CryptoUtil methods
		// get evaluated otherwise every time.
		if (logger.isTraceEnabled()) {
			logger.trace("getPaddingSize: chunk={}", CryptoUtil.toHex(chunk.array(), 64));
	...
	private void encrypt(byte[] dataToEncrypt, PublicKey publicKey, byte[] output, int outputOffset)
		...
		if (dataToEncrypt.length % inputBlockSize != 0) {
			logger.error("Wrong block size in asym encryption: length={} inputBlockSize={}", dataToEncrypt.length,
					inputBlockSize);
			throw new ServiceResultException(StatusCodes.Bad_InternalError,
					"Error in asymmetric encrypt: Input data is not an even number of encryption blocks.");
		}

		CryptoProvider cryptoProvider = CryptoUtil.getCryptoProvider();
		cryptoProvider.encryptAsymm(publicKey, profile.getSecurityPolicy().getAsymmetricEncryptionAlgorithm(),
	...
	private byte[] sign(byte[] dataToSign, RSAPrivateKey senderPrivate) throws ServiceResultException {
		...
		byte[] signature = CryptoUtil.getCryptoProvider().signAsymm(senderPrivate,
				profile.getSecurityPolicy().getAsymmetricSignatureAlgorithm(), dataToSign);

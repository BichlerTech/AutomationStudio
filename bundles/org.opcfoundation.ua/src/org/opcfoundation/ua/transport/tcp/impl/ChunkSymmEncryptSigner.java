...
//			count += policy.getSymmetricSignatureSize();
			...
			if (msm == MessageSecurityMode.Sign || msm == MessageSecurityMode.SignAndEncrypt) {
				...
				// isTraceEnabled checked because potentially time consuming CryptoUtil method
				// gets evaluated otherwise every time.
...
//			count += messageHeaderSize + securityHeader;
	...
	private int encrypt(SecurityToken token, byte[] dataToEncrypt, int inputOffset, int inputLength, byte[] output,
		...
		return CryptoUtil.getCryptoProvider().encryptSymm(token.getSecurityPolicy(), token.getLocalEncryptingKey(),
				token.getLocalInitializationVector(), dataToEncrypt, inputOffset, inputLength, output, outputOffset);
	}

	private void sign(SecurityToken token, byte[] input, int verifyLen, byte[] output) throws ServiceResultException {
		CryptoUtil.getCryptoProvider().signSymm(token.getSecurityPolicy(), token.getLocalSigningKey(), input, 0,
				verifyLen, output, 0);
	}

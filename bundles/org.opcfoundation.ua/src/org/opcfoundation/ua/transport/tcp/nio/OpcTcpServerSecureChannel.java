...
public class OpcTcpServerSecureChannel extends AbstractServerSecureChannel {
	 ...
	 *
	 * @param connection      a
	 *                        {@link org.opcfoundation.ua.transport.tcp.nio.OpcTcpServerConnection}
	 *                        object.
	...
	protected void handleSecureMessage(InputMessage mb, IEncodeable msg) throws ServiceResultException {
		...
		PendingRequest req = new PendingRequest(this, getEndpoint(), getServer(), mb.getRequestId(),
				(ServiceRequest) msg);
	...
	private SecurityToken createToken(OpenSecureChannelRequest req, InputMessage mb) throws ServiceResultException {
		...
		ByteString serverNonce = CryptoUtil
				.createNonce(securityConfiguration.getSecurityPolicy().getSecureChannelNonceLength());
		...
		SecurityToken token = new SecurityToken(securityConfiguration, getSecureChannelId(), tokenId,
				System.currentTimeMillis(), tokenLifetime.longValue(), serverNonce, clientNonce);
		tokens.put(tokenId, token);

		return token;
	}

	private void sendOpenChannelResponse(InputMessage mb, SecurityToken token,
			SecurityConfiguration securityConfiguration) throws ServiceResultException {
		...
		if (isAsync) {
			connection.sendAsymmSecureMessage(msgToWrite, securityConfiguration, token.getSecureChannelId(),
					mb.getRequestId(), sendSequenceNumber);
		} else {
			connection.sendSecureMessage(msgToWrite, activeToken, mb.getRequestId(), TcpMessageType.MESSAGE,
					sendSequenceNumber);
		}
	 ...
	 * @param mb  a {@link org.opcfoundation.ua.transport.tcp.nio.InputMessage}
	 *            object.
	 * @param req a {@link org.opcfoundation.ua.core.OpenSecureChannelRequest}
	 *            object.
	 * @throws org.opcfoundation.ua.common.ServiceResultException if any.
	 */
...
//		}
		...
		securityConfiguration = new SecurityConfiguration(securityMode, localCertificate, remoteCertificate);
	...
	protected void handleRenewSecureChannelRequest(InputMessage mb, OpenSecureChannelRequest req)
		...
		/*
		 * // Untested code, therefore commented out // if ( !getState().isOpen() ) {
		 * msgExchange.sendException( new ServiceResultException(
		 * StatusCodes.Bad_SecureChannelClosed,
	 ...
	 * @param mb  a {@link org.opcfoundation.ua.transport.tcp.nio.InputMessage}
	 *            object.
	 * @param req a {@link org.opcfoundation.ua.core.CloseSecureChannelRequest}
	 *            object.
	 */
	protected void handleCloseSecureChannelRequest(InputMessage mb, CloseSecureChannelRequest req) {
		...
		connection.sendSecureMessage(msg, getActiveSecurityToken(), mb.getRequestId(), TcpMessageType.CLOSE,
				sendSequenceNumber);
	}
	...
	public boolean needsCertificate() {
		return getMessageSecurityMode().hasSigning()
				|| EndpointUtil.containsSecureUserTokenPolicy(getServer().getUserTokenPolicies());
	}

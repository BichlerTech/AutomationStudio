 ...
 * <p>
 * Message is decoded and chunks are deciphered and validated in background
 * threads. Deciphering is executed in StackUtils.getNonBlockerExecutor() which
 * has one thread for each CPU core. Decoding is executed in
 * StackUtils.getBlockerExecutor() which creates new threads as needed.
 */
public class SecureInputMessageBuilder implements InputMessage {
	...
	/**
	 * Token, {@link SecurityToken} if symmetric, {@link SecurityConfiguration} if
...
//	OrderedByteBufferInputStream chunkSink;
	...
	public interface MessageListener {
		/**
		 * On message completed or error occured. Use {@link InputMessage#getMessage()}
		 * to get the message, if null get the error with
		 * {@link InputMessage#getError()}.
		 * 
		 * @param sender
		 */
	...
	/**
	 * Create message builder. Message builder compiles inbound chunks into a
	 * message.
	 *
	 * @param token                  {@link SecurityToken} (symm) or
	 *                               {@link SecurityConfiguration} (asymm)
	 * @param listener               a
	 *                               {@link org.opcfoundation.ua.transport.tcp.nio.SecureInputMessageBuilder.MessageListener}
	 *                               object.
	 * @param ctx                    a
	 *                               {@link org.opcfoundation.ua.transport.tcp.impl.TcpConnectionParameters}
	 *                               object.
	 * @param expectedSequenceNumber a
	 *                               {@link java.util.concurrent.atomic.AtomicInteger}
	 *                               object.
	 * @param encoderCtx             a
	 *                               {@link org.opcfoundation.ua.encoding.EncoderContext}
	 *                               object.
	 */
	public SecureInputMessageBuilder(Object token, MessageListener listener, TcpConnectionParameters ctx,
			EncoderContext encoderCtx, AtomicInteger expectedSequenceNumber) {
		assert (token != null);
		...
		// New bytes become available as chunks are added by handleChunkRuns (see
		// addChunk()).
...
//		chunkSink = new OrderedByteBufferInputStream();
		...
		messageDecoderRun = new Runnable() {
			public void run() {
				try {
					...
					// Payloads of high nesting levels may cause stack overflow. Structure,
					// VariantArray and DiagnosticInfo at least may cause this.
					// JVM setting -Xss influences possible level of nesting. At least 100 levels of
					// nesting must be supported, this should not be a problem with normal thread
					// stack sizes.
					// Inform receiving side that error has happened.
					setError(new ServiceResultException(StatusCodes.Bad_DecodingError,
							"Stack overflow: " + Arrays.toString(Arrays.copyOf(e1.getStackTrace(), 30)) + "..."));
				}
			}
		};
	}
	...
	public synchronized void addChunk(final ByteBuffer chunk) throws ServiceResultException {
		...
		if (chunkType == TcpMessageType.FINAL)
			acceptsChunks = false;
		final Integer expectedSequenceNumber = this.expectedSequenceNumber != null
				? this.expectedSequenceNumber.getAndIncrement()
				: null;
			...
			public void run() {
				...
				try {
					...
					if (token instanceof SecurityToken)
						...
						ChunkAsymmDecryptVerifier asdf = new ChunkAsymmDecryptVerifier(chunk,
								(SecurityConfiguration) token);
						asdf.run();
					...
					if (expectedSequenceNumber != null) {
						...
						if (expectedSequenceNumber != chunkSequenceNumber) {
							// log.warn("chunkSequenceNumber="+chunkSequenceNumber+",
							// expectedSequenceNumber="+expectedSequenceNumber);
							throw new ServiceResultException(StatusCodes.Bad_UnexpectedError, "chunkSequenceNumber="
									+ chunkSequenceNumber + ", expectedSequenceNumber=" + expectedSequenceNumber);
						}
					}
					...
					if (secureChannelId != SecureInputMessageBuilder.this.securityChannelId)
						throw new ServiceResultException(StatusCodes.Bad_UnexpectedError,
								"secureChannelId=" + secureChannelId + ", expected Id");
...
//					chunkSink.offer(chunkNumber, chunk);
	...
	protected void fireComplete() {
		if (listener != null)
			listener.onMessageComplete(this);
	}
	...
	/**
	 * <p>
	 * Getter for the field <code>securityPolicyUri</code>.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Getter for the field <code>senderCertificate</code>.
	 * </p>
	 *

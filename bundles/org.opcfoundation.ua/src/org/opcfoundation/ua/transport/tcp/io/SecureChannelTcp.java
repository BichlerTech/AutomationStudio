 ...
 *
 *
 * If the connection fails, and the transport channel is stateful (TCP), and the
 * secure channel has pending service requests, it attempts to reconnect the
 * transport channel. If the reconnect fails there is a timeout sequence of the
 * following wait periods { 0, 1, 2, 4, 8, 16, 32, 64, 120, 120, ... }.
 * <p>
 *
 * If error recovery state fails to re-establish new security token before the
 * old expires, the secure channel will be closed.
 *
 ...
 */
public class SecureChannelTcp implements IMessageListener, IConnectionListener, ITransportChannel,
	...
	/**
	 * The time when token was issued in time system of
	 * {@link System#currentTimeMillis()}.
	 */
	long tokenIssueTime;

	/**
	 * Value in milliseconds that indicates the token life time. The secure channel
	 * expires after 125% of the token life time has elapsed.
	 */
	long tokenLifetime;
	...
	// Use this interface with SOAP if possible, if not then restructure
	// SecureChannel class
	AtomicReference<IConnection> transportChannel = new AtomicReference<IConnection>(null);

	/**
	 * List on pending requests. All reads and writes are done by synchronizing to
	 * the requests object.
	 */
	Map<Integer, PendingRequest> requests = new ConcurrentHashMap<Integer, PendingRequest>();

	/**
	 * Pending request class. The result is written to error or response, after
	 * which semaphore s is released and its listener awakes.
	 */
	...
	/**
	 * reconnect index points to element in RECONNECT_WAIT_TIME. The index is
	 * reseted once a secure channel is opened succesfully.
	 */
	int errorRecoveryReconnectIndex = 0;

	/**
	 * Task attempts reconnection. The task is canceled if the secure channel is
	 * closed or secure channel establised manually.
	 */
	TimerTask errorRecoveryReconnectTimer;

	/**
	 * If true, the secure channel is in error recover mode. In recovery mode, there
	 * is a reconnect timer that attempts to reconnect the socket. Once connected,
	 * secure channel is re-established.
	 * 
	 * Error recover mode lasts, until connection is established and secure channel
	 * renewed, or until last security token has expired which means that the secure
	 * channel has been closed at the server.
	 */
	...
	/**
	 * Tolerance defines how much sooner the requests timeout compared to the
	 * TimeoutHint; 10% helps to ensure that the response will be delivered back to
	 * the client before it times out there
	 */
	private static final double TIMEOUT_TOLERANCE = 0.1;

	/**
	 * This task renews security token. The task is created after security token is
	 * created, and is canceled on close.
	 */
	TimerTask renewSecurityTokenTask;

	/**
	 * This task timeouts pending requests. The task is created upon async service
	 * request. "requests" is synchronized when timeoutPendingRequests is modified.
	 */
	...
	public void initialize(InetSocketAddress addr, TransportChannelSettings settings, EncoderContext ctx)
		...
		if (url == null || url.isEmpty() || UriUtil.SCHEME_OPCTCP.equals(UriUtil.getTransportProtocol(url))) {
			...
			throw new ServiceResultException(Bad_ServerUriInvalid,
					"The protocol is not supported by the this SecureChannelTcp");
		}
	 ...
	 * </p>
	 *
	 * @param request a {@link org.opcfoundation.ua.builtintypes.ServiceRequest}
	 *                object.
	 * @return a long.
	 */
	protected long getRequestTimeout(ServiceRequest request) {
		UnsignedInteger timeoutHint = request.getRequestHeader() != null ? request.getRequestHeader().getTimeoutHint()
				: null;
	 ...
	 *
	 * If the secure channel is in error recovery state, the request is put to a
	 * send queue. Message is sent upon successful reconnection.
	 * <p>
	 *
	 ...
	 *
	 * If the thread is interrupted with {@link Thread#interrupt()}, the operation
	 * aborts and ServiceResultException Bad_RequestCancelledByClient is thrown.
	 * <p>
	 */
	public ServiceResponse serviceRequest(ServiceRequest request) throws ServiceFaultException, ServiceResultException {
	...
	public ServiceResponse serviceRequest(ServiceRequest request, long operationTimeout)
		...
		// When the secure channel is renewed, it may be that the channel is temporarily
		// closed.
		...
		try {
			try {
				...
				logger.debug("serviceRequest: Message sent, requestId={} secureChannelId={}", req.requestId,
						secureChannelId);
				logger.trace("serviceRequest: message={}", request);
			} catch (ServiceResultException e) {
				// Check isDebugEnabled() here for possible performance reasons.
				if (logger.isDebugEnabled())
					logger.debug("serviceRequest: While sending requestId=" + req.requestId + ", secureChannelId="
							+ secureChannelId + ", message=" + request, e);

				// Put the message to queue
				if (e.getStatusCode().isStatusCode(Bad_CommunicationError)) {
					req.requestToBeSent = request;
					// Sends the result in another thread. This thread can continue with minimal
					// delay
			...
			if (request instanceof CloseSecureChannelRequest) {
				...
				if (operationTimeout == 0) {
					...
					res = (ServiceResponse) req.result.waitForResult(timeout, TimeUnit.MILLISECONDS);
				}
				if (logger.isTraceEnabled())
	 ...
	 * {@link StatusCodes#Bad_SecureChannelClosed} is thrown.
	 *
	 * Errors are written to the result object. ServiceFaultException There was a
	 * service fault in processing of the operation in the server.
	 * ServiceResultException There was an error while transferring the operation
	 * over network.
	 * <p>
	 */
...
//		req.timeoutTime = operationTimeout==0 ? Long.MAX_VALUE : req.startTime + operationTimeout;
		...
		// Sends the result in another thread. Current thread may continue with no
		// further delay
//		logger.debug("scheduling async request to another thread: "+req.requestId);
	...
	/**
	 * Sets new Timer Task that timeouts pending requests. If task already exists
	 * but is too far in the future, it is canceled and new task assigned
	 */
	...
	/**
	 * This runnable goes thru pending requests and sets Bad_Timeout error code to
	 * all requests that have timeouted.
	 */
		...
		public void run() {
			...
			for (PendingRequest req : requests.values()) {
				if (currentTime >= req.timeoutTime) {
					logger.warn("Request id={} timeouted {}ms elapsed. timeout at {}ms", req.requestId,
							(System.currentTimeMillis() - req.startTime), (req.timeoutTime - req.startTime));
	...
	/**
	 * Opens a secure channel. This method does nothing if the secure channel is
	 * already open.
	 *
	 * Sets up a connection, opens it, creates a secure channel. If unable to open
	 * connection an exception is thrown and the secure channel remains closed.
	 *
	 ...
	 */
	private void createSecureChannel(boolean renew) throws ServiceResultException {
		...
		IConnectionListener cloListener = new IConnectionListener() {
			public void onClosed(ServiceResultException closeError) {
				if (closeError == null)
					closeError = new ServiceResultException(Bad_CommunicationError, "Connection Closed");
				errors[0] = closeError;
				s.release(Integer.MAX_VALUE);
			}
		...
		try {
			...
			if (res instanceof OpenSecureChannelResponse == false) {
				throw new ServiceResultException(Bad_UnexpectedError,
						"Unexpected result " + res.getClass().getName() + " OpenSecureChannelResponse expected");
			}
			...
			if (renew) {
				logger.debug("{} Secure channel renewed, SecureChannelId={}, TokenId={}", this.secureChannelId,
						this.secureChannelId, token.getTokenId().longValue());
			} else {
				logger.debug("{} Secure channel opened, SecureChannelId={}, TokenId={}", this.secureChannelId,
						this.secureChannelId, token.getTokenId().longValue());
			}

			// HAX! In Reconnect to secure channel -situation, the C# Server implementation
			// sends
			// two conflicting secure channel id's.
			// The old channel (correct) in message header and a new channel id in the
			// message body.
			//
			if (renew)
				this.secureChannelId = chanId;
	...
	/**
	 * Send all pending request messages. Requests messages are gathered in requests
	 * map.
	 * 
	 */
	private void sendPendingRequestMessages() throws ServiceResultException {
		...
		for (;;) {
			...
			if (currentTime > req.timeoutTime) {
				logger.debug("Request id={} timeouted {}ms elapsed. timeout at {} ms", req.requestId, elapsedTime,
						(req.timeoutTime - req.startTime));
				requests.remove(req.requestId);
				req.result.setError(new ServiceResultException(Bad_Timeout));
				continue;
			}
	...
	/**
	 * Close the secure channel. This method does nothing if the channel is already
	 * closed or has never been opened.
	 * <p>
	 *
	 * This method sends CloseSecureChannelRequest to the server and closes the
	 * socket connection. If sending of the message fails and thus the servers never
	 * receives notification about closed secure channel, then there is no resend
	 * attempt, instead the secure channel will eventually time out in the server.
	 * <p>
	 *
	...
	public void close() {
		...
		// Cancel all pending requests
		{
			...
			// TODO: Is this thread safe? Does it have to be? Should requests be a
			// BlockingQueue?
...
//			else
	...
	/**
	 * <p>
	 * Setter for the field <code>transportChannel</code>.
	 * </p>
	 *
	...
	public int getOperationTimeout() {
		Integer i = null;
		if (settings != null && settings.getConfiguration() != null)
			i = settings.getConfiguration().getOperationTimeout();
		return i == null ? 0 : i;
	}
	 ...
	 * {@inheritDoc}
	 *
	 * Implementation to IMessageListener. Listens to messages incoming from
	 * TcpConnection.
	 */
	@Override
	public void onMessage(int requestId, int secureChannelId, IEncodeable message) {
		if (secureChannelId != this.secureChannelId)
			return;
		...
		if (req == null) {
			if (message instanceof OpenSecureChannelResponse == false) {
				{
					ServiceFault serviceFault = (message instanceof ServiceFault) ? (ServiceFault) message : null;
					if (serviceFault != null && serviceFault.getResponseHeader().getServiceResult()
							.equals(StatusCodes.Bad_TooManyPublishRequests))
						logger.info("ServiceFault={}", serviceFault);
					else {
						logger.warn("{} Unidentified message, RequestId={}, type={}!", secureChannelId, requestId,
	...
	/**
	 * Return true if the secure channel has been opened and is not (hopefully)
	 * closed on the server.
	 *
	 * Secure channel is open as long as it as security token that is alive, even if
	 * its transport layer connection is disconnected.
	 *
	 * @return a boolean.
	 */
	...
	public void onClosed(ServiceResultException closeError) {
		...
		if (errorCode != null && errorCode.isStatusCode(StatusCodes.Bad_ConnectionClosed)) {
			...
			if (closeError == null)
				closeError = new ServiceResultException(StatusCodes.Bad_UnexpectedError);
			// Cancel requests
	...
	Runnable reconnectRunnable = new Runnable() {
		public void run() {
			...
			try {
				...
				// C# server sends Bad_TcpSecureChannelUnknown when it has closed the secure
				// channel
				...
				synchronized (errorRecoveryLock) {
					...
					long currentTime = System.currentTimeMillis();
					long waitTime = errorRecoveryReconnectIndex >= RECONNECT_WAIT_TIME.length
							? RECONNECT_WAIT_TIME[RECONNECT_WAIT_TIME.length - 1] * 1000
							: RECONNECT_WAIT_TIME[errorRecoveryReconnectIndex] * 1000;
					long expireTime = ((long) (tokenLifetime * 1.25)) + tokenIssueTime;
					...
					errorRecoveryReconnectTimer = TimerUtil.schedule(timer, reconnectRunnable, executor,
	...
	private void setErrorRecoveryState(boolean newState) {
		// Cancel recovery mode if enabled
		synchronized (errorRecoveryLock) {
			if (errorRecoveryState == newState)
				return;
			if (newState) {
				...
				long currentTime = System.currentTimeMillis();
				errorRecoveryReconnectTimer = TimerUtil.schedule(timer, reconnectRunnable, executor,
						currentTime + RECONNECT_WAIT_TIME[0]);
			} else {
	...
	public EnumSet<TransportChannelFeature> getSupportedFeatures() {
		return EnumSet.of(TransportChannelFeature.open, TransportChannelFeature.openAsync,
				TransportChannelFeature.close, TransportChannelFeature.closeAync, TransportChannelFeature.sendRequest,

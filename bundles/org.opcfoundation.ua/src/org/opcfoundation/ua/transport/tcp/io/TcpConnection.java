 ...
 * <p>
 *
 * Features included in this class: o Establishing connection, Handshake o Sync
 * and Async encryption Features excluded in this class: o Reconnect (See
 * SecureChannel) o Token Renewal (See SecureChannel)
 *
...
public class TcpConnection implements IConnection {
	...
	/**
	 * Log4J Error logger. Security settings are logged with DEBUG level. Unexpected
	...
	/**
	 * This map captures that last used TokenId of the input channel for a secure
	 * channel Map<SecureChannelId, TokenId>
	 */
	final Map<Integer, SecurityToken> activeTokenIdMap = new ConcurrentHashMap<Integer, SecurityToken>();

	/**
	 * Map<RequestId, ClientNonce> Capture of client nonces. Nonce is captured in
	 * sendRequest( OpenSecureMessageRequest ) and released in Readthread
	 * OpenSecureMessageResponse. The captured nonce is used in the instantiation of
	 * a SecurityToken.
	 */
	...
	/**
	 * This variable contains the agreed protocol version, the version sent by the
	 * server in hand-shake
	 */
	int protocolVersion;
	 ...
	 * channel. Message encrypting is done under the lock. This is because sequence
	 * number is encrypted and messages may not be sent in wrong order.
	 */
	ReentrantLock lock = new ReentrantLock();

	/**
	 * Read thread is instantiated and active while socket connection is
	 * established. The read thread shuts down itself automatically when there is
	 * socket exception while reading the input stream.
	 * 
	...
	/**
	 * IConnectionListener listeners. All close events (from the user and the stack)
	 * are notified to the listeners
	 */
	List<IConnectionListener> connectionListeners = new CopyOnWriteArrayList<IConnectionListener>();

	/**
	 * Possible ReverseHello configuration for the client side. If null, then
	 * operating in non-reverse mode.
	 */
	...
	/**
	 * <p>
	 * Getter for the field <code>receiveBufferSize</code>.
	 * </p>
	 *
	 ...
	 * define the value before the client is connected. The value is applied to the
	 * underlying socket, right the socket is created.
	 * <p>
	...
	/**
	 * Set the default timeout (in milliseconds) for making the initial connection
	 * handshake. Default value is 60000. It should be noted that the actual value
	 * used by the Operating System can be lower, usually the max timeout is around
	 * 20 seconds.
	 */
	 ...
	 * NOTE! This is used to timeout {@link ServerSocket#accept()}. The timeout for
	 * receiving a ReverseHello from the Server after a connection is accepted is
	 * determined by {@link #getDefaultHandshakeTimeout()}. Parameter is in
	 * milliseconds. Default value is 0, meaning infinite wait.
	 */
	...
	/**
	 * <p>
	 * Getter for the field <code>sendBufferSize</code>.
	 * </p>
	 *
	...
	/**
	 * Define the sendBufferSize to use for the connection socket. You must define
	 * the value before the client is connected. The value is applied to the
	 * underlying socket, right the socket is created.
	 * <p>
	 * Default value: 0, which omits the parameter and the default value for the
	 * socket (depending on the operating system) is used.
	 ...
	 *
	 * @param settings a
	 *                 {@link org.opcfoundation.ua.transport.TransportChannelSettings}
	 *                 object.
	 * @param ctx      a {@link org.opcfoundation.ua.encoding.EncoderContext}
	 *                 object.
	...
	public void initialize(InetSocketAddress addr, TransportChannelSettings settings, EncoderContext ctx)
		...
		try {
			...
			SecurityPolicy securityPolicy = SecurityPolicy
					.getSecurityPolicy(endpointDescription.getSecurityPolicyUri());
		...
		try {
			...
			if (!isReverse) {
				...
				try {
					...
					try {
						...
						throw new ServiceResultException(StatusCodes.Bad_UnexpectedError, e,
								"ServerSocket.accept failed (or was closed, possibly due to a timeout)");
					}
					// Disable Nagle's algorithm
					s.setTcpNoDelay(true);
					if (receiveBufferSize > 0) {
			...
			try {
				...
				if (isReverse) {
					...
					// 1.04 Part 6 section 7.1.2.6, if length exceeds 4096, must return
					// Bad_TcpEndpointUrlInvalid
					...
					if (rh.getServerUri() == null || rh.getServerUri().length() > 4096
						...
						logger.error(
								"ReverseHello did not contain correct ServerUri, or is too long, expected: {}, got:{}",
								reverseHelloServerUri, rh.getServerUri());
					...
					if (rh.getEndpointUrl() == null || rh.getEndpointUrl().length() > 4096
						...
						logger.error(
								"ReverseHello did not contain correct EndpointUrl, or is too long, expected: {}, got:{}",
								endpointDescription.getEndpointUrl(), rh.getEndpointUrl());
						throw new ServiceResultException(StatusCodes.Bad_TcpEndpointUrlInvalid);
					}
				...
				h.setMaxChunkCount(UnsignedInteger.valueOf(
						endpointConfiguration.getMaxBufferSize() == null ? TcpMessageLimits.DefaultMaxBufferSize
								: endpointConfiguration.getMaxBufferSize().intValue()));
...
//					logger.warn("retry");
				...
				if (msgType != TcpMessageType.ACKF) {
					throw new ServiceResultException(Bad_TcpMessageTypeInvalid, "Message type was " + msgType
							+ ", expected " + (TcpMessageType.ACKNOWLEDGE | TcpMessageType.FINAL));
				}

				// ACKF
				Acknowledge ack = dec.getEncodeable(null, Acknowledge.class);
				if (ack.getProtocolVersion().intValue() < h.getProtocolVersion().intValue())
					throw new ServiceResultException(Bad_ProtocolVersionUnsupported, "Version "
							+ h.getProtocolVersion().intValue() + " requested, got " + ack.getProtocolVersion());
				...
				if (ack.getReceiveBufferSize().longValue() < TcpMessageLimits.MinBufferSize)
					throw new ServiceResultException(Bad_TcpInternalError,
							"Server recv buffer size < " + TcpMessageLimits.MinBufferSize);
				...
				if (ack.getSendBufferSize().longValue() < TcpMessageLimits.MinBufferSize)
					throw new ServiceResultException(Bad_TcpInternalError,
							"Server send buffer size < " + TcpMessageLimits.MinBufferSize);

				limits = new TcpConnectionLimits();
				limits.maxSendBufferSize = (int) Math.min(ack.getSendBufferSize().longValue(),
						Long.valueOf((long) Integer.MAX_VALUE));
				limits.maxRecvBufferSize = (int) Math.min(ack.getReceiveBufferSize().longValue(),
						Long.valueOf((long) Integer.MAX_VALUE));
				limits.maxSendChunkCount = (int) Math.min(ack.getMaxChunkCount().longValue(),
						Long.valueOf((long) Integer.MAX_VALUE));
				limits.maxRecvChunkCount = (int) Math.min(h.getMaxChunkCount().longValue(),
						Long.valueOf((long) Integer.MAX_VALUE));
				limits.maxSendMessageSize = (int) Math.min(ack.getMaxMessageSize().longValue(),
						Long.valueOf((long) Integer.MAX_VALUE));
				limits.maxRecvMessageSize = (int) Math.min(h.getMaxMessageSize().longValue(),
						Long.valueOf((long) Integer.MAX_VALUE));
				...
				s.setKeepAlive(true);
				if (isReverse) {
					logger.info("Connected (reverse), handshake completed, local={}, remote={}",
							s.getLocalSocketAddress(), s.getRemoteSocketAddress());
				} else {
					logger.info("Connected (non-reverse), handshake completed, local={}, remote={}",
							s.getLocalSocketAddress(), s.getRemoteSocketAddress());
				}
	...
	private void initEncoderContextLimits() {
		int maxMessageSize = Math
				.min(endpointConfiguration.getMaxMessageSize() != null ? endpointConfiguration.getMaxMessageSize()
						: Integer.MAX_VALUE, quotas.maxMessageSize);

		ctx.setMaxMessageSize(maxMessageSize);
		ctx.setMaxArrayLength(
				endpointConfiguration.getMaxArrayLength() != null ? endpointConfiguration.getMaxArrayLength() : 0);
		ctx.setMaxStringLength(
				endpointConfiguration.getMaxStringLength() != null ? endpointConfiguration.getMaxStringLength() : 0);
		ctx.setMaxByteStringLength(
				endpointConfiguration.getMaxByteStringLength() != null ? endpointConfiguration.getMaxByteStringLength()
						: 0);
	}

	/**
	 * Reads ReverseHello from teh given stream using the given decoder. Throws if
	 * fails.
	 */
	private ReverseHello readReverseHello(InputStreamReadable in, BinaryDecoder dec)
		...
		if (msgType != TcpMessageType.RHEF) {
			...
			throw new ServiceResultException(Bad_TcpMessageTypeInvalid, "Message type was " + msgType + ", expected "
					+ (TcpMessageType.REVERSE_HELLO | TcpMessageType.FINAL));
		}
...
//			tokens.clear();			
	...
	/**
	 * ReadThread is a thread that does blocking read to the input stream. If errors
	 * occur in the input stream, the error is logged and the socket closed. It
	 * deciphers and deserializes incoming messages and fires events to
	 * TcpConnection.
	 * 
	 */
	class ReadThread extends Thread {
		...
		/**
		 * Indicator set to true to signal the thread that the socket has been closed in
		 * a controlled manner.
		 */
				...
				read: while (this.s == TcpConnection.this.getSocket()) {
					...
					do {
						if (chunkNumber > limits.maxRecvChunkCount) {
							closeError = new ServiceResultException(
									"Recv chunk count exceeded (max = " + chunkNumber + ")");
						...
						if (chunkMessageType != TcpMessageType.OPEN && chunkMessageType != TcpMessageType.MESSAGE
								&& chunkType != TcpMessageType.ERRF) {
							closeError = new ServiceResultException(
									"Error, unknown message type " + String.format("0x%08x", chunkType));
						...
						if (size > limits.maxRecvBufferSize) {
							closeError = new ServiceResultException(
									"Error, chunk too large (max = " + limits.maxRecvBufferSize + ")");
						...
						if (messageType == TcpMessageType.OPEN) {
							try {
								...
								if (securityPolicy != securityConfiguration.getSecurityPolicy()) {
									closeError = new ServiceResultException(
											"Error, unexpected security policy in OpenSecureChannelResponse");
									logger.warn("{} Error, unexpected security policy in OpenSecureChannelResponse",
											addr);
									break read;
								}

								if (securityConfiguration.getSecurityPolicy() != SecurityPolicy.NONE
										&& !Arrays.equals(encodedLocalCertificateThumbprint,
												securityConfiguration.getEncodedLocalCertificateThumbprint())) {
								...
								if (encodedRemoteCertificate != null && encodedRemoteCertificate.length > 0)
									try {
										remoteCertificate = new Cert(
												CertificateUtils.decodeX509Certificate(encodedRemoteCertificate));
									} catch (CertificateException e) {
										closeError = new ServiceResultException(Bad_CertificateInvalid,
								...
								if (certificateValidator != null) {
									...
									if (code != null && !code.isGood()) {
										closeError = new ServiceResultException(code,
												"Remote certificate not accepted");
								...
								securityConfiguration = new SecurityConfiguration(
										...
										securityConfiguration.getLocalCertificate2(), remoteCertificate);

								ChunkAsymmDecryptVerifier processor = new ChunkAsymmDecryptVerifier(chunk,
						...
						if (messageType == TcpMessageType.MESSAGE) {
							...
							if (token == null) {
								closeError = new ServiceResultException(
										"Unexpected securityTokenId = " + securityTokenId);
								logger.warn("{} Unexpected securityTokenId = {}", addr, securityTokenId);
								break read;
							}
							if (!token.isValid()) {
								closeError = new ServiceResultException(
										"SecurityToken " + securityTokenId + " has timeouted");
						...
						if ((messageType == TcpMessageType.MESSAGE) || (seq != null)) {
							if (!seq.testAndSetRecvSequencenumber(chunkSequenceNumber)) {
								...
								logger.warn("{} Sequence number mismatch: {} vs. {}", addr, seq.getRecvSequenceNumber(),
										chunkSequenceNumber);
								break read;
							}
						}
...
//						chunkCount++;
					...
					if (message instanceof OpenSecureChannelResponse) {
						...
						if (___secureChannelId != __secureChannelId)
							logger.warn("{} OpenSecureChannel, server sent two secureChannelIds {} and {} using {}",
									addr, __secureChannelId, ___secureChannelId, __secureChannelId);

						try {
							SecurityToken token = new SecurityToken(TcpConnection.this.securityConfiguration,
									__secureChannelId, tkn.getTokenId().intValue(), System.currentTimeMillis(),
									tkn.getRevisedLifetime().longValue(), clientNonce, serverNonce);
				...
				if (e instanceof SocketException) {
					if (!closing) {
						...
						closeError = new ServiceResultException(Bad_ConnectionClosed, e,
	...
	/**
	 * <p>
	 * Getter for the field <code>endpointDescription</code>.
	 * </p>
	 *
...
//		logger.warn("dispose: " + Arrays.toString(Thread.currentThread().getStackTrace())); // !!!
	...
	public void sendRequest(ServiceRequest request, int secureChannelId, int requestId) throws ServiceResultException {
		...
		try {
							...
							try {
								if (asymm) {
									...
									for (int i = 0; i < chunks.length; i++) {
										...
										sendAsymmChunk(secureChannelId, requestId, securityMode, chunks[i],
												plaintexts[i], finalChunk);
										plaintexts[i] = null;
										chunks[i] = null;
									}
			...
			logger.warn(
					String.format("sendRequest %s failed: socket=%s, asymm=%s", request.getClass().getName(), s, asymm),
					e);
			throw e;
		}
	}
	 ...
	 * @throws IOException
	 */
	private void sendSymmChunk(int requestId, SecurityToken token, SequenceNumber seq, ByteBuffer chunk,
			final ByteBuffer plaintext, int msgType) throws ServiceResultException, IOException {
		chunk.rewind();
	 ...
	 * @throws IOException
	 */
	private void sendAsymmChunk(int secureChannelId, int requestId, SecurityMode securityMode, ByteBuffer chunk,
			final ByteBuffer plaintext, boolean finalChunk) throws ServiceResultException, IOException {
		chunk.rewind();
	...
	private MessageBuffers encodeMessage(ChunkFactory cf, final int len, IEncodeable request)
			throws ServiceResultException {
		...
		if (ctx.maxMessageSize != 0 && len > ctx.maxMessageSize) {
			final EncodingException encodingException = new EncodingException(StatusCodes.Bad_EncodingLimitsExceeded,
					"MaxMessageSize " + ctx.maxMessageSize + " < " + len);
	 ...
	 * @throws ServiceResultException
	 */
	private ChunkFactory getChunkFactory(boolean asymm, SecurityMode securityMode, int keySize)
			throws ServiceResultException {
		...
		if (asymm) {
			...
			return new ChunkFactory(maxSendBufferSize, 8, 8, 8, signatureSize, cipherBlockSize, messageSecurityMode,
					keySize);
		}
	}

	/**
	 * Get security token to use. The last used security token is preferred if its
	 * still alive. If not the youngest token is returned.
	 * 
	 * @param secureChannelId
	 * @return security token
	 * @throws ServiceResultException Bad_CommunicationError if no suitable token is
	 *                                available
	 */
	private SecurityToken getSecurityTokenToUse(int secureChannelId) throws ServiceResultException {
		...
		for (SecurityToken t : tokens) {
			if ((t.getSecureChannelId() == secureChannelId)
					&& (token == null || token.getCreationTime() < t.getCreationTime()))
				token = t;
		}
	...
	/**
	 * <p>
	 * Getter for the field <code>handshakeTimeout</code>.
	 * </p>
	 *
	 * @return a int.
	 */
	...
	/**
	 * <p>
	 * Setter for the field <code>handshakeTimeout</code>.
	 * </p>
	 *
	...
	/**
	 * Add input stream message listener. All incoming messages are notified to the
	 * listener.
	 *

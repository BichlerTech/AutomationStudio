 ...
 * Extending this class via non-code-generating ways should be avoided.
 */
public abstract class AbstractSerializer implements IEncodeableSerializer {

	/**
	 * Fixes standard namespace identifiers to have the uri for mapping purposes. If
	 * given other namespace index identifiers, throw
	 * {@link IllegalArgumentException}. Additionally throw if given non-local ids.
	...
	private static ExpandedNodeId fixAndValidateId(ExpandedNodeId id) {
		...
		if (!id.isLocal()) {
			throw new IllegalArgumentException(
					"Only ExpandedNodeIds that refer the local server are allowed as parameters");
		}
		...
		if (id.getNamespaceIndex() == 0) {
			...
			return new ExpandedNodeId(NamespaceTable.OPCUA_NAMESPACE, id.getValue(), NamespaceTable.getInstance());
		}
	...
	/**
	 * Creates new {@link AbstractSerializer}. This will call
	 * {@link #AbstractSerializer(Class, ExpandedNodeId, ExpandedNodeId, ExpandedNodeId)}
	 * with type nodeId null.
	 * 
	...
	/**
	 * Creates new {@link AbstractSerializer}. This will call
	 * {@link #AbstractSerializer(Class, ExpandedNodeId, ExpandedNodeId, ExpandedNodeId, ExpandedNodeId)}
	 * with jsonId null.
	 * 
	 * @param clazz    Structure to serialize
	 * @param binaryId binary id for the serialization, shall not be null
	 ...
	 */
	public AbstractSerializer(Class<? extends IEncodeable> clazz, ExpandedNodeId binaryId, ExpandedNodeId xmlId,
			ExpandedNodeId nodeId, ExpandedNodeId jsonId) {
		if (clazz == null) {
	 ...
	 */
	public abstract void putEncodeable(IEncodeable encodeable, IEncoder encoder) throws EncodingException;
	 ...
	 */
	public abstract IEncodeable getEncodeable(IDecoder decoder) throws DecodingException;

	/**
	 * @deprecated see text of
	 *             {@link IEncodeableSerializer#calcEncodeable(Class, IEncodeable, IEncoder)}
	 */
	@Deprecated
	@Override
	...
	/** {@inheritDoc} */
	@Override
	public void putEncodeable(Class<? extends IEncodeable> clazz, IEncodeable encodeable, IEncoder encoder)
			throws EncodingException {
		if (!clazz.equals(this.clazz))
			throw new EncodingException("Cannot encode " + clazz);
	...
	public Class<? extends IEncodeable> getClass(ExpandedNodeId id) {
		return (id.equals(binaryId) || id.equals(xmlId) || id.equals(jsonId) || (nodeId != null && id.equals(nodeId)))
				? clazz
				: null;
	}

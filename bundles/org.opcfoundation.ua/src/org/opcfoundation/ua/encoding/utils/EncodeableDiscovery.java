...
public class EncodeableDiscovery {
	...
	/**
	 * Discover default encodeables. Encodeables are discovered by inspecting all
	 * fields from Identifiers class using reflection.
	 *
	...
	public static void discoverDefaultEncodeables(Map<NodeId, Class<IEncodeable>> map) {
		...
		for (Field f : clazz.getFields()) {
			...
			try {
				...
				if (!IEncodeable.class.isAssignableFrom(c))
					continue;
				for (Field cf : c.getFields()) {
					cf.setAccessible(true);
					if (!cf.getType().equals(NodeId.class))
						continue;
					NodeId nodeId;
					try {

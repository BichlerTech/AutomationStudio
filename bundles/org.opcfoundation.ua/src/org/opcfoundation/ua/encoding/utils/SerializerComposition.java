...
/**
 * A collection of serializers for Structure types. Used by the encoders and
 * decoders.
 */
	...
	public void addSerializer(IEncodeableSerializer serializer) {
		...
		for (Class<? extends IEncodeable> clazz : classes) {
			...
			try {
				...
				/*
				 * XXX Exceptions are not expected at this point, but since this depends on
				 * returning the type NodeId with null EncodeType (this is coded to
				 * AbstractSerializer) we catch just in case some serializer would throw NPE on
				 * null EncodeType.
				 */
			}
		}

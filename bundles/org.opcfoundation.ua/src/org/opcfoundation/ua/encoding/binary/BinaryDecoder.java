...
public class BinaryDecoder implements IDecoder {
	...
	static {
		...
		addKnownFinalClassScalarSerializer(Byte.class, new ScalarDecoder<Byte>() {
			@Override
			public Byte get(BinaryDecoder dec, String fieldName, Class<? extends Byte> clazz) throws DecodingException {
		...
		addKnownFinalClassScalarSerializer(UnsignedInteger.class, new ScalarDecoder<UnsignedInteger>() {
			@Override
			public UnsignedInteger get(BinaryDecoder dec, String fieldName, Class<? extends UnsignedInteger> clazz)
					throws DecodingException {
		...
		addKnownFinalClassScalarSerializer(Long.class, new ScalarDecoder<Long>() {
			@Override
			public Long get(BinaryDecoder dec, String fieldName, Class<? extends Long> clazz) throws DecodingException {
		...
		addKnownFinalClassScalarSerializer(UUID.class, new ScalarDecoder<UUID>() {
			@Override
			public UUID get(BinaryDecoder dec, String fieldName, Class<? extends UUID> clazz) throws DecodingException {
		...
		addKnownFinalClassScalarSerializer(ExpandedNodeId.class, new ScalarDecoder<ExpandedNodeId>() {
			@Override
			public ExpandedNodeId get(BinaryDecoder dec, String fieldName, Class<? extends ExpandedNodeId> clazz)
					throws DecodingException {
		...
		addKnownFinalClassScalarSerializer(QualifiedName.class, new ScalarDecoder<QualifiedName>() {
			@Override
			public QualifiedName get(BinaryDecoder dec, String fieldName, Class<? extends QualifiedName> clazz)
					throws DecodingException {
		...
		addKnownFinalClassScalarSerializer(LocalizedText.class, new ScalarDecoder<LocalizedText>() {
			@Override
			public LocalizedText get(BinaryDecoder dec, String fieldName, Class<? extends LocalizedText> clazz)
					throws DecodingException {
		...
		scalarSerializerExtensionObject = new ScalarDecoder<ExtensionObject>() {
			@Override
			public ExtensionObject get(BinaryDecoder dec, String fieldName, Class<? extends ExtensionObject> clazz)
					throws DecodingException {
		...
		scalarSerializerDiagnosticInfo = new ScalarDecoder<DiagnosticInfo>() {
			@Override
			public DiagnosticInfo get(BinaryDecoder dec, String fieldName, Class<? extends DiagnosticInfo> clazz)
					throws DecodingException {
		...
		addKnownFinalClassScalarSerializer(Object.class, new ScalarDecoder<Object>() {
			@Override
			public Object get(BinaryDecoder dec, String fieldName, Class<? extends Object> clazz)
					throws DecodingException {
	 ...
	 * </p>
	 *
	 * @param in a {@link org.opcfoundation.ua.utils.bytebuffer.IBinaryReadable}
	 *           object.
	 */
	...
	public DiagnosticInfo getDiagnosticInfo(String fieldName) throws DecodingException {
		try {
			...
			return new DiagnosticInfo(additionalInfo, innerDiagnosticInfo, innerStatusCode, locale, localizedText,
					namespaceUri, symbolicId);
		} catch (IOException e) {
			throw toDecodingException(e);
		}
	}
	...
	public ExpandedNodeId getExpandedNodeId(String fieldName) throws DecodingException {
		try {
			...
			if (hasNamespaceUri)
				namespaceUri = getString(null);
			if (hasServerIndex)
				serverIndex = getUInt32(null);

			if (namespaceUri != null)
				return new ExpandedNodeId(serverIndex, namespaceUri, id, NamespaceTable.getInstance());
			else
	...
	/** {@inheritDoc} */
	@Override
	public ExtensionObject getExtensionObject(String fieldName) throws DecodingException {
		try {
			...
			if (encodingByte == 0) {
				...
				if ((typeId == null) || typeId.equals(NodeId.NULL))
					return null;
	...
	/** {@inheritDoc} */
	@Override
	public NodeId getNodeId(String fieldName) throws DecodingException {
		try {
			byte encodingByte = in.get();
	...
	public QualifiedName getQualifiedName(String fieldName) throws DecodingException {
		...
		if (name == null)
			return new QualifiedName(namespaceIndex, null);
	 ...
	 * </p>
	 *
	 * @return a {@link org.opcfoundation.ua.utils.bytebuffer.IBinaryReadable}
	 *         object.
	 */
	...
	public Variant getVariant(String fieldName) throws DecodingException {
		try {
			...
			Object value = isNull ? null
					: isArray ? getArrayObject(null, builtinType) : getScalarObject(null, builtinType);
			int[] dims = hasDimensionLengths ? getInt32Array_(null) : null;
			boolean multiDimension = isArray && dims != null && dims.length > 1;

			/*
			 * GH#53, Spec 1.03 Part 6, section 5.2.2.16 "If ArrayDimensions are
			 * inconsistent with the ArrayLength then the decoder shall stop and raise a
			 * Bad_DecodingError", therefore we must check it here.
			 */
			...
			if (value instanceof ExtensionObject && isDecimal((ExtensionObject) value)) {
				try {
					...
					throw new DecodingException("Did not get an ExtensionObject with ByteString data for Decimal type",
							e);
				}
			} else {
	 ...
	 * @throws DecodingException
	 */
	private void assertArrayLength(int len, int elementSizeInBytes) throws DecodingException {
		if (len < -1) {
	 ...
	 * @throws DecodingException
	 */
	private void assertByteStringLength(int len) throws DecodingException {
		if (len < -1) {
	 ...
	 * @throws DecodingException
	 */
	private void assertStringLength(int len) throws DecodingException {
		if (len < -1) {

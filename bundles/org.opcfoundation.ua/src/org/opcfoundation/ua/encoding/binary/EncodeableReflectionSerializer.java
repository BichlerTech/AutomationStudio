...
/**
 * Serializes {@link IEncodeable}s using reflection. This class can encode
 * anything that implements IEncodeable.
 *
...
public class EncodeableReflectionSerializer implements IEncodeableSerializer {
	 ...
	 *
	 * @param table a
	 *              {@link org.opcfoundation.ua.encoding.utils.EncodeableDescTable}
	 *              object.
	 */
	...
	public IEncodeable getEncodeable(Class<? extends IEncodeable> clazz, IDecoder decoder) throws DecodingException {
		...
		try {
			...
			for (EncodeableDesc.FieldInfo fi : info.fields) {
				// Decode builtin type
				if (fi.builtinType >= 0) {
					Object value = fi.isArray ? decoder.getArrayObject(fi.field.getName(), fi.builtinType)
							: decoder.getScalarObject(fi.field.getName(), fi.builtinType);
				...
				if (vi != null) {
					Object value = fi.isArray ? decoder.getEncodeableArray(fi.field.getName(), vi.clazz)
							: getEncodeable(vi.clazz, decoder);
					fi.field.set(result, value);
					continue;
				}
				...
				if (fi.isArray && Enumeration.class.isAssignableFrom(fi.type.getComponentType())) {
					Object value = decoder.getEnumerationArray(fi.field.getName(),
							(Class<Enumeration>) fi.type.getComponentType());
					fi.field.set(result, value);
					continue;
				}

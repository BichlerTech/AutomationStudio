...
public class XmlDecoder implements IDecoder {
	...
	public XmlDecoder(XmlElement element, EncoderContext context) throws DecodingException {
		...
		initialize();
		try {
			this.reader = XMLFactoryCache.getXMLInputFactory()
					.createXMLStreamReader(new StringReader(element.toString()));
	...
	/** {@inheritDoc} */
	@Override
	public Object getArrayObject(String fieldName, int builtinTypeId) throws DecodingException {
	...
	public <T extends IEncodeable> T getEncodeable(String fieldName, Class<? extends T> encodeableClass)
		...
		// if (encodeableClass == null) throw new
		// IllegalArgumentException("systemType");
		//
		// IEncodeable value = null;// TODO construct encodeable of the right class
		// Activator.CreateInstance(systemType) as IEncodeable;
		//
		// if (value == null)
		// {
		...
		// m_reader.LocalName == fieldName && m_reader.getNamespaceURI() ==
		// m_namespaces.peek()))
		// {
		...
		// throw new DecodingException(
		// StatusCodes.Bad_DecodingError,
		// "Unexpected end of stream decoding field '" + fieldName + "' for type '" +
		// systemType.FullName + "'.");
		// }
		//
		// skip();//m_reader.Skip();
	...
	/**
	 * <p>
	 * Getter for the field <code>encoderContext</code>.
	 * </p>
	 *
	...
	@Override
	@SuppressWarnings("unchecked")
	public <T extends Enumeration> T getEnumeration(String fieldName, Class<T> enumType) throws DecodingException {
		// #if SILVERLIGHT
	...
	public ExpandedNodeId getExpandedNodeId(String fieldName) throws DecodingException {
		...
		if (beginFieldSafe(fieldName, true)) {
			value = ExpandedNodeId.parseExpandedNodeId(getString("Identifier"), this.namespaceTable);
	...
	public ExtensionObject getExtensionObject(String fieldName) throws IllegalArgumentException, DecodingException {
		...
		ExpandedNodeId absoluteId = encoderContext.getNamespaceTable().toExpandedNodeId(typeId);// NodeId.ToExpandedNodeId(typeId,
																								// m_context.NamespaceUris);

		if (!NodeId.isNull(typeId) && ExpandedNodeId.isNull(absoluteId)) {
			logger.error(
					"Cannot de-serialized extension objects if the NamespaceUri is not in the NamespaceTable: Type = {}",
					typeId);
		}
	 ...
	 * </p>
	 *
	 * @param typeId a {@link org.opcfoundation.ua.builtintypes.ExpandedNodeId}
	 *               object.
	...
	public Object getExtensionObjectBody(ExpandedNodeId typeId) throws DecodingException {
		...
		// try {
		// clazz =
		// m_context.getEncodeableSerializer().getClass(m_context.getNamespaceTable().toNodeId(typeId));
	...
	/**
	 * <p>
	 * Getter for the field <code>namespaceTable</code>.
	 * </p>
	 *
	...
	public NodeId getNodeId(String fieldName) throws IllegalArgumentException, DecodingException {
		...
		if (namespaceMappings != null && namespaceMappings.length > value.getNamespaceIndex()) {
			value = NodeId.get(value.getIdType(), namespaceMappings[value.getNamespaceIndex()].intValue(),
					value.getValue());
		}
	...
	/** {@inheritDoc} */
	@Override
	public Object getScalarObject(String fieldName, int builtinTypeId) throws DecodingException {
		switch (builtinTypeId) {
		case 1:
	...
	/**
	 * <p>
	 * Getter for the field <code>serverTable</code>.
	 * </p>
	 *
	...
	/// <summary>
	/// This method calls IsStartElement followed by Read to position you on the
	/// content of that element found in the input stream.
	/// </summary>
	/**
	 * <p>
	 * getStartElement.
	...
	/** {@inheritDoc} */
	@Override
	public Structure getStructure(String fieldName) throws DecodingException {
		// try {
		...
		// if (encodingByte==1) {
		// Class<? extends IEncodeable> clazz =
		// m_context.getEncodeableSerializer().getClass(typeId);
		...
		return (Structure) getEncodeable(fieldName, clazz);
		// }
		// throw new DecodingException("Unexpected encoding byte ("+encodingByte+") in
		// ExtensionObject");
		// } catch (IOException e) {
	...
	/** {@inheritDoc} */
	@Override
	public Structure[] getStructureArray(String fieldName) throws DecodingException {
		...
		// if (len>remaining()) throw new DecodingException(StatusCodes.Bad_EndOfStream,
		// "Buffer underflow");
	...
	public Object getVariantContents() throws DecodingException// out TypeInfo typeInfo)
	{
		...
		// skip whitespace.
		while (reader.getEventType() != XMLStreamConstants.START_ELEMENT)// XmlNodeType.Element) TODO should there also
																			// be END_ELEMENT?
		{
			try {
				reader.next();
		...
		if (typeName.startsWith("ListOf")) {
			...
			if (type.equals("Boolean")) {
				return getBooleanArray(typeName);
			} else if (type.equals("SByte")) {
				return getSByteArray(typeName);
			} else if (type.equals("Byte")) {
				return getByteArray(typeName);
			} else if (type.equals("Int16")) {
				return getInt16Array(typeName);
			} else if (type.equals("UInt16")) {
				return getUInt16Array(typeName);
			} else if (type.equals("Int32")) {
				return getInt32Array(typeName);
			} else if (type.equals("UInt32")) {
				return getUInt32Array(typeName);
			} else if (type.equals("Int64")) {
				return getInt64Array(typeName);
			} else if (type.equals("UInt64")) {
				return getUInt64Array(typeName);
			} else if (type.equals("Float")) {
				return getFloatArray(typeName);
			} else if (type.equals("Double")) {
				return getDoubleArray(typeName);
			} else if (type.equals("String")) {
				return getStringArray(typeName);
			} else if (type.equals("DateTime")) {
				return getDateTimeArray(typeName);
			} else if (type.equals("Guid")) {
				return getGuidArray(typeName);
			} else if (type.equals("ByteString")) {
				return getByteStringArray(typeName);
			} else if (type.equals("XmlElement")) {
				return getXmlElementArray(typeName);
			} else if (type.equals("NodeId")) {
				return getNodeIdArray(typeName);
			} else if (type.equals("ExpandedNodeId")) {
				return getExpandedNodeIdArray(typeName);
			} else if (type.equals("StatusCode")) {
				return getStatusCodeArray(typeName);
			} else if (type.equals("DiagnosticInfo")) {
				return getDiagnosticInfoArray(typeName);
			} else if (type.equals("QualifiedName")) {
				return getQualifiedNameArray(typeName);
			} else if (type.equals("LocalizedText")) {
				return getLocalizedTextArray(typeName);
			} else if (type.equals("ExtensionObject")) {
				return decode(getExtensionObjectArray(typeName));
			} else if (type.equals("DataValue")) {
				return getDataValueArray(typeName);
			} else if (type.equals("Variant")) {
				return getVariantArray(typeName);
			}
		}
		...
		// process scalar types.
			...
			if (typeName.equals("Null")) {
				...
				return getBoolean(typeName);
			} else if (typeName.equals("SByte")) {
				return getSByte(typeName);
			} else if (typeName.equals("Byte")) {
				return getByte(typeName);
			} else if (typeName.equals("Int16")) {
				return getInt16(typeName);
			} else if (typeName.equals("UInt16")) {
				return getUInt16(typeName);
			} else if (typeName.equals("Int32")) {
				return getInt32(typeName);
			} else if (typeName.equals("UInt32")) {
				return getUInt32(typeName);
			} else if (typeName.equals("Int64")) {
				return getInt64(typeName);
			} else if (typeName.equals("UInt64")) {
				return getUInt64(typeName);
			} else if (typeName.equals("Float")) {
				return getFloat(typeName);
			} else if (typeName.equals("Double")) {
				return getDouble(typeName);
			} else if (typeName.equals("String")) {
				return getString(typeName);
			} else if (typeName.equals("DateTime")) {
				return getDateTime(typeName);
			} else if (typeName.equals("Guid")) {
				return getGuid(typeName);
			} else if (typeName.equals("ByteString")) {
				return getByteString(typeName);
			} else if (typeName.equals("XmlElement")) {
				return getXmlElement(typeName);
			} else if (typeName.equals("NodeId")) {
				return getNodeId(typeName);
			} else if (typeName.equals("ExpandedNodeId")) {
				return getExpandedNodeId(typeName);
			} else if (typeName.equals("StatusCode")) {
				return getStatusCode(typeName);
			} else if (typeName.equals("DiagnosticInfo")) {
				return getDiagnosticInfo(typeName);
			} else if (typeName.equals("QualifiedName")) {
				return getQualifiedName(typeName);
			} else if (typeName.equals("LocalizedText")) {
				return getLocalizedText(typeName);
			} else if (typeName.equals("ExtensionObject")) {
				ExtensionObject extensionObject = getExtensionObject(typeName);
				try {
			...
			// }
				...
				return getMatrix(typeName);
			}
		}

		throw new DecodingException(StatusCodes.Bad_DecodingError, "Element '" + reader.getNamespaceURI() + ":"
				+ reader.getLocalName() + "' is not allowed in an Variant.");
		// }
		// finally
		// {
		// m_namespaces.pop();
	 ...
	 * @throws org.opcfoundation.ua.encoding.DecodingException if any.
	 */
	public boolean loadStringTable(String tableName, String elementName, List<String> stringTable)
			throws DecodingException {
		// pushNamespace(OPC_UA_XSD_NAMESPACE);
	...
	/**
	 * <p>
	 * Setter for the field <code>encoderContext</code>.
	 * </p>
	 *
	...
	/**
	 * Define the namespace table to use for mapping the namespace indexes of the
	 * XML data to the application data.
	 *
	 * @param namespaceTable a {@link org.opcfoundation.ua.common.NamespaceTable}
	 *                       object.
	 */
	...
	/**
	 * Define the server table to use for mapping the server indexes of the XML data
	...
	private boolean beginField(String fieldName, boolean isOptional) throws DecodingException {
		...
		if (!isStartElement(fieldName))// , m_namespaces.peek()))
		{
			if (!isOptional) {
				throw new DecodingException(StatusCodes.Bad_DecodingError,
						String.format("Encountered element: '{1}:{0}' when expecting element: '{2}'.",
								reader.getLocalName(), reader.getNamespaceURI(), fieldName));// , m_namespaces.peek()));
			}
			return false;
		}
	...
	/// <summary>
	/// Reads the start of filed where the presences of the xsi:nil attribute is not
	/// significant.
	/// </summary>
	...
	private UnsignedShort[] createMapping(UriTable source, UriTable table, boolean updateTable) {
		if (source == null) {
			return null;
		}
	...
	private void endField(String fieldName) throws DecodingException {
		if (!isNullOrEmpty(fieldName)) {
			...
			if (eventType != XMLStreamConstants.END_ELEMENT)
				throw new DecodingException(StatusCodes.Bad_DecodingError, "No end element found: '" + localName + ":"
						+ reader.getNamespaceURI() + "' eventType=" + eventType);

			if (!localName.equals(fieldName))// || m_reader.getNamespaceURI() != m_namespaces.peek())
			{
				throw new DecodingException(StatusCodes.Bad_DecodingError, "Encountered end element: '" + localName
						+ ":" + reader.getNamespaceURI() + "' when expecting element: '" + fieldName +
						// ":"+
						// m_namespaces.peek()+
						"'.");
			}
	...
	private BigDecimal parseDecimalFromExtensionObjectXMLElementData(XmlElement data) throws DecodingException {
		 ...
		 * Other places define it as signed. Assuming it is signed as that makes more
		 * sense, as it allows arbitrary large numbers as well by defining negative
		 * scaling.
		 */
	 ...
	 * Element, EndElement, EntityReference, or EndEntity) node. If the node is not
	 * a content node, the reader skips ahead to the next content node or end of
	 * file. It skips over nodes of the following type: ProcessingInstruction,
	...
	private boolean moveToElement(String elementName) throws DecodingException {
		while (!reader.isStartElement()) {
			if (/*
				 * m_reader.getEventType() == XmlStreamConstants.None || TODO is this necessary?
				 */reader.getEventType() == XMLStreamConstants.END_ELEMENT) {
				return false;
			}
	...
	private void skip(QName qname) throws DecodingException {
		...
		while (depth > 0) {
			if (reader.getEventType() == XMLStreamConstants.END_ELEMENT) {
				if (reader.getLocalName().equals(qname.getLocalPart())
						&& reader.getNamespaceURI().equals(qname.getNamespaceURI())) {
					depth--;
				}
			}

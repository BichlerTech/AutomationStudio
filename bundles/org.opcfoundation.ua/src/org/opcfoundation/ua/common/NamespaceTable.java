 ...
 *
 * Use {@link #add} to add entries to the table. Use {@link #getIndex} to find
 * the index of an URI or {@link #getUri} to find the Uri of an index.
 */
public class NamespaceTable extends UriTable {
	...
	private static NamespaceTable instance;
	...
	/**
	 * Compare 2 ExpandedNodeId objects. This method is intended for cases where one
	 * ExpandedNodeId is defined with NamespaceUri and another is defined with
	 * NamespaceIndex.
	 *
	 * @param n1 first
	 * @param n2 second
	 ...
	 * where the ExpandedNodeId is defined with NamespaceUri and a comparison to
	 * NodeId which has NamespaceIndex is wanted.
	 *
	 * @param n1 first
	 * @param n2 second
	 * @return true if they are equal
	...
	public ExpandedNodeId toExpandedNodeId(NodeId nodeId) {
		return new ExpandedNodeId(null, getUri(nodeId.getNamespaceIndex()), nodeId.getValue(), this);
	}
	 ...
	 * @throws org.opcfoundation.ua.common.ServiceResultException if there is no
	 *                                                            entry for the
	 *                                                            namespaceUri used
	 *                                                            in the
	 *                                                            expandedNodeId
	...
	public NodeId toNodeId(ExpandedNodeId expandedNodeId) throws ServiceResultException {
		...
		if (uri == null)
			return NodeId.get(expandedNodeId.getIdType(), expandedNodeId.getNamespaceIndex(),
					expandedNodeId.getValue());
		int index = this.getIndex(uri);
		if (index < 0)
			throw new ServiceResultException("Index for uri \"" + uri + "\" not found in NamespaceTable");
		return NodeId.get(expandedNodeId.getIdType(), index, expandedNodeId.getValue());
	}

	/**
	 * Check if the node IDs refer to the same name space. Compares the
	 * NamespaceIndex of the IDs.
	 *
	 ...
	 *
	 * @param nodeId         a {@link org.opcfoundation.ua.builtintypes.NodeId}
	 *                       object.
	 * @param expandedNodeId a
	...
	public boolean namespaceEquals(NodeId nodeId, ExpandedNodeId expandedNodeId) {
		int expandedNamespaceIndex = expandedNodeId.getNamespaceUri() != null
				? getIndex(expandedNodeId.getNamespaceUri())
				: expandedNodeId.getNamespaceIndex();
	 ...
	 * checked for the NamespaceUri or Index depending on which is used.
	 *
	 * @param expandedNodeId1 a
	 *                        {@link org.opcfoundation.ua.builtintypes.ExpandedNodeId}
	 *                        object.
	 * @param expandedNodeId2 a
	 *                        {@link org.opcfoundation.ua.builtintypes.ExpandedNodeId}
	 *                        object.
	 * @return true if the nodes are in the same name space
	 */
	public boolean namespaceEquals(ExpandedNodeId expandedNodeId1, ExpandedNodeId expandedNodeId2) {
		int expandedNamespaceIndex1 = expandedNodeId1.getNamespaceUri() != null
				? getIndex(expandedNodeId1.getNamespaceUri())
				: expandedNodeId1.getNamespaceIndex();
		int expandedNamespaceIndex2 = expandedNodeId2.getNamespaceUri() != null
				? getIndex(expandedNodeId2.getNamespaceUri())
				: expandedNodeId2.getNamespaceIndex();
		return expandedNamespaceIndex1 == expandedNamespaceIndex2;
	}

	/**
	 * <p>
	 * Getter for the field <code>defaultInstance</code>.
	 * </p>
	 *
	 * @return a default instance which can be used when no other namespace table is
	 *         available. DO NOT use, if you have a valid application context with
	 *         an initialized namespace table available.
	 */
	 ...
	 * </p>
	 *
	 * @return a default instance which can be used, it must be filled at server
	 *         startup.
	 */
	public static NamespaceTable getInstance() {
		if (instance == null)
			instance = NamespaceTable.getDefaultInstance();
		return instance;
	}

...
public class Client {
	 ...
	 * </p>
	 *
	 * @return a {@link org.opcfoundation.ua.transport.tcp.io.OpcTcpSettings}
	 *         object.
	 */
	 ...
	 * @param maxResponseMessageSize  max size of response messages - if null, use
	 *                                4194304
	 * @param requestedSessionTimeout requested session time out (in ms) - if null,
	 *                                use 3600000 (one hour)
	 * @param sessionName             session name - if null a random GUID is used
	 ...
	 *
	 * @param channel                 open channel to use
	 * @param maxResponseMessageSize  max size of response messages - if null,
	 *                                defaulting to 4194304
	 * @param requestedSessionTimeout requested session timeout (ms) - if null,
	 *                                defaulting to 3600000 (one hour)
	 * @param sessionName             session name - if null, a random GUID is used
	 *                                to generate the name
	 * @param discoveredEndpoints     list of previously discovered Endpoints using
	 *                                GetEndpoints service. This list is checked
	 *                                against the Endpoint list received from
	 *                                CreateSessionResponse, if it does not match,
	 *                                an exception is thrown, if null the check is
	 *                                disabled
	 * @return the session
	 * @throws java.lang.IllegalArgumentException                 if endpoint or
	 *                                                            channel is null
	 * @throws org.opcfoundation.ua.common.ServiceResultException if error
	 */
	public Session createSession(SecureChannel channel, UnsignedInteger maxResponseMessageSize,
		...
		if (!MessageSecurityMode.None.equals(channel.getMessageSecurityMode())) {
			...
			// certificate
			boolean ok = CryptoUtil.getCryptoProvider().verifyAsymm(
					session.serverCertificate.getCertificate().getPublicKey(), algorithm, dataServerSigned,
					ByteString.asByteArray(serverSignature.getSignature()));
			if (!ok)
		...
		/*
		 * Check that the Endpoint list received from the response match the ones given
		 * as parameter, i.e. Endpoint list from the GetEndpoint/Discovery service.
		 * Based on spec part 4: 5.6.2.1, the client shall verify that
		 * EndpointDescriptions with matching transportProfileUri match.
		 * 
		 * Based on spec part 4: 5.6.2.2. (Table 11) we need to verify the following
		 * parameters: endpointUrl, securityMode, securityPolicyUri, userIdentityTokens,
		 * transportProfileUri and securityLevel. In case the endpoints differ, the
		 * client shall close the Session and report error.
		 */
	 ...
	 * separately. SessionChannel.closeSession() SessionChannel.closeSecureChannel()
	 *
	...
	/**
	 * Create a secure channel to a UA Server This method first queries endpoints,
	 * chooses the most suitable and connects to it.
	 *
	 * @param endpointUrl Endpoint identifier and socket address
	 ...
	 * <p>
	 * This implementation accepts only connections with opc.tcp protocol and with
	 * encryption.
	 * <p>
	 * Note this implementation is unsafe as the dialog with discover endpoint is
	 * not encrypted.
	 *
	 * @param applicationDescription application description
	 * @return service channel
	...
	public SecureChannel createSecureChannel(String connectUrl, EndpointDescription endpoint)
		...
		// hack to
		settings.setOpctcpSettings(null);
		settings.setHttpsSettings(null);
		settings.setConfiguration(null);
		settings.setNamespaceUris(null);
...
//		}
	 ...
	 *
	 * @param connectUrl        address that contains the socket address to the
	 *                          endpoint
	 ...
	 * 
	 * The EndpointUrl of the EndpointDescription in the given
	 * {@link TransportChannelSettings} must be sent by the server in the
	 * ReverseHello Message.
	 * 
	 * @param localEndpointUrl,    The client opens a server socket based on this
	 *                             address, listens server sent ReverseHello
	 * @param serverApplicationUri expected Server(Application)Uri of the
	 *                             ReverseHello
	...
	/**
	 * Create a service channel to a UA Server This method first queries endpoints,
	 * chooses the most suitable and connects to it.
	 *
	 ...
	 * <p>
	 * Note this implementation is unsecure as the dialog with discover endpoint is
	 * not encrypted.
	 *
	 * @param applicationDescription application description
	 * @return service channel
	 ...
	 * @throws org.opcfoundation.ua.common.ServiceResultException Error that occured
	 *                                                            while processing
	 *                                                            the operation.
	 */
	 ...
	 * @param connectUrl           address that contains the socket address to the
	 *                             endpoint
	 * @param discoveryEndpointUri the endpoint identifier that identifies the
	 *                             endpoints that are being discovered
	 * @return Endpoint Descriptions
	 * @throws org.opcfoundation.ua.common.ServiceResultException Error that occured
	 *                                                            while processing
	 *                                                            the operation.
	 ...
	 * Discover applications
	 *
	 * @param connectUrl                address that contains the socket address to
	 *                                  the endpoint
	...
	public ApplicationDescription[] discoverApplications(String connectUrl, String discoverServerEndpointUri)
		...
		ChannelService chan = new ChannelService(channel);
		try {
			FindServersRequest req = new FindServersRequest(null, discoverServerEndpointUri, new String[0],
					new String[0]);
			req.setRequestHeader(new RequestHeader());
			req.getRequestHeader().setTimeoutHint(UnsignedInteger.valueOf(getTimeout()));
	...
	public ServerOnNetwork[] discoverServersOnNetwork(String connectUrl, String discoverServerEndpointUri,
		...
		return discoverServersOnNetwork(connectUrl, discoverServerEndpointUri, UnsignedInteger.ZERO,
				UnsignedInteger.ZERO, serverCapabilities).getServers();
	}

	/**
	 * Get FindServersOnNetworkResponse to retrieve ServerOnNetwork records and
	 * lastCounterResetTime associated with them
	 * 
	 * @param connectUrl                address that contains the socket address to
	 *                                  the endpoint
	 * @param discoverServerEndpointUri endpointURI or ""
	 ...
	 * @param serverCapabilities        list of server capability filters
	 * @return response to the FindServersOnNetworkRequest constructed from the
	 *         parameters
	 * @throws org.opcfoundation.ua.common.ServiceResultException if error
	 */
	public FindServersOnNetworkResponse discoverServersOnNetwork(String connectUrl, String discoverServerEndpointUri,
		...
		ChannelService chan = new ChannelService(channel);
		try {
			FindServersOnNetworkRequest req = new FindServersOnNetworkRequest(null, startingRecordId,
	 ...
	 * </p>
	 *
	 * @param endpointConfiguration a
	 *                              {@link org.opcfoundation.ua.core.EndpointConfiguration}
	 *                              object.
	 */
	...
	private void validateEndpoints(String uri, EndpointDescription[] endpoints,
		...
		/*
		 * We need to verify the following parameters: endpointUrl, securityMode,
		 * securityPolicyUri, userIdentityTokens, transportProfileUri and securityLevel.
		 * Since .equals does compare all these, lets use it. It also compares server
		 * ApplicationDescription and Certificate, so we clone and set those to null
		 */

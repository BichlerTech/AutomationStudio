...
public class SessionChannel extends ChannelService implements RequestChannel {
	...
	public ActivateSessionResponse activate(String username, String password) throws ServiceResultException {
		UserIdentityToken token = EndpointUtil.createUserNameIdentityToken(session.getEndpoint(),
				session.getServerNonce(), username, password);
		return activate(token, null);
	}
	...
	public ActivateSessionResponse activate(byte[] issuedIdentityToken) throws ServiceResultException {
		UserIdentityToken token = EndpointUtil.createIssuedIdentityToken(session.getEndpoint(),
				session.getServerNonce(), issuedIdentityToken);
		return activate(token, null);
	}
	 ...
	 * @param identitySignature used with {@link X509IdentityToken} and
...
//		session.serverNonce = ByteString.asByteArray(asres.getServerNonce());
	 ...
	 *
	 * This convenience method logs the errors to default Logger but doesn't throw
	 * exceptions. Rationale is that in typical case, the client cannot handle close
	 * errors.
	 *

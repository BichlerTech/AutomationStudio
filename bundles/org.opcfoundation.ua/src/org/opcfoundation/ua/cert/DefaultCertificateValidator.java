...
public class DefaultCertificateValidator implements CertificateValidator {
	 ...
	 * provide additional custom handling. For example to enable prompting the user
	 * if he wants to trust a certificate which is not trusted otherwise.
	 *
			...
			try {
				...
				for (Cert c : trustedCerts) {
					try {
						...
						if (issuerResult.isStatusCode(StatusCodes.Bad_CertificateRevoked))
							result = new StatusCode(StatusCodes.Bad_CertificateIssuerRevoked);
						else if (issuerResult.isStatusCode(StatusCodes.Bad_CertificateTimeInvalid))
							result = new StatusCode(StatusCodes.Bad_CertificateIssuerTimeInvalid);
						else if (issuerResult.isStatusCode(StatusCodes.Bad_CertificateChainIncomplete)
								|| issuerResult.isStatusCode(StatusCodes.Bad_CertificateIssuerRevoked)
								|| issuerResult.isStatusCode(StatusCodes.Bad_CertificateIssuerTimeInvalid))
							result = issuerResult;
						else if (issuerResult.isNotGood())
							result = new StatusCode(StatusCodes.Bad_CertificateInvalid);
					} catch (GeneralSecurityException e1) {
						continue;
					}
				}
				if (!issuerFound)
					result = new StatusCode(StatusCodes.Bad_CertificateChainIncomplete);
				if (result.isNotGood()) {
				...
				try {
					...
					if (e.getCause().getMessage().contains(INVALID_URI_ERROR)) {
						...
						if ((causeSplit.length == 2) && causeSplit[1].equals(applicationUriOfApplicationDescription)) {
							...
							logger.warn("The provided certificate contains an invalid ApplicationURI: {}",
									causeSplit[1]);
	 ...
	 * @param passedChecks           the certificate checks that were OK
	 * @return how to handle the certificate: trust or reject; once or permanently?
	 */

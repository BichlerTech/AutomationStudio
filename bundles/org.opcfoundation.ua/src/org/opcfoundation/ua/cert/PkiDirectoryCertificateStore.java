...
public class PkiDirectoryCertificateStore implements CertificateStore {
	 ...
	 * ("certs"), rejectedDir ("rejected") and revokedDir ("crl").
	 *
	 * @param baseDir the path to the base directory where the certificate
	 ...
	 *
	 * @param baseDir       the path to the base directory where the certificate
	 *                      directories are kept. Use null, if you wish to define
	 *                      the other directories with absolute paths.
	 * @param trustedDir    the name of the directory in which the trusted
	 *                      certificates are kept. If baseDir is defined, this is a
	 *                      relative path to that, e.g. a subdirectory name.
	 * @param rejectedDir   the name of the directory in which the rejected
	 *                      certificates are kept. If baseDir is defined, this is a
	 *                      relative path to that, e.g. a subdirectory name.
	 * @param revocationDir the name of the directory in which revoked certificates
	 *                      are kept. If baseDir is defined, this is a relative path
	 *                      to that, e.g. a subdirectory name.
	 */
	public PkiDirectoryCertificateStore(String baseDir, String trustedDir, String rejectedDir, String revocationDir) {
	...
	/**
	 * Add a certificate revocation list to the store. NOTE! currently it is NOT
	 * written to the disk.
	 * 
	 ...
	 * stored in the certificate store or not. If they are stored, they are stored
	 * in the RejectedCertificates. You may want to store them to be able to
	 * validate the certificate contents later on from the file.
	 * <p>
	 * Default: true
	 *
	 ...
	 * outside of this instance (e.g. move files on disk) and want to refresh before
	 * a validateCertificate call happens (which does this automatically).
	 */
	public void refresh() {
		init();
	}
	 ...
	 *
	 * @param removeFiles if true, removes all certificate files as well. Be careful
	 *                    with this!
	 */
	...
	private void initCertificates(Map<String, Cert> certificates, File dir,
			Map<String, Cert> removeFromCertificateList) {
		if (!dir.exists())
			dir.mkdirs();
		if (dir.isDirectory())
	...
	private void initCRL(File file) {
		try {
			...
			revocationLists.add(crl);
			logger.info("CRL initialized from " + file + ": "
					+ (crl.getRevokedCertificates() == null ? "no revoked certificates"
							: crl.getRevokedCertificates().size() + " certificates revoked"));
		} catch (Exception e) {

  ...
  class DatapointsParser extends DefaultHandler
  {
    ...
    public void startElement(String uri, String localname, String name, Attributes attrs) throws SAXException
    {
      ...
      else if (name.compareTo("operation") == 0)
      {
        if (actNode != null)
        {
          actNode.getScript().append(attrs.getValue("value").replace("$lower$", "<").replace("$greater$", ">"));
          actNode.getCalculationExpressions().add(
              new CalculationExpression(attrs.getValue("value").replace("$lower$", "<").replace("$greater$", ">")));
        }
      }
        ...
        if (actNode != null)
        {
          try
          {
            ...
            if (arrayIndex > -1)
            {
              ...
              actNode.getScript()
                  .append("((ComDRVManager.getDRVManager().readFromDriver(NodeId.parseNodeId(\"" + id.toString()
                      + "\"), Attributes.Value, \"" + arrayIndex + "\", null, new Long(" + drvId
                      + "), new Double(0.0), TimestampsToReturn.Both).getValue().getValue())[0])" + dt[1]);
              // Object obj = ((Boolean))
              // Object obj =
            ...
            else
            {
              actNode.getScript()
                  .append("ComDRVManager.getDRVManager().readFromDriver(NodeId.parseNodeId(\"" + id.toString()
                      + "\"), Attributes.Value, null, null, new Long(" + drvId

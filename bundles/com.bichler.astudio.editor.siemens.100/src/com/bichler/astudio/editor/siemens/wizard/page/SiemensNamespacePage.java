...
public class SiemensNamespacePage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		Label lblStartknoten = new Label(container, SWT.NONE);
		lblStartknoten.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblStartknoten.setText(
				CustomString.getString(SiemensActivator.getDefault().RESOURCE_BUNDLE, "siemens.namespace.startnode"));
		this.txt_startnode = new Text(container, SWT.BORDER);
		this.txt_startnode.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		this.btn_browse = new Button(container, SWT.NONE);
		btn_browse.setImage(SiemensSharedImages.getImage(SiemensSharedImages.ICON_SEARCH));
		btn_browse.setToolTipText(
				CustomString.getString(SiemensActivator.getDefault().RESOURCE_BUNDLE, "siemens.namespace.browse"));
		Label lblStartknotenName = new Label(container, SWT.NONE);
		lblStartknotenName.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblStartknotenName.setText(CustomString.getString(SiemensActivator.getDefault().RESOURCE_BUNDLE,
				"siemens.namespace.startnodename"));
		this.txt_startnodeName = new Text(container, SWT.BORDER);
		this.txt_startnodeName.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1));
	...
	private void fill() {
		...
		txt_startnode.setData(Identifiers.ObjectsFolder);
	...
	private void setHandler() {
		this.btn_browse.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				NodeId data = (NodeId) txt_startnode.getData();
				if (!NodeId.isNull(data)) {
					dialog.setSelectedNodeId(data);
				}
				dialog.setFormTitle(CustomString.getString(SiemensActivator.getDefault().RESOURCE_BUNDLE,
						"siemens.dialog.search.node.title"));
				if (dialog.open() == Dialog.OK) {
					...
					txt_startnode.setData(dialog.getSelectedNodeId());
					txt_startnode.setText(path);
				}
	...
	public String getStartNodeName() {
		return this.txt_startnodeName.getText();
	}

	public NodeId getStartNodeId() {
		return (NodeId) this.txt_startnode.getData();
	}

	public void setStartNodeName(String name) {
		this.txt_startnodeName.setText(name);
	}

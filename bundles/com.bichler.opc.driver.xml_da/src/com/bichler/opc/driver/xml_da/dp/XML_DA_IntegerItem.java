...
public class XML_DA_IntegerItem extends XML_DA_DPItem {
	private int[] internArray = null;
	private ArrayOfInt intern = new ArrayOfInt();
	...
	public void setArraylength(int arraylength) {
		...
		this.internArray = new int[arraylength];
	}

	@Override
	protected Object createDRVValueArray() {
		for (int i = 0; i < arraylength; i++) {
			this.internArray[i] = ((Integer) this.array[i]).intValue();
		}
		intern.set_int(internArray);
		return this.intern;
	}
		...
		try {
			if (this.transform != null) {
				...
				if (mapping == XML_DA_MAPPING_TYPE.SCALAR) {
					...
					try {
						ArrayOfInt values = (ArrayOfInt) data;
						// verify opc ua array length and xml-da length
						if (values.get_int().length != this.arraylength) {
							String message = "";
							if (values.get_int().length > this.arraylength) {
						...
						for (int i = 0; i < values.get_int().length; i++) {
							this.array[i] = this.transform.transToIntern(values.get_int(i));
						}
						this.value.setValue(new Variant(this.array));
					} catch (ClassCastException ex) {
						// we couldn't cast xml - da array to ArrayOfInt, so
						...
						Logger.getLogger(getClass().getName()).log(Level.SEVERE,
								"Couldn't cast XML-DA value to OPC UA Value! Data point: " + this.getDisplayname()
										+ " Required datatype: ArrayOfInt Received: "

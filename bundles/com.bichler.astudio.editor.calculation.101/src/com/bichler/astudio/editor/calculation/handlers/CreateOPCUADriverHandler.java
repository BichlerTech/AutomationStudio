...
import com.bichler.astudio.editor.calculation.CalculationActivator;
...
public class CreateOPCUADriverHandler extends AbstractOPCCreateDriverModel
{
  ...
  public Object execute(ExecutionEvent event) throws ExecutionException
  {
    ...
    File defaultDrvStore = CalculationActivator.getDefault().getFile(CalculationActivator.getDefault().getBundle(),
    ...
    if (!filesystem.isDir(driverPath))
    {
      ...
      try (InputStream is = filesystem.readFile(defaultDrvStore.getAbsolutePath());
          OutputStream output = filesystem.writeFile(new Path(driverPath).append("driver.com").toOSString());

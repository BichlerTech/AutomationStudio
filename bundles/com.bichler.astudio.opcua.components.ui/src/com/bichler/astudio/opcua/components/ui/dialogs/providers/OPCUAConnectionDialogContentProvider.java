...
public class OPCUAConnectionDialogContentProvider implements ITreeContentProvider {
	...
	public OPCUAConnectionDialogContentProvider(Label errorMessage) {
	...
	public Object[] getChildren(Object parentElement) {
		if (parentElement instanceof ApplicationDescription) {
			UADiscoveryClient discoveryClient = new UAClient()
					.createDiscoveryClient();
			Map<String, EndpointDescription[]> discoveryResults = null;
			String[] discUris = null;

			List<String> urls = new ArrayList<String>();
			discUris = ((ApplicationDescription) parentElement)
					.getDiscoveryUrls();
			...
			for (String uri : discUris) {
				if (alwaysIP) {
					try {
						...
						uri = discoveryURIToMAP.toASCIIString().replaceFirst(
								discoveryURIToMAP.getHost(),
								endpointURI.getHost());
				...
				try {
					discoveryResults = discoveryClient.getEndpoints(alwaysValid,
							new String[] { uri });
					...
					if (e instanceof ServiceFaultException) {
						urls.add("Error: "
								+ ((ServiceFaultException) e).getServiceFault()
										.getResponseHeader().getServiceResult()
										.getName());
					}
				}
			}
			if (urls.isEmpty()) {
				if (!errorMessage.isDisposed()) {
					errorMessage.getDisplay().asyncExec(new Runnable() {
						...
						public void run() {
							errorMessage
									.setText("GetEndpoints service received no results!");
			...
			try {
				discoveryResult = discoveryClient.getEndpoints(alwaysValid,
	...
	public boolean hasChildren(Object element) {
		if (element instanceof ApplicationDescription) {
			return (((ApplicationDescription) element).getDiscoveryUrls() != null && ((ApplicationDescription) element)
					.getDiscoveryUrls().length > 0);
